...
\begin{frame}{Fase I: Algoritmo de Fulkerson}
	...
	\begin{enumerate}
		\item \textbf{Inicializar}:
		      ...
		      \end{itemize}
		\item \textbf{Enquanto} existir fonte $R_k$ em $\mathcal{C}(D_0)$ com $r \notin R_k$:
		      \begin{itemize}
			      ...
			      \item $F := F \cup \{f_k\}$
			      \item $c := c - \lambda_k \mathbf{1}_{\delta^-(R_k)}$ {\small (reduzir custos)}
...
\begin{frame}{Fase I: Iteração 2 - Análise da Condensação}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Estado:} $F = \{(b,a)\}$

		\vspace{0.2cm}
		\textbf{Passo 1:} Construir $D_0 = (V, F)$
		\begin{itemize}
			\item CFCs de $D_0$: $\{r\}, \{b\}, \{a\}, \{c\}$
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Passo 2:} Identificar fontes em $\mathcal{C}(D_0)$
		\begin{itemize}
			\item Fonte: CFC sem arco entrando \emph{em $D_0$}
			\item Todas são fontes: $\{r\}, \{b\}, \{a\}, \{c\}$
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Passo 3:} Escolher $r$-conjunto
		\begin{itemize}
			\item $r$-conjuntos: $\{b\}, \{c\}$ (não contêm $r$)
		\end{itemize}
		...
		\vspace{0.2cm}
		{\small Digrafo com custos reduzidos}
	\end{columns}
	\begin{alertblock}{Observação Importante}
		Em $D_0 = (V, \{(b,a)\})$, todos os vértices são fontes! Mas apenas $\{b\}$ e $\{c\}$ são $r$-conjuntos.
	\end{alertblock}
\end{frame}

\begin{frame}{Fase I: Iteração 2 - Seleção do Arco}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Escolha:} $R_1 = \{c\}$

		\vspace{0.2cm}
		Arcos que entram em $\{c\}$ no digrafo original:
		\begin{itemize}
			\item $(a,c): 4$ ← único arco
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Seleção:}
		\begin{itemize}
			\item $\lambda_1 = 4$ (peso mínimo)
			\item $f_1 = (a,c)$
			\item Adicionar $(a,c)$ a $F$
		\end{itemize}
...
\begin{frame}{Fase I: Iteração 2 - Redução de Custos}
	\begin{columns}[c]
		...
		\begin{itemize}
			\item CFCs: $\{r\}, \{b\}, \{a\}, \{c\}$ (singleton)
			\item Fontes em $D_0$: $\{r\}, \{b\}$
			\item Única fonte que contém $r$: $\{r\}$ \checkmark
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Mas atenção: $\{b\}$ ainda é r-conjunto!}
...
\begin{frame}{Fase I: Iteração 3 - Cobrir $\{b\}$}
	\begin{columns}[c]
		\column{0.48\textwidth}
		...
		\begin{itemize}
			...
			\item $\{b\}$ é r-conjunto não coberto!
		\end{itemize}
...
\begin{frame}{Fase I: Iteração 3 - Estado Final}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Atualização:}
		\begin{itemize}
		...
		\textbf{Redução:}
		\begin{itemize}
			\item $c(r,b) = 10 - 10 = 0$ \checkmark
		\end{itemize}

		\vspace{0.2cm}
		$\mathcal{C}(D_0)$ com $F = \{(b,a), (a,c), (r,b)\}$:
		\begin{itemize}
			\item CFC única: $\{r,b,a,c\}$ (todos conectados!)
			\item Apenas 1 fonte contendo $r$ \checkmark
		\end{itemize}
