...
\begin{frame}{Escolha Gulosa}
	\begin{columns}[c]
		...
		\vspace{0.2cm}
		{\small Prova: Para qualquer outra $r$-arborescência $F$, temos $w(T) \le w(F)$.}
		...
		\begin{tikzpicture}[scale=0.75, >=Stealth,
			...
			\draw[other] (b) to[bend left=12] node[left,font=\scriptsize,text=black] {4} (a);
			\draw[other] (a) to[bend left=14] node[right,font=\scriptsize,text=black] {5} (b);
			\draw[other] (r) .. controls (0.8,2.4) and (5.2,3.0) .. node[yshift=4pt,pos=0.53,font=\scriptsize,text=black] {3} (c);
			\draw[other] (a) -- node[right,font=\scriptsize,text=black] {3} (d);
			\draw[other] (c) .. controls (7.6,2.6) and (7.6,-2.6) .. node[xshift=8pt,pos=0.52,font=\scriptsize,text=black] {5} (e);

			\draw[chosen] (r) -- node[above,font=\scriptsize,text=blue] {1} (a);
			\draw[chosen] (r) -- node[below,font=\scriptsize,text=blue] {2} (b);
			\draw[chosen] (a) -- node[above,font=\scriptsize,text=blue] {1} (c);
			\draw[chosen] (b) -- node[above,font=\scriptsize,text=blue] {1} (d);
			\draw[chosen] (b) -- node[below,font=\scriptsize,text=blue] {2} (e);
		\end{tikzpicture}
	\end{columns}

	\vspace{0.2cm}
	\begin{alertblock}{Resultado}
		$T = \{(r,a), (r,b), (a,c), (b,d), (b,e)\}$ é uma $r$-arborescência de custo mínimo!
	\end{alertblock}
\end{frame}
...
\begin{frame}{E quando a escolha gulosa falha?}
	\begin{columns}[c]
		...
		\begin{enumerate}
			\item Reduzir custos
			\item Identificar ciclos
			\item Contrair ciclos
...
\begin{columns}[t] % alinhamento pelo topo
	...
	\begin{alertblock}{Objetivo}
		...
		\begin{itemize}
			...
			\item Contração de ciclos e recursão.
		\end{itemize}
		\vspace{4mm}
	\end{alertblock}
...
\begin{frame}{Custos Reduzidos}
	\begin{columns}[c]
		...
		$T$ é ótima em $(D, w)$ se, e somente se, $T$ é ótima em $(D, w_\lambda)$.
		...
		\begin{tikzpicture}[>=Stealth, node distance=2.0cm,
			...
			\draw[arc] (r) to[bend left=8] node[midway, above, sloped] {\scriptsize 2} (a);
...
\begin{columns}[c]
	\column{0.5\textwidth}
	\textbf{Escolha gulosa:}

	Para cada $v \neq r$, calcular:
	$$\lambda(v) := \min\{w(a) : a \in \delta^-(v)\}$$

	\vspace{0.1cm}
	Custos $\lambda$-reduzidos:
	$$w_\lambda(uv) := w(uv) - \lambda(v)$$

	\vspace{0.1cm}
	\textbf{Valores de $\lambda$:}
	\begin{itemize}
		\item $\lambda(a) = 1$, $\lambda(b) = 2$
		\item $\lambda(c) = 1$, $\lambda(d) = 1$, $\lambda(e) = 2$
	\end{itemize}

	\column{0.5\textwidth}
	\begin{tikzpicture}[scale=0.85, >=Stealth,
			vertex/.style={circle,draw,fill=blue!8,thick,inner sep=1.2pt,minimum size=7mm},
			edge/.style={->,semithick,draw=black!35}]
		\node[vertex] (r) at (0,0) {$r$};
		...
		\draw[edge] (r) -- node[above,font=\scriptsize ,text=blue] {0} (a);
		\draw[edge] (r) -- node[below,font=\scriptsize ,text=blue] {0} (b);
		\draw[edge] (r) .. controls (0.6,2.0) and (4.2,2.5) .. node[above,yshift=3pt,pos=0.53,font=\scriptsize ,text=accentpurple] {2} (c);
		\draw[edge] (b) to[bend left=12] node[left,font=\scriptsize ,text=accentpurple] {2} (a);
		\draw[edge] (a) to[bend left=14] node[right,font=\scriptsize ,text=accentpurple] {3} (b);
		\draw[edge] (a) -- node[above right,font=\scriptsize ,text=accentpurple] {2} (d);
		\draw[edge] (a) -- node[above,font=\scriptsize ,text=blue] {0} (c);
		\draw[edge] (b) -- node[above,font=\scriptsize ,text=blue] {0} (d);
		\draw[edge] (b) -- node[below,font=\scriptsize ,text=blue] {0} (e);
		\draw[edge] (c) .. controls (6.3,2.2) and (6.3,-2.2) .. node[xshift=7pt,pos=0.52,font=\scriptsize ,text=accentpurple] {3} (e);
	\end{tikzpicture}
\end{columns}
...
\begin{frame}{Passo 2: Construção de $D_0$}
	\begin{columns}[c]
		...
		Para cada $v \neq r$, escolher um arco $a_v \in \delta^-(v)$ com $w_\lambda(a_v) = 0$
		...
		\begin{tikzpicture}[scale=0.85, >=Stealth,
			...
			\draw[unselected] (b) to[bend left=12] node[left,font=\scriptsize,text=black] {2} (a);
			\draw[unselected] (a) to[bend left=14] node[right,font=\scriptsize,text=black] {3} (b);
	...
	\vspace{0.3cm}
	\begin{alertblock}{$D_0$ é uma $r$-arborescência!}
		Neste exemplo, $D_0 = \{(r,a), (r,b), (a,c), (b,d), (b,e)\}$ forma uma $r$-arborescência.
	\end{alertblock}
\end{frame}

\begin{frame}{Contração de Ciclos}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Operação:}
		...
		Resolver problema no digrafo menor recursivamente.
		...
		\begin{tikzpicture}[>=Stealth, node distance=2cm,
			...
			\draw[->, very thick] (r) to[bend left=12] node[midway, above, sloped] {\scriptsize 1} (xC);
		\end{tikzpicture}
...
\begin{frame}{Passo 3: Contração de Ciclos}
\begin{columns}[c]
	\column{0.5\textwidth}
...
\begin{frame}{Reexpansão da Solução}
	\begin{columns}[c]
		\column{0.48\textwidth}
		...
		\begin{tikzpicture}[>=Stealth, node distance=2.0cm,
			...
			\draw[selected] (r) to[bend left=8] node[midway, above, sloped] {\scriptsize 5} (a);
...
\begin{frame}{Passo 4: Chamada Recursiva}
\begin{center}
	\begin{tikzpicture}[scale=1.2, >=Stealth,
			vertex/.style={circle,draw,fill=blue!8,thick,minimum size=7mm},
			super/.style={circle,draw,very thick,fill=orange!10,minimum size=1.2cm}]
