...
\begin{frame}{Passo 1: Redução de Custos}
	\begin{columns}[c]
		...
		\begin{tikzpicture}[scale=0.85, >=Stealth,
			...
			\draw[edge] (r) -- node[above,font=\scriptsize] {0} (a);
			\draw[edge] (r) -- node[below,font=\scriptsize] {0} (b);
			\draw[edge] (r) .. controls (0.6,2.0) and (4.2,2.5) .. node[yshift=3pt,pos=0.53,font=\scriptsize] {2} (c);
			\draw[edge] (b) to[bend left=12] node[left,font=\scriptsize] {2} (a);
			\draw[edge] (a) to[bend left=14] node[right,font=\scriptsize] {3} (b);
			\draw[edge] (a) -- node[right,font=\scriptsize] {2} (d);
			\draw[edge] (a) -- node[above,font=\scriptsize] {0} (c);
			\draw[edge] (b) -- node[above,font=\scriptsize] {0} (d);
			\draw[edge] (b) -- node[below,font=\scriptsize] {0} (e);
			\draw[edge] (c) .. controls (6.3,2.2) and (6.3,-2.2) .. node[xshift=7pt,pos=0.52,font=\scriptsize] {3} (e);
	...
	\begin{block}{Custos Reduzidos}
		Após subtrair $\lambda(v)$ de cada arco entrando em $v$, os arcos com custo zero formam candidatos para $D_0$
	\end{block}
\end{frame}
