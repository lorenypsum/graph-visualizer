...
\setbeamercolor{palette primary}{bg=green!40!black,fg=white}
\setbeamercolor{palette secondary}{bg=green!40!black,fg=white}
\setbeamercolor{palette tertiary}{bg=green!40!black,fg=white}
\setbeamercolor{frametitle}{bg=green!50!black,fg=white}
% Cores da caixa do título na capa
\setbeamercolor{title}{fg=white!30!white}
\setbeamercolor{titlelike}{bg=green!40!black,fg=white}

% Cores dos blocos (caixas de texto)
\definecolor{blockbg}{RGB}{46, 125, 50}      % Verde escuro para título
\definecolor{blockbody}{RGB}{232, 245, 233}  % Verde muito claro para corpo
\definecolor{alertbg}{RGB}{106, 27, 154}     % Roxo para alerta
\definecolor{alertbody}{RGB}{245, 245, 245}  % Cinza claro para corpo alerta

\setbeamercolor{block title}{bg=blockbg,fg=white}
\setbeamercolor{block body}{bg=blockbody,fg=black}
\setbeamercolor{block title alerted}{bg=alertbg,fg=white}
\setbeamercolor{block body alerted}{bg=alertbody,fg=black}
\setbeamercolor{block title example}{bg=blockbg,fg=white}
\setbeamercolor{block body example}{bg=blockbody,fg=black}

% Remover/customizar sombra dos blocos
...
\title[r-Arborescências Geradoras Mínimas]{Algoritmos para r-Arborescências Geradoras Mínimas em Digrafos:}
\subtitle{Uma Aplicação Web Interativa}
\author{Lorena Sampaio, Samira Haddad \\ Orientador: Prof. Dr. Mário Leston Rey}
\institute{Universidade Federal do ABC \\ Centro de Matemática, Computação e Cognição}
\date{\today}
...
\begin{frame}{O Problema}
	\begin{block}{Arborescência de Custo Mínimo}
		Dado um digrafo $D = (V, A)$ com pesos $w: A \to \mathbb{R}$ e raiz $r \in V$:
		\begin{itemize}
			\item Encontrar uma $r$-arborescência geradora $T$
			\item Que minimize $w(T) = \sum_{a \in A(T)} w(a)$
		\end{itemize}
	\end{block}
