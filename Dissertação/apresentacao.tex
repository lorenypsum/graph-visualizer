...
\begin{frame}{Passo 1: Redução de Custos}
	...
	\vspace{0.1cm}
	\begin{block}{}
		Arcos com custo zero são candidatos para $A_0$ em $D_0$.
	\end{block}
\end{frame}

\ifshowimplementation
\begin{frame}[fragile]{Passo 1: Implementação - Redução de Custos}
...
\begin{frame}{Passo 3: Verificação de $D_0$}
	...
	\begin{alertblock}{$D_0$ não é uma $r$-arborescência!}
		Neste exemplo, $A_0 = \{(r,a), (a,b), (c,a)\}$ não forma uma $r$-arborescência pois contém o ciclo $(a, b, c, a)$.
	\end{alertblock}
\end{frame}
...
\begin{frame}{Complexidade do Algoritmo}
	...
	\begin{itemize}
		...
		\item Cada chamada envolve operações de redução de custos, construção de $D_0$, detecção de ciclos e contração, cada uma com complexidade $O(m)$
	\end{itemize}
...
\begin{frame}{Fase I: Conceitos Fundamentais}
	\textbf{Definições:}
	\begin{block}{$r$-conjunto minimal}
	...
	\begin{block}{Sequência $w$-disjunta}
		Uma sequência $((R_i, \lambda_i))_{i \in [k)}$ é \textbf{$w$-disjunta} se:
		$$\sum_{i \in [k)} \lambda_i \,[a \in \delta^-(R_i)] \;\le\; w(a) \quad \text{para cada } a \in A(D)$$
	\end{block}
	{\small A soma dos $\lambda_i$ sobre os conjuntos que $a$ entra não excede o peso de $a$.}
\end{frame}
	...
	\begin{enumerate}
		...
		\item \textbf{Enquanto} existir fonte $R$ em $\mathcal{C}(D_0)$ com $r \notin R$:
		      \begin{itemize}
			      ...
			      \item $F := F \cup \{f\}$
			      \item $c := c - \lambda \mathbf{1}_{\delta^-(R)}$ {\small (reduzir custos)}
...
\begin{frame}{Exemplo: Digrafo Inicial}
	\begin{columns}[c]
		...
		\end{itemize}
		\column{0.52\textwidth}
		\begin{tikzpicture}[>=Stealth, node distance=1.6cm, scale=0.9, transform shape,
				v/.style={circle, draw, fill=blue!8, minimum size=6.5mm},
				arc/.style={->, thick, draw=gray!70}]
			\node[v,fill=red!20] (r) {\small $r$};
			\node[v] (a) [below right=1.3cm and 1.1cm of r] {\small $a$};
			\node[v] (b) [above right=1.3cm and 1.1cm of r] {\small $b$};
			\node[v] (c) [right=2.2cm of a] {\small $c$};
...
\begin{frame}{Fase I: Iteração 2 - Redução de Custos}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Atualização:}
		\begin{itemize}
...
\begin{frame}{Fase I: Iteração 3 - Redução de Custos}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Atualização:}
		\begin{itemize}
		...
		\vspace{0.2cm}
		$\mathcal{C}(D_0)$ com $F = \{(b,a), (a,c)\}$:

		\vspace{0.2cm}
		\begin{itemize}
			\item CFCs: $\{r\}, \{b\}, \{a\}, \{c\}$
			\item Fontes em $D_0$: $\{r\}, \{b\}$
		\end{itemize}

		\column{0.52\textwidth}
		\begin{tikzpicture}[>=Stealth, node distance=1.6cm, scale=0.85, transform shape,
				v/.style={circle, draw, fill=blue!8, minimum size=6mm},
				zero/.style={->, very thick, draw=blue!70},
				arc/.style={->, thick, draw=gray!70}]
			\node[v,fill=red!20] (r) {\small $r$};
			\node[v] (a) [below right=1.3cm and 1.0cm of r] {\small $a$};
			\node[v] (b) [above right=1.3cm and 1.0cm of r] {\small $b$};
			\node[v] (c) [right=2.0cm of a] {\small $c$};
		...
		{\small Arcos com custo zero em azul}
	\end{columns}
\end{frame}

\begin{frame}{Fase I: Iteração 4 - Redução de Custos}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Estado:} $F = \{(b,a), (a,c)\}$

		\vspace{0.2cm}
		$\mathcal{C}(D_0)$ ainda tem:
		\begin{itemize}
			\item Fonte $\{b\}$ não contém $r$
		\end{itemize}
		...
		\begin{tikzpicture}[>=Stealth, node distance=1.6cm, scale=0.9, transform shape,
			...
			\draw[arc] (r) -- node[midway, below left] {\scriptsize 1} (a);
			\draw[selected] (r) to[bend left=15] node[midway, above left] {\scriptsize 10} (b);
			\draw[zero] (b) -- node[midway, above right] {\scriptsize 0} (a);
			\draw[zero] (a) -- node[midway, below] {\scriptsize 0} (c);

			\node[rset, fit=(b)] {};
			\node[orange!70, above=0.2cm of b] {\small $R_2$};
		\end{tikzpicture}
	\end{columns}
\end{frame}
...
\begin{frame}{Fase I: Iteração 5 - Estado Final}
	\begin{columns}[c]
		...
		\vspace{0.2cm}
		$\mathcal{C}(D_0)$ com $F = \{(b,a), (a,c), (r,b)\}$:
		...
		\begin{tikzpicture}[>=Stealth, node distance=1.6cm, scale=0.85, transform shape,
			...
			\draw[arc] (r) -- node[midway, below left] {\scriptsize 1} (a);
			\draw[zero] (r) to[bend left=15] node[midway, above left] {\scriptsize 0} (b);
			\draw[zero] (b) -- node[midway, above right] {\scriptsize 0} (a);
			\draw[zero] (a) -- node[midway, below] {\scriptsize 0} (c);
		\end{tikzpicture}

		{\small Todos arcos de $F$ têm custo zero!}
	\end{columns}
	\begin{block}{Sequência devolvida}
		$\sigma = [(f_0=(b,a), R_0=\{a\}, \lambda_0=1), (f_1=(a,c), R_1=\{c\}, \lambda_1=4), (f_2=(r,b), R_2=\{b\}, \lambda_2=10)]$
	\end{block}
\end{frame}
...
\begin{frame}{Fase II: Exemplo - Extração da Arborescência}
	\begin{columns}[c]
		...
		\begin{itemize}
			\item $F = [f_0, f_1, f_2]$ $= [(b,a), (a,c), (r,b)]$
		\end{itemize}
		...
		\column{0.45\textwidth}
		\begin{tikzpicture}[>=Stealth, node distance=1.7cm, scale=0.85, transform shape,
				v/.style={circle, draw, fill=blue!8, minimum size=6.5mm},
				inU/.style={circle, draw, fill=green!20, minimum size=6.5mm},
				arc/.style={->, thick, draw=gray!70},
				covered/.style={->, thick, draw=blue!70}]
			\node[v,fill=red!20,inU] (r) {\small $r$};
			\node[v] (a) [below right=1.3cm and 1.0cm of r] {\small $a$};
			\node[v] (b) [above right=1.3cm and 1.0cm of r] {\small $b$};
			\node[v] (c) [right=2.1cm of a] {\small $c$};			\draw[arc] (r) -- node[midway, below left] {\scriptsize 2} (a);
			\draw[arc] (r) to[bend left=15] node[midway, above left] {\scriptsize 10} (b);
...
\begin{frame}{Fase II: Iteração 1}
	\begin{columns}[c]
		...
		\column{0.52\textwidth}
		\begin{tikzpicture}[>=Stealth, node distance=1.7cm, scale=0.9, transform shape,
				v/.style={circle, draw, fill=blue!8, minimum size=6.5mm},
				inU/.style={circle, draw, fill=green!20, minimum size=6.5mm},
				selected/.style={->, very thick, draw=green!70},
			...
			\node[v] (a) [below right=1.3cm and 1.1cm of r] {\small $a$};
			\node[inU] (b) [above right=1.3cm and 1.1cm of r] {\small $b$};
			\node[v] (c) [right=2.2cm of a] {\small $c$};
...
\begin{frame}{Fase II: Iteração 2}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Estado:} $U = \{r,b\}$, $J = \{(r,b)\}$

		\vspace{0.2cm}
		\textbf{Procurar em $F$:}
		\begin{itemize}
			\item $f_0=(b,a)$: $b \in U$, $a \notin U$ \checkmark
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Ação:}
		\begin{itemize}
			\item Adicionar $(b,a)$ a $J$
			\item $U := U \cup \{a\} = \{r,b,a\}$
		\end{itemize}
		...
		\begin{tikzpicture}[>=Stealth, node distance=1.7cm, scale=0.9, transform shape,
				...
				arc/.style={->, thick, draw=gray!60}]
			\node[inU,fill=red!20] (r) {\small $r$};
			\node[inU] (a) [below right=1.3cm and 1.1cm of r] {\small $a$};
			\node[inU] (b) [above right=1.3cm and 1.1cm of r] {\small $b$};
			\node[v] (c) [right=2.2cm of a] {\small $c$};
...
\begin{frame}{Fase II: Iteração 3 - Estado Final}
	\begin{columns}[c]
		\column{0.48\textwidth}
		\textbf{Estado:} $U = \{r,b,a\}$, $|J| = 2$

		\vspace{0.2cm}
		\textbf{Procurar em $F$:}
		\begin{itemize}
			\item $f_1=(a,c)$: $a \in U$, $c \notin U$ \checkmark
		\end{itemize}

		\vspace{0.2cm}
		\textbf{Ação:}
		\begin{itemize}
			\item Adicionar $(a,c)$ a $J$
			\item $U := U \cup \{c\} = V$
		...
		\begin{tikzpicture}[>=Stealth, node distance=1.7cm, scale=0.9, transform shape,
			...
			\node[inU] (a) [below right=1.3cm and 1.1cm of r] {\small $a$};
			\node[inU] (b) [above right=1.3cm and 1.1cm of r] {\small $b$};
			\node[inU] (c) [right=2.2cm of a] {\small $c$};

			\draw[->, thick, draw=gray!40] (r) -- node[midway, below left] {\scriptsize 2} (a);
			\draw[selected] (r) to[bend left=15] node[midway, above left] {\scriptsize 10} (b);
			\draw[selected] (b) -- node[midway, above right] {\scriptsize 1} (a);
	...
	\begin{exampleblock}{Resultado Final}
		$J = \{(r,b), (b,a), (a,c)\}$ com custo $w(J) = 10 + 1 + 4 = 15$ (custo mínimo!)
	\end{exampleblock}
\end{frame}
