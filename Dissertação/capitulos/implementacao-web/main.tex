...
\subsection{Visão geral das páginas}
% A  aplicação \textit{web} é organizada em páginas HTML independentes, cada uma dedicada a uma parte específica da experiência do usuário. A página \texttt{home.html} apresenta um panorama geral da ferramenta; \texttt{chuliu.html} executa passo a passo o algoritmo de Chu-Liu e Edmonds;
%  \texttt{andrasfrank\_v1.html} e \texttt{andrasfrank\_v2.html} oferecem a estrutura inicial para a futura visualização da abordagem primal–-dual em suas duas versões; e \texttt{draw\_graph.html} funciona como um editor livre para criação manual de grafos. Todas as páginas carregam dinamicamente o componente compartilhado de navegação lateral (\texttt{sidebar.html}) e importam apenas os scripts necessários ao seu funcionamento, garantindo modularidade e reduzindo sobrecarga desnecessária na interface.

% (ii) \texttt{chuliu.html} (execução passo a passo do algoritmo de Chu--Liu/Edmonds), (iii-a) \texttt{andrasfrank\_v1.html} (estrutura para futura visualização da abordagem primal--dual em sua primeira implementação), (iii-b) \texttt{andrasfrank\_v2.html} (estrutura para futura visualização da abordagem primal--dual em sua primeira implementação), (iv) \texttt{draw\_graph.html} (editor livre de grafos), além do componente compartilhado de navegação lateral \texttt{sidebar.html}. Cada página injeta o \texttt{sidebar} dinamicamente e carrega scripts específicos.
