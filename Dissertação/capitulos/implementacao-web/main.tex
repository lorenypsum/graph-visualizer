...
Nesta seção apresentaremos a aplicação web que desenvolvemos para materializar, de forma interativa e didática, os algoritmos discutidos nos capítulos anteriores. Objetiva-se descrever a finalidade da ferramenta, os princípios de design operacionalizados na interface e os principais recursos que sustentam a exploração pedagógica de arborescências dirigidas. Apresentamos aqui uma visão de alto nível da aplicação antes de detalhar a sua arquitetura, páginas e fluxo de interação.

% A respeito do desenvolvimento, ele se deu pela adoção de tecnologias \textit{web} modernas, combinando HTML5 e Tailwind CSS para composição responsiva, PyScript para execução de código Python diretamente no navegador e bibliotecas científicas amplamente utilizadas em grafos e visualização. NetworkX foi empregado para a manipulação de grafos dirigidos e Matplotlib para gerar snapshots estáticos dos estados intermediários dos algoritmos. 
...
A aplicação organiza-se em módulos funcionais desenhados para atender aos objetivos didáticos do projeto. A navegação é dividida em três eixos principais:

\begin{itemize}
    \item \textbf{Visualização Algorítmica:} O núcleo da ferramenta consiste em três páginas dedicadas à execução passo a passo dos métodos estudados. Uma página detalha o algoritmo de Chu-Liu/Edmonds, enquanto outras duas exploram implementações distintas do algoritmo de Frank. Em todas elas, o sistema exibe as iterações de forma sequencial, apresentando as alterações visuais no grafo e o resultado final da $r$-arborescência.

    \item \textbf{Modelagem Livre:} Uma interface de edição (\textit{sandbox}) permite que o usuário desenhe grafos arbitrariamente. Este recurso possibilita a exploração de diferentes topologias e estruturas, oferecendo liberdade para testar hipóteses fora dos exemplos pré-definidos.

    \item \textbf{Disseminação Científica:} Uma seção informativa dedicada à divulgação da dissertação e do projeto. O objetivo é contextualizar a ferramenta, explicando as motivações de seu desenvolvimento e servindo como ponto de difusão do conhecimento teórico sobre o funcionamento dos algoritmos implementados.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Camada de Apresentação:} Responsável pela interface com o usuário, desenvolvida em HTML5 e estilizada com o \textit{framework} utilitário Tailwind CSS. Esta camada gerencia a entrada de dados e a exibição dos resultados, mantendo a responsividade em diferentes dispositivos.
...
[Image of client side web application architecture diagram]
