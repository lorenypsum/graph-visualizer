...
A ferramenta está disponível publicamente para acesso via navegador (ver Anexo~\ref{anx:aplicacao}).
...
Na barra lateral, dispomos do menu que o usuário pode utilizar para navegar entre as páginas da aplicação, mantendo consistência cognitiva. Tal menu é apresentado em todas as páginas da aplicação. Uma visualização detalhada da barra lateral é apresentado na figura a seguir.
\begin{figure}[H]\centering
	\includegraphics[width=0.2\textwidth]{../assets/menuhtml.png}
	\caption{Visão detalhada da barra lateral esquerda de navegação.}
	\label{fig:menu_html_screenshot}
\end{figure}

Nas páginas dos algoritmos, o conteúdo interativo central combina o editor de grafos com a visualização da arborescência final e um log textual que detalha cada ação. 
Os controles de execução, dispoem da ordem correta de manipulação das ferramenta e são padronizado em todas as páginas de algoritmos, tornando a navegação mais intuitiva.
A imagem \ref{fig:controles_html_screenshot} dá um \textit{zoom} na seção de controles dessas páginas.
\begin{figure}[H]\centering
	\includegraphics[width=0.95\textwidth]{../assets/controleshtml.png}
	\caption{\textit{Zoom} da seção de controles padronizados para execução dos algoritmos.}
	\label{fig:controles_html_screenshot}
\end{figure}
Já o log textual, apresentado abaixo, detalha cada passo do algoritmo, explicando as operações realizadas e os estados intermediários do grafo de forma textual. Isso reforça a compreensão dos conceitos teóricos por trás das ações visuais.
\begin{figure}[H]\centering
	\includegraphics[width=0.95\textwidth]{../assets/loghtml.png}
	\caption{\textit{Zoom} da seção log textual}
	\label{fig:log_html_screenshot}
\end{figure}
À direita, o painel de passos guia o usuário através da execução do algoritmo, reforçando o encadeamento lógico das operações. Na imagem a seguir, detalhamos essa interface com um \textit{zoom}, evidenciando tanto a navegação entre os passos quanto a visualização minuciosa de cada etapa.
\begin{figure}[H]\centering
	\includegraphics[width=0.3\textwidth]{../assets/passoshtml.png}
	\caption{Passos da página de algoritmos, permitindo navegação sequencial e detalhamento.}
	\label{fig:passo_a_passo_html_screenshot}
\end{figure}

Dessa forma, arquitetura modular e reutilizável das páginas \textit{web} facilita manutenção, extensão e consistência. Além disso, a sequência lógica de páginas, guia o usuário durante a navegação, alinhando-se a princípios pedagógicos. Essa estrutura coesa apoia o aprendizado eficaz dos conceitos de arborescências dirigidas e algoritmos associados. 
