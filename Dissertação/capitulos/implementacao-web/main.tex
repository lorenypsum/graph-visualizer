...
Nos momentos de processamento de informação também atualizamos a interface para indicar progresso, evitando que o usuário fique incerto sobre o estado atual.

\subsection{Engajamento ativo}

Engajamento ativo refere-se à participação dos usuários no processo de aprendizagem, incentivando-os a explorar, experimentar e interagir com o sistema. Nossa aplicação promove o engajamento ativo ao permitir que os usuários manipulem o grafo, testem diferentes abordagens e visualizem os resultados de suas ações em tempo real, além de acessar descrições passo a passo da execução dos algoritmos.
...
A aplicação \textit{web} é composta por páginas HTML autônomas carregadas no navegador, cada uma focada em um aspecto: (i) \texttt{home.html} (apresentação / resumo), (ii) \texttt{chuliu.html} (execução passo a passo do algoritmo de Chu--Liu/Edmonds), (iii-a) \texttt{andrasfrank_v1.html} (estrutura para futura visualização da abordagem primal--dual em sua primeira implementação), (iii-b) \texttt{andrasfrank_v2.html} (estrutura para futura visualização da abordagem primal--dual em sua primeira implementação), (iv) \texttt{draw\_graph.html} (editor livre de grafos), além do componente compartilhado de navegação lateral \texttt{sidebar.html}. Cada página injeta o \texttt{sidebar} dinamicamente e carrega scripts específicos.

% TODO - Não sei se essa seção precisa da descrição do nome dos arquivos
...
\begin{itemize}\setlength{\itemsep}{2pt}
	...
	\item \textbf{Bibliotecas centrais:} NetworkX para manipulação de grafos dirigidos; \texttt{matplotlib} para renderização de snapshots; (em páginas avançadas) Cytoscape.js previsto para interação rica (estrutura já importada em \texttt{chuliu.html}, \texttt{andrasfrank_v1.html} e \texttt{andrasfrank_v2.html}).
...
A aplicação centraliza-se nas páginas \texttt{chuliu.html}, \texttt{andrasfrank_v1.html} e {andrasfrank_v2.html}, que permitem executar os algoritmos de Chu--Liu/Edmonds e Frank (1° e 2° implementação), respectivamente. A seguir, detalhamos os componentes funcionais principais da interface:
...
\begin{enumerate}\setlength{\itemsep}{2pt}
	...
	\item \textbf{Incorporação de PDF:} \texttt{tese.html} exibe nossa tese lado a lado à aplicação, incentivando leitura entrelaçada teoria-execução. 
\end{enumerate}
...
\begin{enumerate}\setlength{\itemsep}{2pt}
	...
	\item Executa o algoritmo: são aplicadas normalizações e seleção de arestas (implementação apresentada em seções anteriores / listagens).
...
\begin{itemize}\setlength{\itemsep}{2pt}
	...
	\item \texttt{andrasfrank_v1.html}: página dedicada à implementação do algoritmo de Frank (primeira implementação).
	\item \texttt{andrasfrank_v2.html}: página dedicada à implementação do algoritmo de Frank (segunda implementação).
