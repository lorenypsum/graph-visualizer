...
% A escolha da melhor aresta de entrada para cada vértice parece, à primeira vista, uma estratégia natural. No entanto, decisões localmente ótimas podem produzir ciclos, comprometendo a coerência global da solução. Uma dificuldade recorrente consiste na tendência de estudantes projetarem sua intuição local para o comportamento global do algoritmo, o que gera surpresa diante de ciclos “inesperados” e aumenta a carga cognitiva ao colocar em conflito heurísticas locais e restrições estruturais do problema, em exercícios práticos, por exemplo, é comum que estudantes apliquem a regra “escolha a aresta de entrada mais barata de cada vértice” de maneira direta, assumindo que escolhas localmente econômicas sempre conduziriam a uma solução global coerente. No entanto, ao final da seleção, descobrem que as escolhas formaram um ciclo — justamente a situação que o algoritmo precisa detectar e tratar.

% VERSÃO 2
% A articulação entre decisões locais e coerência global podem ocorrer em diversas etapas da resolução de ambos os métodos, por exemplo durante o processo de seleção da menor aresta de entrada para cada vértice costuma parecer, de imediato, uma estratégia intuitiva e suficiente. Entretanto, escolhas que são localmente ótimas podem gerar ciclos e, portanto, inviabilizar a coerência global exigida por uma arborescência. Uma dificuldade recorrente está justamente nessa extrapolação, durante o processo de aprendizagem podemos aplicar regras simples de decisão local, esperam que o comportamento global do algoritmo “acompanhe” essa intuição.

% Em atividades práticas, por exemplo, é frequente que adotem a regra de escolher a aresta de entrada de menor custo de forma mecânica, acreditando que tal procedimento conduzirá automaticamente a uma solução válida. A surpresa surge quando, ao final dessas seleções, percebem que as escolhas formaram um ciclo — exatamente o tipo de estrutura que o algoritmo precisa identificar e contrair. Esse descompasso entre expectativas locais e consequências globais não apenas gera estranhamento, mas também aumenta a carga cognitiva ao confrontar heurísticas intuitivas com restrições estruturais fundamentais do problema.
