...
Ao final, \emph{expandimos} as contrações removendo exatamente um arco interno de cada ciclo contraído. Essa remoção é necessária para preservar duas propriedades fundamentais da arborescência: primeiro, que cada vértice (exceto a raiz) possua grau de entrada exatamente igual a 1; segundo, que o dígrafo resultante seja \textbf{acíclico}, ou seja, não contenha nenhum ciclo dirigido. Chamamos essa segunda propriedade de \textbf{aciclicidade global}, pois deve valer para todo o dígrafo, e não apenas localmente para subconjuntos de vértices.
...
Observamos que a contração pode gerar arcos paralelos entre um mesmo par de vértices, mas descartamos laços em \(x_C\). Os custos dos arcos que entram no supervértice \(x_C\) devem ser ajustados de modo a preservar a otimalidade, conforme estabelecido na proposição a seguir.
