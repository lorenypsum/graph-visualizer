...
Para preservar a otimalidade na correspondência entre arborescências do grafo contraído e do grafo original, ajustamos os custos dos arcos que entram no supervértice \(x_C\) da seguinte forma: para cada arco \((u, w)\) com \(u \notin C\) e \(w \in C\), o custo do arco contraído \((u, x_C)\) é definido como \(c_\lambda(u,w)\), onde \(\lambda(w) = \min\{c(a) : a \in \delta^-(w)\}\) é o custo mínimo de entrada em \(w\).

Agora vamos ilustrar como essa contração é feita e como os custos são ajustados.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=2.0cm]
		...
		% ciclo de escolhas mais baratas (custo 1)
		\draw[cheap] (v1) to[bend left=12] node[midway, above, sloped] {\scriptsize 3} (v2);
		\draw[cheap] (v2) to[bend left=12] node[midway, above, sloped] {\scriptsize 2} (v3);
		\draw[cheap] (v3) to[bend left=14] node[midway, below, sloped] {\scriptsize 3} (v1);
		% arcos da raiz mais caros (não escolhidos pelo critério local)
		\draw[expensive] (r) to[bend left=8] node[midway, above, sloped] {\scriptsize 3} (v1);
		\draw[expensive] (r) to[bend left=18] node[midway, above, sloped] {\scriptsize 4} (v2);
		\draw[expensive] (r) to[bend right=10] node[midway, below, sloped] {\scriptsize 5} (v3);
		% arcos saindo do ciclo
		\draw[expensive] (v2) -- node[midway, above] {\scriptsize 1} (u);
		\draw[expensive] (v3) -- node[midway, below] {\scriptsize 2} (w);
		\draw[expensive] (v3) -- node[midway, below] {\scriptsize 4} (u);
	\end{tikzpicture}
	\caption{O dígrafo \(D\) com o ciclo \(C = (v_1, v_2, v_3, v_1)\). Os arcos azuis representam os arcos de custo mínimo; os tracejados representam arcos mais caros da raiz.}
\end{figure}

Após a normalização dos custos, os arcos internos do ciclo passam a ter custo reduzido zero:
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=2.0cm]
		...
		\tikzset{
			...
			cheap/.style={->, very thick, draw=blue!70},
			expensive/.style={->, dashed, draw=gray!60},
			redexpensive/.style={->, dashed, draw=red!60}
		}
		% nós
		\node[v] (r) {\small $r$};
		\node[v] (v1) [right=2.2cm of r] {\small $v_1$};
		...
		\draw[expensive] (r) to[bend left=8] node[midway, above, sloped] {\scriptsize 3} (v1);
		\draw[expensive] (r) to[bend left=18] node[midway, above, sloped] {\scriptsize 4} (v2);
		\draw[expensive] (r) to[bend right=10] node[midway, below, sloped] {\scriptsize 3} (v3);
		% arcos saindo do ciclo com custo reduzido zero
		\draw[redexpensive] (v2) -- node[midway, above] {\scriptsize 1} (u);
		\draw[expensive] (v3) -- node[midway, below] {\scriptsize 2} (w);
		\draw[redexpensive] (v3) -- node[midway, below] {\scriptsize 4} (u);
	\end{tikzpicture}
	\caption{Dígrafo com custos \(\lambda\)-reduzidos. Os arcos internos do ciclo \(C\) têm custo zero (em azul). Os arcos da raiz para o ciclo têm custos 4, 3 e 3 (tracejados).}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=2.0cm]
		...
		\draw[cheap] (r) to[bend left=12] node[midway, above, sloped] {\scriptsize 3} (xC);
		% arcos que saem de xC
		\draw[cheap] (xC) to[bend left=8] node[midway, above] {\scriptsize 1} (u);
		\draw[cheap] (xC) to[bend right=8] node[midway, below] {\scriptsize 2} (w);
	\end{tikzpicture}
	\caption{Dígrafo \(D'\) após a contração do ciclo \(C\). O supervértice \(x_C\) substitui todos os vértices do ciclo. Originalmente, havia três arcos paralelos de \(r\) para o ciclo: \((r, v_1)\), \((r, v_2)\) e \((r, v_3)\) com custos reduzidos 4, 3 e 3; mantemos apenas o de menor custo 3. Os arcos que saíam do ciclo agora saem de \(x_C\): \((x_C, u)\) com custo 1 e \((x_C, w)\) com custo 2. Note que havia dois arcos de vértices do ciclo para \(u\); mantemos apenas o de menor custo.}
\end{figure}

Observamos que caso a contração gere arcos paralelos entre um mesmo par de vértices, mantemos apenas o arco de menor custo, conforme ilustrado do vértice \(r\) para o supervértice \(x_C\) e de \(x_C\) para \(u\) e isso não afeta a otimalidade, já que na reexpansão qualquer escolha entre arcos paralelos conduz à mesma solução ótima.
