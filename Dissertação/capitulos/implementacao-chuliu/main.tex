...
def reduce_weights(D: nx.DiGraph, node: str):    
    ...
    yv = min((data.get("w", 0) for _, _, data in 
	D.in_edges(node, data=True)))
...
Por fim, a função devolve o grafo \texttt{D} atualizado no próprio objeto com todos os arcos de entrada em \texttt{r0} removidos (linha 5). A complexidade total da função é \(O(\deg^-(r_0))\), pois a operação coleta e remove cada arco de entrada uma única vez.
