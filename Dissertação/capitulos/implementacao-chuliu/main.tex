...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, node distance=2.0cm]
		...
		\node[v] (w) [right=2.0cm of v3] {\small $w$};
		% ciclo de escolhas mais baratas (custo 1)
		\draw[arcs] (v1) to[bend left=12] node[midway, above, sloped] {\scriptsize 3} (v2);
		\draw[arcs] (v2) to[bend left=12] node[midway, above, sloped] {\scriptsize 2} (v3);
		\draw[arcs] (v3) to[bend left=14] node[midway, below, sloped] {\scriptsize 3} (v1);
		% arcos da raiz mais caros (não escolhidos pelo critério local)
		\draw[arcs] (r) to[bend left=8] node[midway, above, sloped] {\scriptsize 3} (v1);
		...
		% arcos saindo do ciclo
		\draw[arcs] (v2) -- node[midway, above] {\scriptsize 0} (u);
		\draw[arcs] (v3) -- node[midway, below] {\scriptsize 0} (w);
...
É importante observar que, embora diferentes arborescências ótimas em \(D\) possam ser mapeadas para arborescências ótimas em \(D'\), o valor do custo ótimo é preservado no processo de contração. Além disso, ao reexpandir uma arborescência ótima de \(D'\), podemos obter múltiplas arborescências ótimas em \(D\), evidenciando que a correspondência entre soluções ótimas pode não ser bijetiva. O exemplo a seguir ilustra essa situação:
