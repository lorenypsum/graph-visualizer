...
E em relação aos custos dos arcos que entram no supervértice \(x_C\) em \(D'\) ajustamos-os da seguinte forma: para cada arco \((u, w)\) com \(u \notin C\) e \(w \in C\), o custo do arco contraído \((u, x_C)\) é definido como \(c_\lambda(u,w)\), onde \(\lambda(w) = \min\{c(a) : a \in \delta^-(w)\}\) é o custo mínimo de entrada em \(w\).

Para os custos em \(D'\), definimos o custo reduzido de cada arco conforme a seguinte regra: para cada arco \((u, x_C)\) com \(u \notin C\), correspondente a um arco \((u, w)\) com \(w \in C\) no grafo original, atribuímos \(c'((u, x_C)) := c_\lambda(u, w)\), onde \(c_\lambda(u, w) = c(u, w) - \lambda(w)\) e \(\lambda(w) = \min\{c(a) : a \in \delta^-(w)\}\) é o custo mínimo de entrada em \(w\). Os demais custos seguem a mesma lógica de redução, preservando a consistência com a definição anterior de custos reduzidos.
