...
Ao obtermos a \(r\)-arborescência \(T\) em \(D\) a partir de \(T'\) em \(D'\), garantimos que \(T\) é uma \(r\)-arborescência de custo mínimo em \(D\). Pois, os custos foram ajustados de forma a preservar a otimalidade durante a contração e reexpansão do ciclo \(C\). E assim, os custos finais nesse exemplo ficam da seguinte forma:
