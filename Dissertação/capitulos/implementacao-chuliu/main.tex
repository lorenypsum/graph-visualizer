...
\paragraph*{Verificação de aciclicidade — caso base (linhas 7--10):} Verifica se \(A_0\) é uma arborescência válida usando \texttt{nx.is\_arborescence(A\_zero)} (linha 7). Caso sim, restaura os pesos originais de \texttt{D} para cada arco de \(A_0\) (linhas 8--9) e devolve \texttt{A\_zero} como solução (linha 10). A função \texttt{nx.is\_arborescence} testa conectividade, aciclicidade e o grau de entrada correto simultaneamente. \textcolor{red}{Precisamos verificar se \(A_0\) é uma arborescência porque, embora cada vértice não-raiz tenha exatamente um arco de entrada, \(A_0\) pode conter ciclos que violam a estrutura de árvore. ??? PORQUE PRECISAMOS VERIFICAR SE A\_ZERO é uma arborescência? Isso não é contraditório com o fato de cada vértice não-raiz ter exatamente um arco de entrada?}
...
\begin{tcolorbox}[
	...
	\begin{itemize}
		\item Para resolver o digrafo contraído $D'$ aplica-se uma chamada recursiva:
		      \texttt{F\_prime = chuliu(D\_copy, r0, level+1)} (linha 16 na implementação).
	...
	\begin{itemize}
		...
		\item Remove-se o arco interno que entrava em \texttt{v} (quebrando o ciclo) — função \texttt{remove\_internal\_edge\_to\_cycle\_entry(C, v)} —, adiciona-se o arco externo \texttt{(u,v)} e reintegram-se os demais arcos de $C$; saídas do ciclo são tratadas via \texttt{out\_from\_cycle} (implementado nas linhas 17--30).
