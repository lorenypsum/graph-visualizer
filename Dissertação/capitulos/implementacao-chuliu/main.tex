...
Após isso reinserimos os pesos originais do arcos. A figura a seguir ilustra esse processo:
...
\textcolor{red}{\textbf{TODO: TEMOS QUE OLHAR ISSO AQUI DEPOIS}}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Dígrafo \(D\) com custos originais. O ciclo \(C = (v_1, v_2, v_3, v_1)\) tem arcos com custos 5, 5 e 2. Existem dois arcos da raiz para o ciclo, ambos com custo 5: \((r, v_1)\) e \((r, v_3)\).}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Arborescência \(T'\) no dígrafo contraído \(D'\). O arco \((r, x_C)\) pode corresponder ou ao arco \((r, v_1)\) ou ao \((r, v_3)\) em \(D\) e o arco \((x_C, u)\) corresponde ao arco normalizado \((v_2, u)\) em \(D\).}
\end{figure}

TODO -> REVISAR ESSA LEGENDA
...
A estrutura do código é modular: funções auxiliares tratam cada etapa do algoritmo — normalização dos custos, detecção e contração de ciclos, construção do subgrafo gerador e reexpansão da solução. Todas operam sobre objetos \texttt{nx.DiGraph} e são coordenadas por uma função principal que gerencia o fluxo recursivo. As subseções seguintes detalham cada função auxiliar, abordando lógica, parâmetros, saídas e complexidade.
...
\begin{itemize}\setlength{\itemsep}{2pt}
	...
	\item \texttt{D.out\_edges(u, data="w")}: devolve arcos saintes de \(u\) com pesos, análogo a \texttt{in\_edges}. 
	\item \texttt{D[u][v]["w"]}: acessa diretamente o peso da arco \((u,v)\) para leitura ou modificação. 
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{2pt}
	...
	\item \texttt{D.remove\_edges\_from(edges)}: remove múltiplas arcos em lote. 
	\item \texttt{D.remove\_nodes\_from(nodes)}: remove vértices e todas as suas arcos incidentes. 
\end{itemize}

% \textcolor{red}{\textbf{TODO: REVISAR ISSO AQUI TAMBÉM DEPOIS}}
...
Esta função implementa a normalização de custos reduzidos: calcula \(y(v)=\min\{w(u,v)\}\) e substitui cada peso \(w(u,v)\) por \(w(u,v)-y(v)\), garantindo que ao menos uma arco de entrada tenha custo zero. Como cada r-arborescência possui exatamente uma arco entrando em cada vértice não-raiz, a soma total dos valores \(y(v)\) subtraídos é constante para qualquer solução, preservando assim a ordem de otimalidade entre diferentes arborescências.

Recebe como entrada um digrafo \texttt{D} (objeto \texttt{nx.DiGraph}) e o rótulo \texttt{node} do vértice a ser normalizado. A implementação coleta todos os arcos de entrada de \texttt{node} com seus pesos usando o método \texttt{D.in\_edges(node, data="w")}, que devolve uma lista de tuplas \((u, node, w)\) (linha 2). Em seguida, verifica se a lista está vazia e se estiver devolve imediatamente sem fazer alterações (linhas 3--4). Caso contrário, calcula o peso mínimo \texttt{yv} através de uma compreensão de gerador que extrai o terceiro elemento de cada tupla (linha 5) e, para cada predecessor \texttt{u}, subtrai \texttt{yv} do peso armazenado em \texttt{D[u][node]["w"]} (linha 6).

Não devolve nenhum valor, pois a operação é realizada modificando diretamente a estrutura: o grafo \texttt{D} passado como parâmetro é modificado diretamente, e ao menos uma arco de entrada de \texttt{node} terá custo reduzido zero após a execução. A complexidade é \(O(\deg^-(v))\), pois cada operação percorre as arcos de entrada uma única vez.
...
\begin{tcolorbox}[
	...
	\emph{Normaliza os pesos das arcos que entram em \texttt{node}, subtraindo de cada uma o menor peso de entrada. Modifica o grafo D no próprio objeto.}
	\tcblower
...
\begin{figure}[H]
	...
	\caption{Exemplo de normalização de custos reduzidos. À esquerda, vértice \(v\) com três arcos de entrada (pesos 5, 3 e 7). À direita, após aplicar \texttt{reduce\_weights(D, v)}: o menor peso \(y(v)=3\) é subtraído de todas as entradas, resultando em custos reduzidos 2, 0 e 4. A arco \((u_2,v)\) (em vermelho) tem custo zero e será selecionada para \(A_0\).}
	\label{fig:normalize-example}
\end{figure}

Observe que as diferenças relativas são preservadas: a arco mais cara permanece 4 unidades acima da mais barata, e a intermediária mantém sua posição relativa. Como cada r-arborescência contém exatamente uma arco entrando em cada vértice não-raiz, a soma \(\sum_{w\neq r} y(w)\) é constante para qualquer solução, garantindo que a ordem de otimalidade seja preservada.

\subsection{Construção de \texorpdfstring{\(A_0\)}{A_0}:}
Esta função constrói o subdigrafo \(A_0\) selecionando, para cada vértice \(v\neq r_0\), uma única arco de custo reduzido zero que entra em \(v\).

Recebe como entrada um digrafo \texttt{D} (objeto \texttt{nx.DiGraph}) e o rótulo \texttt{r0} da raiz. A implementação cria um novo digrafo vazio \texttt{A\_zero} (linha 2) em vez de modificar \texttt{D} diretamente; essa escolha de criar uma estrutura separada é fundamental porque \(A_0\) é um subgrafo conceitual usado para detecção de ciclos, e preservar \texttt{D} inalterado permite que as operações subsequentes (como contração) trabalhem com o grafo original completo, evitando perda de informação sobre arcos não selecionadas que podem ser necessárias após reexpansões.

Em seguida, para cada vértice \texttt{v} diferente de \texttt{r0} (linhas 3--4), utilizando o método \texttt{D.nodes()}, coleta todos os arcos de entrada de \texttt{v} com seus pesos em uma lista e armazena na variável \texttt{in\_edges} (linha 5); a materialização em lista é necessária porque a subsequente iteração sobre as arcos para encontrar aquela de peso zero poderia causar problemas se trabalhássemos diretamente com a visão devolvida por \texttt{in\_edges}, especialmente em cenários de modificação concorrente. Se não houver arcos de entrada, prossegue para o próximo vértice (linhas 6--7) usando \texttt{continue}, pois um vértice isolado ou inacessível não contribui para \(A_0\) e sua ausência será detectada posteriormente como violação das hipóteses de conectividade.

Caso contrário, utiliza uma compreensão de gerador combinada com \texttt{next} para encontrar o primeiro predecessor \texttt{u} cuja arco \texttt{(u, v)} tem peso zero (linha 8); a escolha de \texttt{next} com gerador em vez de uma busca exaustiva é eficiente porque interrompe a iteração assim que encontra a primeira arco de custo zero, evitando processamento desnecessário das arcos restantes (embora teoricamente todos os arcos de custo zero sejam equivalentes, na prática apenas uma é necessária para \(A_0\)). A função \texttt{next} devolve \texttt{None} se nenhuma arco de peso zero existir, o que teoricamente não deveria ocorrer após a normalização correta (que garante ao menos uma arco de custo zero por vértice), mas o tratamento defensivo evita erros em casos degenerados. Se tal arco existir, adiciona-a a \texttt{A\_zero} com peso zero usando o método \texttt{add\_edge} (linhas 9--10); a especificação explícita de \texttt{w=0} garante que \(A_0\) contenha apenas arcos de custo reduzido zero, propriedade fundamental para a corretude do algoritmo.

Devolve o digrafo \texttt{A\_zero} contendo exatamente uma arco entrando em cada \(v\neq r_0\), todas com custo reduzido zero. O grafo original \texttt{D} não é modificado, preservando o estado para operações futuras. A complexidade é \(O(m)\), onde \(m\) é o número de arcos, pois cada arco é considerada no máximo uma vez durante a iteração sobre todos os vértices: para cada um dos \(n-1\) vértices não-raiz, examina-se suas arcos de entrada (totalizando no máximo \(m\) arcos ao longo de todas as iterações), e para cada vértice a busca por arco de peso zero é interrompida no primeiro match, resultando em tempo linear no tamanho do grafo.
...
\begin{tcolorbox}[
	...
	\emph{Constrói o subdigrafo $A_0$ a partir do digrafo D, selecionando para cada vértice (exceto a raiz r0) uma arco de custo reduzido zero que entra nele.}
	\tcblower
...
\begin{figure}[H]
	...
	\caption{Exemplo de construção de \(A_0\) a partir de um digrafo normalizado. À esquerda, o digrafo \(D\) após normalização, onde cada vértice não-raiz possui ao menos uma arco de entrada com custo zero (em vermelho). À direita, o subgrafo \(A_0\) resultante contém apenas as arcos de custo zero selecionadas, uma por vértice. Note que \(A_0\) pode conter ciclos (como \(\{v_1, v_2\}\)) que serão tratados nas etapas subsequentes.}
...
Após coletar todos os vértices do ciclo, constrói e devolve uma cópia do subgrafo induzido por eles (linha 7); a cópia é necessária porque o método \texttt{subgraph} devolve apenas uma visão dinâmica sobre o grafo original.
...
No final, um subgrafo contendo os vértices e arcos do ciclo detectado é devolvido, ou \texttt{None} se não houver ciclo. O grafo original \texttt{A\_zero} não é modificado. A complexidade é \(O(m)\), onde \(m\) é o número de arcos, pois a DFS visita cada arco no máximo uma vez.
...
\begin{tcolorbox}[
	...
	\emph{Detecta um ciclo dirigido em $A_0$ e devolve um subgrafo contendo seus vértices e arcos, ou \texttt{None} se for acíclico.}
	\tcblower
	\begin{lstlisting}[mathescape=true, language=Python]
...
\begin{figure}[H]
	...
	\caption{Exemplo de detecção de ciclo em \(A_0\). À esquerda, o subgrafo \(A_0\) contém um ciclo formado pelos vértices \(\{v_2, v_3, v_4\}\) (destacados em amarelo). A DFS percorre o grafo e detecta o ciclo ao encontrar a arco \((v_4, v_2)\), onde \(v_2\) já está na pilha de recursão. À direita, a função devolve uma cópia do subgrafo induzido pelos vértices do ciclo, contendo apenas os três vértices e os três arcos que formam o ciclo.}
...
\begin{figure}[H]
	...
	\caption{Exemplo de contração de ciclo. À esquerda, grafo original \(D\) com ciclo \(C=\{v_2, v_3, v_4\}\) (em amarelo). Vértices externos \(r\), \(v_1\) e \(v_5\) têm arcos conectando ao ciclo: \(r\) envia arco para \(v_2\) (peso 2) e \(v_4\) (peso 5); \(v_4\) envia arco para \(v_5\) (peso 1). À direita, após a contração: o ciclo é substituído pelo supervértice \(x_C\) (vermelho). Os arcos de entrada são redirecionadas: \((r, x_C)\) recebe peso 2 (menor entre 2 e 5). A arco de saída \((x_C, v_5)\) mantém peso 1. Os dicionários \texttt{in\_to\_cycle} e \texttt{out\_from\_cycle} armazenam os mapeamentos originais para posterior reexpansão.}
...
Em detalhes, ela recebe como entrada um digrafo \texttt{D} (objeto \texttt{nx.DiGraph}) e o rótulo \texttt{r0} da raiz. A implementação armazena em uma lista todos os arcos que entram em \texttt{r0} usando o método \texttt{in\_edges} (linha 2). Se a lista não estiver vazia (linha 3), remove todos esses arcos usando o método \texttt{remove\_edges\_from} (linha 4). Este método da biblioteca NetworkX recebe como parâmetro uma lista de tuplas representando arcos na forma \texttt{(u, v)} e remove cada uma delas do grafo. A operação é realizada em lote: NetworkX itera sobre a lista fornecida e, para cada tupla \texttt{(u, v)}, remove a arco correspondente da estrutura interna de adjacência. Se alguma arco especificada não existir no grafo, ela é silenciosamente ignorada sem gerar erro. A complexidade de \texttt{remove\_edges\_from} é \(O(k)\), onde \(k\) é o número de arcos na lista de entrada, pois cada remoção individual tem custo \(O(1)\) em média devido ao uso de dicionários aninhados para armazenar arcos.

Por fim, a função devolve o grafo \texttt{D} atualizado no próprio objeto com todos os arcos de entrada em \texttt{r0} são removidas (linha 5). A complexidade total da função é \(O(\deg^-(r_0))\), pois a operação coleta e remove cada arco de entrada uma única vez.
...
\begin{tcolorbox}[
	...
	\emph{Remove todos os arcos que entram na raiz \texttt{r0}, modificando \texttt{D} ao invés de criar uma cópia e devolve o grafo atualizado.}
	\tcblower
	\begin{lstlisting}[mathescape=true, language=Python]
...
Esta função é invocada durante a fase de reexpansão do ciclo contraído, após a chamada recursiva devolver com a arborescência ótima \(T'\) do grafo contraído. Quando o supervértice \(x_C\) é expandido de volta para o ciclo original \(C\), uma arco externa \((u,v)\) é adicionada conectando um vértice externo \(u\) a um vértice \(v\) dentro do ciclo. Como o ciclo \(C\) originalmente continha exatamente uma arco entrando em cada um de seus vértices (formando um ciclo fechado), e agora \(v\) recebe uma arco adicional vinda do exterior, esse vértice teria grau de entrada 2, violando a propriedade fundamental de arborescência (cada vértice não-raiz deve ter exatamente uma entrada). Para restaurar essa propriedade, a função remove a arco interna que anteriormente entrava em \(v\), mantendo apenas a nova arco externa. Essa remoção "quebra" o ciclo no ponto de entrada, transformando-o em um caminho que se integra corretamente à estrutura de árvore.

A função recebe como entrada o subgrafo do ciclo \texttt{C} (objeto \texttt{nx.DiGraph}) e o vértice de entrada \texttt{v}. A implementação utiliza uma compreensão de gerador combinada com \texttt{next} para encontrar o predecessor de \texttt{v} dentro do ciclo (linha 2): a expressão \texttt{(u for u, \_ in C.in\_edges(v))} itera sobre os arcos de entrada de \texttt{v}, extraindo apenas o vértice origem \texttt{u} (ignorando metadados com \texttt{\_}), e \texttt{next} devolve o primeiro (e teoricamente único) predecessor. Em seguida, remove a arco \texttt{(predecessor, v)} do ciclo usando o método \texttt{remove\_edge} (linha 3).

A função modifica diretamente o subgrafo \texttt{C} e não devolve valor. A complexidade é \(O(\deg^-(v))\), dominada pela operação de busca dos arcos de entrada, embora em ciclos simples isso seja tipicamente \(O(1)\) pois cada vértice tem exatamente um predecessor.
...
\begin{tcolorbox}[
	...
	\emph{Remove a arco interna que entra no vértice de entrada \texttt{v} do ciclo \texttt{C} durante a reexpansão, pois \texttt{v} passa a receber uma arco externa, e manter ambas violaria a propriedade de arborescência.}
	\tcblower
	\begin{lstlisting}[mathescape=true, language=Python]
...
\begin{figure}[H]
	...
	\caption{Remoção de arco interna durante reexpansão. À esquerda, ciclo $C=\{v_2, v_3, v_4\}$ após adicionar arco externa $(u, v_2)$ vindoura da arborescência $T'$: o vértice $v_2$ tem grau de entrada 2 (arco externa vermelha de $u$ e arco interna do ciclo vinda de $v_4$), violando a propriedade de arborescência. À direita, após remover a arco interna $(v_4, v_2)$: o vértice $v_2$ passa a ter grau de entrada 1, o ciclo é "quebrado" no ponto de entrada, transformando-se em um caminho que se integra corretamente à estrutura de árvore. A arco removida é mostrada tracejada em cinza.}
...
\paragraph*{Reexpansão do ciclo contraído (linhas 17--30):} Identifica a arco externa que entra no supervértice em \(F'\) (linha 17) e extrai o vértice externo \texttt{u} (linha 18). Consulta \texttt{v = in\_to\_cycle[u]} para determinar o vértice do ciclo que recebe a conexão (linha 19). Remove a arco interna que entrava em \(v\) chamando \texttt{remove\_internal\_edge\_to\_cycle\_entry(C, v)} (linha 20), quebrando o ciclo no ponto de entrada. Adiciona a arco externa \texttt{(u, v)} a \(F'\) (linha 21) e reintegra os demais arcos do ciclo (linhas 22--23). Processa os arcos de saída (linhas 24--26): para cada \texttt{(contracted\_label, w)} em \(F'\), adiciona \texttt{(v\_out, w)} usando \texttt{out\_from\_cycle[w]}. Remove o supervértice (linha 27), restaura os pesos originais (linhas 28--29) e devolve \texttt{F\_prime} (linha 30).

A função devolve um digrafo contendo exatamente \(|V|-1\) arcos onde cada vértice \(v \neq r_0\) tem grau de entrada 1, todos os vértices são alcançáveis a partir de \(r_0\), e o custo total é mínimo. O grafo original \texttt{D} não é modificado devido à cópia (linha 2). A complexidade é \(O(mn)\) no pior caso, onde cada nível de recursão (até \(O(n)\) níveis) processa \(O(m)\) arcos durante normalização, detecção de ciclos e contração/expansão.
...
A seguir, demonstramos a execução completa do algoritmo de Chu--Liu/Edmonds em um grafo exemplo, ilustrando cada fase do processo: normalização, construção de $A_0$, detecção de ciclos, contração, resolução recursiva e reexpansão.

\begin{figure}[H]
	\centering
	\input{capitulos/implementacao-chuliu/figures/fig_exemplo_grafo_inicial.tex}
	\caption{Grafo direcionado ponderado inicial com raiz no vértice 0. O grafo contém 9 vértices e múltiplas arcos com pesos variados. O primeiro passo do algoritmo seria remover arcos que entram na raiz, porém não há nenhuma neste caso, logo não existe necessidade de alterar o grafo.}
	\label{fig:exemplo-grafo-inicial}
\end{figure}

O primeiro passo do nosso algoritmo seria remover as arcos que entram na raiz (vértice $0$), porém não há nenhuma nesse caso, logo não existe a necessidade de alterar o grafo.

O próximo passo é normalizar os pesos das arcos de entrada para cada vértice. Nessa etapa, para cada vértice $v$ (exceto a raiz), o algoritmo encontra a arco de menor peso que entra em $v$ e subtrai esse menor peso de todos os arcos que entram em $v$ (isso serve para zerar o peso da arco mínima de entrada em cada vértice).

\begin{figure}[H]
	\centering
	\input{capitulos/implementacao-chuliu/figures/fig_exemplo_normalizacao_parcial.tex}
	\caption{Normalização parcial das arcos de entrada para o vértice 1. As arcos de entrada são $(0 \to 1)$ com peso original 3 e $(2 \to 1)$ com peso original 1. Elegendo a arco $(2 \to 1)$ como a de menor peso (peso mínimo = 1), subtraímos este valor de todos os arcos de entrada: $(0 \to 1)$ passa de peso 3 para 2, e $(2 \to 1)$ passa de peso 1 para 0 (destacadas em vermelho). Esse processo é repetido para todos os demais vértices.}
	\label{fig:exemplo-normalizacao-parcial}
\end{figure}

Com os pesos normalizados, o próximo passo é construir $A_0$: para isso, selecionamos para cada vértice a arco de custo reduzido zero de entrada. Detectamos um ciclo em $A_0$, formado pelos vértices $\{1, 2\}$. Portanto, precisamos contrair esse ciclo em um supervértice $n*0$.

\begin{figure}[H]
	\centering
	\input{capitulos/implementacao-chuliu/figures/fig_exemplo_grafo_contraido.tex}
	\caption{Grafo contraído após detecção do ciclo $C = \{1, 2\}$ em $A_0$. O ciclo foi contraído no supervértice $n*0$ (destacado em vermelho). As arcos que entravam ou saíam do ciclo foram redirecionadas para o supervértice, com custos ajustados segundo as fórmulas $c'(u,x_C) := c(u,w) - y(w)$ para arcos de entrada e $c'(x_C,v) := c(w,v)$ para arcos de saída.}
...
\begin{figure}[H]
	...
	\caption{Arborescência ótima $F'$ obtida no grafo contraído. todos os arcos selecionadas têm custo reduzido 0 (destacados em vermelho), e o grafo forma uma arborescência válida enraizada em 0: cada vértice (exceto a raiz) tem exatamente uma arco de entrada, não há ciclos, e todos os vértices são alcançáveis a partir da raiz. Como $F'$ é acíclico, alcançamos o caso base da recursão.}
	\label{fig:exemplo-arborescencia-contraida}
\end{figure}

Após validarmos que $A_0$ não possui mais ciclos e forma uma arborescência, iniciamos o processo de reexpansão do ciclo contraído para obter a arborescência final no grafo original. Adicionamos a arco de entrada ao ciclo $(0, 1)$, as arcos internas do ciclo modificado $(1, 2)$, e as arcos de saída $(1, 3)$, chegando a uma arborescência válida.

\begin{figure}[H]
	\centering
	\input{capitulos/implementacao-chuliu/figures/fig_exemplo_arborescencia_final.tex}
	\caption{Arborescência ótima final no grafo original com pesos restaurados. O supervértice $n*0$ foi expandido de volta para os vértices 1 e 2, com a arco externa $(0, 1)$ escolhida pela solução recursiva conectando ao ciclo. A arco interna $(2, 1)$ do ciclo original foi removida para manter a propriedade de arborescência ($\deg^{-}(v)=1$). O resultado é uma 0-arborescência de custo mínimo com exatamente 8 arcos, onde cada vértice não-raiz tem grau de entrada 1 e todos são alcançáveis a partir da raiz 0.}
...
\begin{table}[H]
	...
	\begin{tabular}{>{\raggedright\arraybackslash}p{0.46\textwidth}|>{\raggedright\arraybackslash}p{0.46\textwidth}}
		...
		Expanda \(x_C\) para o ciclo original \(C\). Se \((u,x_C)\in F'\), adicione \((u,v)\) onde \(v\) é o vértice do ciclo mapeado por \(u\), remova a arco interna entrando em \(v\), e reintegre demais arcos de \(C\). Restaure custos originais.
		                                  &
		\textbf{Linhas 17--30:}
