...
A figura a seguir ilustra o funcionamento da função \texttt{get\_arcs\_entering\_X} em um digrafo que vamos denotar por \(D_{32}\). Os vértices em laranja pertencem ao conjunto \(X\), e a função identifica apenas os arcos em vermelho, que saem de vértices fora de \(X\) e entram em vértices dentro de \(X\). Arcos internos a \(X\), externos a \(X\), ou saindo de \(X\) não são retornados.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth]
		...
		% ===== ANTES (esquerda) =====
		\begin{scope}
			\node[font=\bfseries, blue!80!black] at (2, 3.5) {Antes: pesos originais};
			...
			\draw[orange!60!black, thick, dashed, rounded corners=8pt]
			(3.2, 3.2) rectangle (4.8, -2.2);
			\node[orange!80!black, font=\bfseries\small] at (5.4, 3.0) {$X$};

			\node[font=\small, green!60!black] at (2, -3) {$\Delta(X) = 2$};
		\end{scope}

		% Seta indicando transformação
		\draw[->, ultra thick, blue!60!black] (5.8, 0.5) -- node[above, font=\small] {subtrai $\Delta(X)$} (7.2, 0.5);

		% ===== DEPOIS (direita) =====
		\begin{scope}[xshift=8cm]
			\node[font=\bfseries, blue!80!black] at (2, 3.5) {Depois: pesos atualizados};

			% Vértices fora de X
			\node[vtx] (u1b) at (0, 2) {$u_1$};
			\node[vtx] (u2b) at (0, 0) {$u_2$};
			\node[vtx] (u3b) at (0, -2) {$u_3$};

			% Vértices dentro de X
			\node[vtxX] (v1b) at (4, 2.5) {$v_1$};
			\node[vtxX] (v2b) at (4, 0.5) {$v_2$};
			\node[vtxX] (v3b) at (4, -1.5) {$v_3$};

			% Arcos entrando em X (atualizados)
			\draw[arcEntering] (u1b) -- node[cost, above] {1} (v1b);
			\draw[arcTight] (u2b) -- node[costTight, above] {0} (v2b);
			\draw[arcEntering] (u3b) -- node[cost, above] {2} (v3b);
			\draw[arcEntering] (u1b) to[bend right=15] node[cost, below, pos=0.3] {3} (v2b);

			% Arcos internos a X (inalterados)
			\draw[arc] (v1b) to[bend right=20] node[cost, right, pos=0.6] {1} (v2b);
			\draw[arc] (v2b) to[bend right=20] node[cost, left, pos=0.4] {2} (v3b);

			% Arcos externos a X (inalterados)
			\draw[arc] (u1b) to[bend right=20] node[cost, left, pos=0.6] {1} (u2b);
			\draw[arc] (u2b) to[bend right=20] node[cost, left, pos=0.6] {3} (u3b);

			% Arcos saindo de X (inalterados)
			\draw[arc] (v1b) to[bend left=30] node[cost, above, pos=0.7] {2} (u1b);
			...
			\node[font=\small, blue!70!black] at (2, -3) {Arco \textcolor{blue!70!black}{apertado} $\to A_0$};
		\end{scope}
	\end{tikzpicture}
...
Recebe como entrada um dígrafo \texttt{D\_original} e a raiz \texttt{r0}. A implementação cria uma cópia do dígrafo original (linha 2) para preservar a entrada, inicializa estruturas auxiliares \texttt{A\_zero} (lista de arcos apertados), \texttt{Dual\_list} (lista de pares \((X, \Delta(X))\) para fins de validação dual), e \texttt{D\_zero} (dígrafo de arcos apertados) (linhas 3-5).

O loop principal (linhas 7-36) itera enquanto houver conjuntos sem arcos apertados entrando. Em cada iteração, calcula-se as componentes fortemente conexas de \(D_0\) usando \texttt{nx.condensation} (linha 11), que devolve um grafo acíclico dirigido (DAG) onde cada nó representa uma componente e contém o atributo \texttt{"members"} com os vértices originais. Utilizamos componentes fortemente conexas porque elas identificam naturalmente os conjuntos maximais de vértices que ainda não possuem arcos apertados entrando, evitando a necessidade de rastrear manualmente quais conjuntos já foram processados.

Em seguida, identificam-se as fontes (componentes sem arcos entrando) no grafo de condensação (linha 14). Se há apenas uma fonte, significa que todos os vértices estão em uma única componente alcançável pela raiz através de arcos apertados, garantindo que \(D_0\) contém uma r-arborescência e encerrando o loop (linhas 17-19).

Para cada fonte \(u\) diferente da componente contendo \(r_0\) (linha 21), obtém-se o conjunto \(X\) de vértices da componente (linha 22), identificam-se os arcos entrando em \(X\) usando \texttt{get\_arcs\_entering\_X} (linha 24), calcula-se o peso mínimo \(\Delta(X)\) usando \texttt{get\_minimum\_weight\_cut} (linha 25), e atualiza-se os pesos com \texttt{update\_weights\_in\_X}, registrando novos arcos apertados (linha 29). A elevação simultânea de potenciais para todos os vértices de \(X\) mantém a propriedade de que arcos internos a \(X\) permanecem com o mesmo custo reduzido relativo, preservando a correção do algoritmo. Finalmente, adiciona-se \((X, \Delta(X))\) à lista dual se \(\Delta(X) > 0\) (linhas 32-35), permitindo verificação posterior das condições de otimalidade dual.
