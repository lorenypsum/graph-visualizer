...
Se esses arcos de custo zero formam uma \(r\)-arborescência, o algoritmo termina. Caso contrário, identificamos novamente os subconjuntos minimais sem arcos de custo zero entrando neles. Podemos entender também esses subconjuntos minimais como as componentes fortemente conexas (SCCs) do dígrafo formado pelos arcos justos.

No exemplo da Figura~\ref{fig:frank-iter1-result}, os arcos justos não formam uma \(r\)-arborescência, pois o conjunto \(\{c,d\}\) forma uma componente fortemente conexa com dois arcos justos entre \(c\) e \(d\). Portanto, \(\{c,d\}\) é um subconjunto minimal.
