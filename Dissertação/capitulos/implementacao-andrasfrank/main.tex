...
Recebe como entrada um dígrafo \texttt{D}, lista de \texttt{arcs} entrando em \(X\), o valor \texttt{min\_weight} a ser subtraído, uma lista \texttt{A\_zero} para armazenar arcos de peso zero, e o dígrafo \texttt{D\_zero} para adicionar os arcos apertados.
...
A seguir apresentamos a função principal da Fase~1, responsável por elevar os potenciais dos vértices iterativamente até que cada conjunto de vértices possua ao menos um arco apertado entrando. O processo utiliza componentes fortemente conexas para identificar quais conjuntos necessitam elevação.

Recebe como entrada um dígrafo \texttt{D\_original} e a raiz \texttt{r0}. A implementação cria uma cópia do dígrafo original (linha 2) para preservar a entrada, inicializa estruturas auxiliares \texttt{A\_zero} (lista de arcos apertados), \texttt{Dual\_list} (lista de pares \((X, \Delta(X))\) para fins de validação dual), e \texttt{D\_zero} (dígrafo de arcos apertados) (linhas 3-5). Um contador de iterações é inicializado na linha 6.

O loop principal (linhas 7-22) itera enquanto houver conjuntos sem arcos apertados entrando. Em cada iteração, incrementa-se o contador (linha 8), calcula-se as componentes fortemente conexas de \(D_0\) usando \texttt{nx.condensation} (linha 9), que devolve um grafo acíclico dirigido (DAG, do inglês \emph{directed acyclic graph}) onde cada vértice representa uma componente e contém o atributo \texttt{"members"} com os vértices originais. Utilizamos componentes fortemente conexas porque elas identificam naturalmente os conjuntos maximais de vértices que ainda não possuem arcos apertados entrando, evitando a necessidade de rastrear manualmente quais conjuntos já foram processados.

Em seguida, identificam-se as fontes (componentes sem arcos entrando) no grafo de condensação (linha 10). Se há apenas uma fonte, significa que todos os vértices estão em uma única componente alcançável pela raiz através de arcos apertados, garantindo que \(D_0\) contém uma r-arborescência e encerrando o loop (linhas 11-12).

Para cada fonte \(u\) no grafo de condensação (linha 13), obtém-se o conjunto \(X\) de vértices da componente (linha 14). Se \(r_0 \in X\), a fonte é ignorada (linhas 15-16), pois a componente contendo a raiz não necessita elevação. Caso contrário, identificam-se os arcos entrando em \(X\) usando \texttt{get\_arcs\_entering\_X} (linha 17), calcula-se o peso mínimo \(\Delta(X)\) usando \texttt{get\_minimum\_weight\_cut} (linha 18), e atualiza-se os pesos com \texttt{update\_weights\_in\_X}, registrando novos arcos apertados (linha 19). A elevação simultânea de potenciais para todos os vértices de \(X\) mantém a propriedade de que arcos internos a \(X\) permanecem com o mesmo custo reduzido relativo, preservando a correção do algoritmo. Finalmente, adiciona-se \((X, \Delta(X))\) à lista dual se \(\Delta(X) > 0\) (linhas 20-21), permitindo verificação posterior das condições de otimalidade dual.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.85, every node/.style={scale=0.85}]
		...
		\begin{scope}[xshift=12cm]
			...
			\node[font=\small, blue!70!black] at (2, -2) {Arcos apertados em $A_0$};
		\end{scope}
	...
	\end{tikzpicture}
	\caption{Execução da função \texttt{phase1} em \(D_{32}\). À esquerda, o dígrafo original \(D\) com pesos dos arcos. Ao centro, \(D_0\) inicial após linha 5 (\texttt{build\_D\_zero}), contendo apenas vértices sem arcos. À direita, \(D_0\) final após o loop de elevação de potenciais, contendo apenas os arcos \textcolor{blue!70!black}{apertados} (custo reduzido zero) que formam uma r-arborescência. Durante as iterações (linhas 7-22), os potenciais são elevados para cada componente sem arcos apertados entrando, até que todos os vértices sejam alcançáveis a partir de \(r_0\) através de arcos em \(D_0\).}
\end{figure}

A seguir ilustramos a execução da função \texttt{phase2} em \(D_{32}\), mostrando como a r-arborescência é construída incrementalmente a partir do conjunto \(A_0\) de arcos apertados obtidos da Fase~1. A figura apresenta quatro estados: o conjunto inicial \(A_0\) com 6 arcos apertados, e três estados intermediários da arborescência \texttt{Arb} sendo construída, destacando em verde os arcos adicionados em cada iteração.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.72, every node/.style={scale=0.72}]
		...
		\begin{scope}
			\node[font=\bfseries, blue!80!black] at (2, 4.5) {$A_0$ (arcos apertados)};
...
\begin{tcolorbox}[
	...
	\begin{itemize}
		...
		\item \textbf{Implementação:} A função \texttt{build\_D\_zero(D)} inicializa o dígrafo \(D_0\) vazio, que será populado apenas com arcos apertados. Implicitamente, os potenciais iniciam em zero, pois os pesos no dígrafo \(D\) representam os custos reduzidos \(c_y(u,v) = c(u,v) - y(v)\). Com \(y(v) = 0\), temos \(c_y = c\) no início.
	\end{itemize}

	\vspace{2mm}
	\textbf{Passo 2 — Iteração de elevação de potenciais:}
	\begin{itemize}
		\item \textbf{Descrição teórica:} Enquanto existir conjunto \(X \subseteq V \setminus \{r\}\) sem arco apertado entrando:
		      \begin{itemize}
			      \item Calcule \(\Delta(X) := \min\{c(u,v) - y(v) : u \notin X, v \in X\}\).
			      \item Para cada \(v \in X\), atualize \(y(v) := y(v) + \Delta(X)\).
		      \end{itemize}
		\item \textbf{Implementação:} Esse processo de iteração é realizado pela composição sequencial de três funções auxiliares:
		      \begin{enumerate}
			      ...
			      \item \texttt{update\_weights\_in\_X(D, arcs, min\_weight, A\_zero, D\_zero)}: Implementa a atualização dos potenciais. Para cada arco \((u,v)\) entrando em \(X\), subtrai \texttt{min\_weight} de \texttt{D[u][v]["w"]}, efetivamente calculando o novo custo reduzido:
			            ...
			            onde \(y'(v) = y(v) + \Delta(X)\) é o novo potencial. Arcos cujo custo reduzido atinge zero (\texttt{D[u][v]["w"] == 0}) são adicionados a \(A_0\) e \(D_0\), tornando-se \textbf{apertados}.
		      \end{enumerate}
	\end{itemize}
...
\begin{tcolorbox}[
	...
	\emph{Implementa o algoritmo completo de András Frank, integrando as fases de construção dos potenciais duais, obtenção dos arcos apertados, construção da arborescência e verificação de otimalidade dual.}
	\tcblower
	\begin{lstlisting}[language=Python]
...
\paragraph*{Fase 1 — Iteração 1:} O algoritmo inicia com todos os potenciais \(y(v) = 0\) e \(D_0\) vazio (sem arcos apertados). Calcula-se as componentes fortemente conexas de \(D_0\): cada vértice forma sua própria componente. As fontes são todos os vértices exceto \(r_0\). Para o conjunto \(X = \{u_1, u_2, u_3\}\), identifica-se os arcos entrando: \((r_0, u_1)\) com peso 1, \((r_0, u_2)\) com peso 2, \((r_0, u_3)\) com peso 1. O mínimo é \(\Delta(X) = 1\). Subtraindo esse valor, os arcos \((r_0, u_1)\) e \((r_0, u_3)\) tornam-se apertados (peso 0) e são adicionados a \(A_0\).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.85, every node/.style={scale=0.85}]
		...
		\begin{scope}[xshift=7cm]
			\node[font=\bfseries, blue!80!black] at (2, 4.5) {Depois: arcos apertados};
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Fase~1, Iteração 1: Elevação de potenciais para \(X_1 = \{u_1, u_2, u_3\}\). À esquerda, o conjunto \(X_1\) (em laranja) sem arcos apertados entrando. Calcula-se \(\Delta(X_1) = \min\{1, 2, 1\} = 1\). À direita, após subtrair \(\Delta(X_1)\), os arcos \((r_0, u_1)\) e \((r_0, u_3)\) tornam-se \textcolor{blue!70!black}{apertados} (custo reduzido 0) e são adicionados a \(A_0\).}
	\label{fig:frank-exemplo-iter1}
\end{figure}

\paragraph*{Fase 1 — Iteração 2:} Com \(A_0 = \{(r_0, u_1), (r_0, u_3)\}\), as componentes fortemente conexas são: \(\{r_0, u_1, u_3\}\) e \(\{u_2\}\), \(\{v_1\}\), \(\{v_2\}\), \(\{v_3\}\). A fonte (excluindo a raiz) é \(\{u_2\}\). Para \(X = \{u_2\}\), o único arco entrando é \((r_0, u_2)\) com peso atual 1 (após iteração anterior). Portanto \(\Delta(X) = 1\), e após subtração, \((r_0, u_2)\) torna-se apertado. Também na mesma iteração, para \(X = \{v_1, v_2, v_3\}\), calcula-se \(\Delta(X) = \min\{3, 2, 4, 5\} = 2\), e os arcos são atualizados conforme ilustrado.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.75, every node/.style={scale=0.75}]
		...
		% Arcos entrando em X (antes de ficarem apertados)
		...
		\node[font=\small, orange!80!black] at (3, -0.5) {Após subtração: $(u_2, v_2)$ torna-se apertado};
	\end{tikzpicture}
	\caption{Fase~1, Iteração 2: Para o conjunto \(X = \{v_1, v_2, v_3\}\) (em laranja), calcula-se \(\Delta(X) = \min\{3, 2, 4, 5\} = 2\). Após subtrair esse valor dos arcos entrando em \(X\), o arco \((u_2, v_2)\) atinge custo reduzido 0 e é adicionado a \(A_0\). Note que \((u_1, u_2)\) também se tornou apertado na mesma iteração ao processar \(X = \{u_2\}\).}
	\label{fig:frank-exemplo-iter2}
\end{figure}

\paragraph*{Fase 1 — Estado final:} Após mais iterações elevando potenciais para os conjuntos restantes, o dígrafo \(D_0\) de arcos apertados fica completo, contendo uma r-arborescência. A Figura~\ref{fig:frank-exemplo-A0-final} mostra o conjunto final \(A_0\) com 6 arcos de custo reduzido zero.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.85, every node/.style={scale=0.85}]
		...
		\node[font=\bfseries, blue!80!black] at (3, 4.5) {$A_0$ final: arcos apertados após Fase~1};
...
\paragraph*{Verificação de otimalidade dual:} A função \texttt{check\_dual\_optimality\_condition} confirma que para cada par \((X, \Delta(X))\) em \texttt{Dual\_list} (conjuntos cujos potenciais foram elevados com \(\Delta(X) > 0\)), existe exatamente um arco da arborescência final cruzando a fronteira de \(X\). Essa condição, juntamente com os arcos apertados, garante que as condições de folga complementar da programação linear dual são satisfeitas, implicando que a arborescência encontrada é de custo mínimo global.
...
\begin{table}[H]
	...
	\begin{tabular}{>{\raggedright\arraybackslash}p{0.46\textwidth}|>{\raggedright\arraybackslash}p{0.46\textwidth}}
		...
		\midrule
		\rowcolor{blue!5}
		\textbf{Passo 1:} Inicialização de potenciais e \(A_0\)

		Defina \(y(v) := 0\) para todo \(v \in V\).

		Inicialize \(A_0 := \emptyset\).

		Construa dígrafo vazio \(D_0\) que conterá apenas arcos apertados (\(c_y = 0\)).
		                                  &
		...
		\vspace{2mm}
		Potenciais implícitos \(y(v) = 0\) pois custos reduzidos iniciais \(c_y(u,v) = c(u,v) - y(v) = c(u,v)\).
		\\
		\midrule
		\rowcolor{green!5}
		\textbf{Passo 2:} Elevação de potenciais (iterativo)

		Enquanto existir \(X \subseteq V \setminus \{r\}\) sem arco apertado entrando:

		\quad (a) Calcule \(\Delta(X) := \min\{c_y(u,v) : u \notin X, v \in X\}\)

		\quad (b) Para cada \(v \in X\), atualize \(y(v) := y(v) + \Delta(X)\)

		\quad (c) Adicione arcos que atingirem \(c_y = 0\) ao conjunto \(A_0\)
		                                  &
		...
		\quad \texttt{if C.in\_degree(x) == 0]}

		Para cada fonte \texttt{u} (exceto raiz):

		\quad \texttt{X = C.nodes[u]["members"]}

		\quad \texttt{arcs = get\_arcs\_entering\_X(D, X)}

		\quad \texttt{min\_w = get\_minimum\_weight\_cut(arcs)}

		\quad \texttt{update\_weights\_in\_X(D, arcs, min\_w,}

		\quad\quad \texttt{A\_zero, D\_zero)}

		\vspace{2mm}
		Identifica componentes fortemente conexas sem arcos apertados, calcula \(\Delta(X)\) e atualiza pesos.
		\\
		\midrule
		\rowcolor{orange!5}
		\textbf{Passo 2(a):} Identificação de arcos cruzando fronteira

		Para conjunto \(X\), determine

		\(\{(u,v) \in A : u \notin X, v \in X\}\)

		isto é, arcos que entram em \(X\) vindos de fora.
		                                  &
		\textbf{Função \texttt{get\_arcs\_entering\_X}:}

		\texttt{return [(u, v, data)}

		\quad \texttt{for u, v, data in D.edges(data=True)}

		\quad \texttt{if u not in X and v in X]}

		\vspace{2mm}
		Compreensão de lista filtra arcos cruzando fronteira de \(X\).
		\\
		\midrule
		\rowcolor{purple!5}
		\textbf{Passo 2(b):} Cálculo do valor \(\Delta(X)\)

		Determine o custo mínimo entre todos os arcos entrando em \(X\):
		...
		\quad \texttt{for \_, \_, data in arcs)}

		\vspace{2mm}
		Função \texttt{min} com gerador extrai peso mínimo da lista de arcos.
		\\
		\midrule
		\rowcolor{yellow!5}
		\textbf{Passo 2(c):} Atualização de pesos e construção de \(A_0\)

		Para cada arco \((u,v)\) entrando em \(X\):

		\quad Atualize \(c_y(u,v) := c_y(u,v) - \Delta(X)\)

		\quad Se \(c_y(u,v) = 0\), adicione \((u,v)\) a \(A_0\)
		                                  &
		\textbf{Função \texttt{update\_weights\_in\_X}:}

		\texttt{for u, v, \_ in arcs:}

		\quad \texttt{D[u][v]["w"] -= min\_weight}

		\quad \texttt{if D[u][v]["w"] == 0:}

		\quad\quad \texttt{A\_zero.append((u, v))}

		\quad\quad \texttt{D\_zero.add\_edge(u, v)}

		\vspace{2mm}
		Subtrai \(\Delta(X)\) dos pesos e registra arcos apertados.
		\\
		\midrule
		\rowcolor{cyan!5}
		\textbf{Passo 3:} Verificação de término

		Se \(D_0 = (V, A_0)\) contém r-arborescência (todos vértices alcançáveis via arcos apertados), encerre Fase~1.
		                                  &
		\textbf{Condição de parada — Linhas 11--12:}

		\texttt{if len(sources) == 1:}

		\quad \texttt{break}

		\vspace{2mm}
		Uma única fonte significa que \(D_0\) forma r-arborescência acíclica.
		\\
		\midrule
		\rowcolor{red!5}
		\textbf{Passo 4:} Construção da arborescência (Fase~2)

		Caso acíclico: Construa r-arborescência \(F\) a partir de \(A_0\), selecionando arcos que conectam vértices já incluídos a novos vértices.

		Devolva \(F\) com pesos originais restaurados.
		                                  &
		\textbf{Função \texttt{phase2} — Versão incremental:}

		\texttt{Arb = nx.DiGraph()}

		\texttt{Arb.add\_node(r0)}

		\texttt{for \_ in range(n - 1):}

		\quad \texttt{for u, v in A\_zero:}

		\quad\quad \texttt{if u in Arb and v not in Arb:}

		\quad\quad\quad \texttt{Arb.add\_edge(u, v, **edge\_data)}

		\quad\quad\quad \texttt{break}

		\vspace{2mm}
		Complexidade: \(O(nm)\). Adiciona um vértice por iteração.
		\\
		\midrule
		\rowcolor{red!5}
		\textbf{Passo 4:} Construção otimizada (Fase~2 versão BFS)

		Mesma ideia, mas usando fila de prioridade para exploração ordenada, evitando buscas lineares repetidas.
		                                  &
		\textbf{Função \texttt{phase2\_v2} — Versão BFS:}

		\texttt{Arb = nx.DiGraph()}

		\texttt{for i, (u, v) in enumerate(A\_zero):}

		\quad \texttt{Arb.add\_edge(u, v, w=i)}

		\texttt{q = []} \quad \emph{(fila de prioridade)}

		\texttt{while q:}

		\quad \texttt{\_, u, v = heapq.heappop(q)}

		\quad \texttt{if v not in V: continue}

		\quad \texttt{A.add\_edge(u, v, w=D[u][v]["w"])}

		\quad \texttt{[adiciona vizinhos à fila]}

		\vspace{2mm}
		Complexidade: \(O(m \log m)\). Mais eficiente para grafos densos.
		\\
		\midrule
		\rowcolor{pink!5}
		\textbf{Verificação de otimalidade dual}

		Para cada conjunto \(X\) elevado (com \(\Delta(X) > 0\)), verifique que exatamente um arco de \(F\) cruza \(\delta^-(X)\).

		Condição de folga complementar: garante otimalidade via dualidade forte.
		                                  &
		\textbf{Função \texttt{check\_dual\_optimality\_condition}:}

		\texttt{for X, z in Dual\_list:}

		\quad \texttt{count = 0}

		\quad \texttt{for u, v in Arb.edges():}

		\quad\quad \texttt{if u not in X and v in X:}

		\quad\quad\quad \texttt{count += 1}

		\quad\quad\quad \texttt{if count > 1: return False}

		\texttt{return True}

		\vspace{2mm}
		Verifica que cada corte dual possui exatamente um arco da arborescência.
		\\
		\bottomrule
	\end{tabular}
	\caption{Correspondência entre a descrição teórica do algoritmo de András Frank e sua implementação em Python. Cada linha da coluna direita mostra a tradução direta dos conceitos primal-duais da coluna esquerda em operações concretas sobre grafos. As cores distinguem as diferentes fases: inicialização (azul), elevação de potenciais (verde/laranja/roxo/amarelo), verificação (ciano), construção da arborescência (vermelho) e validação dual (rosa).}
...
\texttt{check\_dual\_}-
\texttt{optimality\_condition} encapsulam exatamente as operações descritas na teoria, preservando as propriedades de correção e as garantias de otimalidade do algoritmo original. A utilização de componentes fortemente conexas (\texttt{nx.condensation}) para identificar conjuntos sem arcos apertados entrando é uma implementação eficiente da verificação teórica, evitando enumeração explícita de todos os subconjuntos de vértices.
