...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.1]
		...
		\draw[tight] (a) -- node[costB, above] {4} (c);
		\draw[tight] (c) to[bend left=15] node[costB, right] {0} (d);
		\draw[tight] (b) -- node[costB, below] {0} (e);

		% arcos não justos
		\draw[loose] (r) -- node[costG, below left] {1} (b);  % 3-2 = 1 (ainda não é justo!)
		...
		\draw[loose] (d) to[bend left=15] node[costG, left] {0} (c);  % 1-1 = 0 (forma ciclo!)
