...
Voltemos ao exemplo da Figura~\ref{fig:frank-iter1-result}. Após a primeira iteração de reduções, os arcos justos (custo zero) são: \((r,a)\), \((r,b)\), \((b,e)\), \((c,d)\) e \((d,c)\). Todos os vértices não-raiz possuem arcos justos entrando, exceto que o conjunto \(\{c,d\}\) forma um ciclo justo. Como não existem arcos justos entrando em \(\{c,d\}\) vindos de fora do conjunto, ele é um \textbf{subconjunto minimal}.

Um conjunto \(X \subseteq V \setminus \{r\}\) é dito \textbf{minimal} se: (i) não há arcos justos entrando em \(X\), e (ii) para qualquer subconjunto próprio \(Y \subset X\), existe ao menos um arco justo entrando em \(Y\). A segunda condição é essencial: no nosso exemplo, não poderíamos escolher apenas \(\{c\}\) ou apenas \(\{d\}\) isoladamente, pois esses subconjuntos já possuem arcos justos entrando --- os arcos do ciclo interno.
