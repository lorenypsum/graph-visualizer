...
Este exemplo foi construído para exibir:
\begin{itemize}
	\item \textbf{Primeira iteração:} Redução de custos em vértices isolados.
	\item \textbf{Formação de ciclo:} Os arcos \((c,d)\) e \((d,c)\) formarão um ciclo justo.
	\item \textbf{Segunda iteração:} Identificação do ciclo como componente fortemente conexa,
	      tratamento do conjunto \(\{c,d\}\) como subconjunto minimal.
	\item \textbf{Contração e expansão:} Necessidade de contrair o ciclo para resolver
