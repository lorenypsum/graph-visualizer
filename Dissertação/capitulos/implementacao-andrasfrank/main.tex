...
\paragraph{Visão geral do capítulo.} Este capítulo está organizado da seguinte forma:
\begin{itemize}\setlength{\itemsep}{2pt}
	\item \textbf{Seção~\ref{sec:algoritmo}:} Apresentação intuitiva do algoritmo através de um exemplo completo, ilustrando redução de custos, identificação de componentes fortemente conexas, formação de ciclos justos e contração.
	\item \textbf{Seção~\ref{sec:descricao-algoritmo}:} Descrição formal do algoritmo em duas fases (redução de custos e construção da arborescência), discussão de corretude, complexidade e conceitos-chave como conjuntos minimais e fontes no grafo de condensação.
	\item \textbf{Seção~\ref{sec:implementacao}:} Implementação detalhada em Python, com funções modulares correspondendo diretamente aos passos teóricos, análise de complexidade e exemplos de execução.
\end{itemize}

Conceitos centrais abordados incluem: \emph{arcos justos} (custo reduzido zero), \emph{subconjuntos minimais} (conjuntos sem arcos justos entrando), \emph{componentes fortemente conexas} (SCCs) e \emph{fontes no grafo de condensação} (SCCs sem arcos justos entrando, correspondendo exatamente aos conjuntos minimais).
