...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.1]
		...
		\draw[tight] (r) -- node[costG, below left] {0} (b);  
		% arcos não justos
...
No nosso exemplo, após a primeira iteração, os arcos justos formam o ciclo \(\{c,d\}\).
A Figura~\ref{fig:frank-sccs} ilustra a identificação das componentes fortemente conexas.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.1]
		...
		\draw[loose, opacity=0.3] (r) .. controls (2,3.5) and (5,3.5) .. node[costG, above, opacity=0.5] {0} (c);
		...
		% Destacar as SCCs
		% SCC da raiz + alcançáveis
		\draw[green!60, ultra thick, dashed, rounded corners] (-0.6,-0.6) rectangle (3.6,2.6);
		\node[green!70!black, font=\small\bfseries, above] at (1.5,2.9) {$\{r,a,b,e\}$ (raiz)};

		% SCC do ciclo c-d - MINIMAL para fins de contração
		\draw[orange!70, ultra thick, dashed, rounded corners] (5.3,2.6) rectangle (6.7,-0.6);
		\node[orange!70, font=\small\bfseries, right] at (6.9,1) {$\{c,d\}$};
		\node[orange!70, font=\small\bfseries, right, font=\scriptsize] at (6.9,0.3) {\textbf{minimal}};
		\node[orange!70, font=\small, right, font=\scriptsize] at (6.9,-0.3) {(ciclo)};
	\end{tikzpicture}
	\caption{Identificação das componentes fortemente conexas (SCCs) no grafo formado pelos arcos justos
		após a primeira iteração. A componente que contém a raiz inclui \(\{r,a,b,e\}\) (em verde).
		O conjunto \(\{c,d\}\) (em laranja) forma um ciclo e será tratado como \textbf{minimal}
		para fins de contração, pois é uma SCC não-trivial que forma um ciclo nos arcos justos.}
	\label{fig:frank-sccs}
\end{figure}Repetimos o processo: para cada subconjunto minimal \(X\), calculamos \(\delta(X)\)
...
\begin{align*}
	...
	\delta(\{b\}) & = \min\{c(r,b)\} = 3,                                   \\
	\delta(\{c\}) & = \min\{c(r,c), c(a,c)\} = \min\{7, 5\} = 5,            \\
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.1]
		...
		% arcos não justos
		\draw[loose] (r) .. controls (2,3.5) and (5,3.5) .. node[costG, above] {0} (c);  % 7-5-2 = 0
		\draw[loose] (a) -- node[costG, right] {2} (d);  % 4-2 = 2
		\draw[loose] (b) to[bend right=15] node[costG, left] {3} (a);  % 5-2 = 3
		\draw[loose] (d) -- node[costG, right] {1} (e);  % 3-2 = 1
		\draw[loose] (c) .. controls (7.5,2.5) and (7.5,-2.5) .. node[costG, right] {2} (e);  % 6-2-2 = 2
	\end{tikzpicture}
	\caption{Custos reduzidos após Iteração 1. Arcos justos em azul: \((r,a)\), \((r,b)\), \((a,c)\),
		\((c,d)\), \((d,c)\) e \((b,e)\). Os arcos \((c,d)\) e \((d,c)\) formam um ciclo justo.
		Note que todos os vértices não-raiz possuem ao menos um arco justo entrando.}
	\label{fig:frank-iter1}
\end{figure}

\textbf{Análise:} Observe que:
\begin{itemize}
	\item Todos os vértices não-raiz (\(a\), \(b\), \(c\), \(d\), \(e\)) possuem arcos justos entrando.
...
\begin{itemize}
	\item \(\{r\}\) --- contém a raiz (componente verde)
	\item \(\{a\}\) --- componente trivial com \((r,a)\) justo entrando
	\item \(\{b\}\) --- componente trivial com \((r,b)\) justo entrando
	\item \(\{c, d\}\) --- componente não-trivial (ciclo) com \((a,c)\) justo entrando
	\item \(\{e\}\) --- componente trivial com \((b,e)\) justo entrando
\end{itemize}

Observamos que \textbf{nenhuma} componente não-raiz é minimal, pois todas possuem arcos justos
entrando vindos de fora. Entretanto, como os arcos justos formam um ciclo, devemos prosseguir
com a contração. Na próxima iteração, após modificarmos a identificação de componentes minimais
(como veremos adiante), o conjunto \(\{c,d\}\) será tratado como minimal para fins de contração.

O conjunto atual de arcos justos após a primeira iteração é:
\[
	A_0 = \{(r,a), (r,b), (a,c), (c,d), (d,c), (b,e)\}.
\]

Como \(A_0\) contém o ciclo \(C = \{c,d\}\), não temos uma \(r\)-arborescência ainda.
