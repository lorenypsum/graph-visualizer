...
Um conjunto \(X \subseteq V \setminus \{r\}\) é dito \textbf{minimal} se: (i) não há arcos justos entrando em \(X\), e (ii) para qualquer subconjunto próprio \(Y \subset X\), existe ao menos um arco justo entrando em \(Y\).
...
No exemplo da Figura~\ref{fig:frank-iter1-result}, os arcos justos não formam uma \(r\)-arborescência, pois nem todos os vértices são alcançáveis a partir de \(r\) pelos arcos justos, mais especificamente, após a primeira iteração de reduções, os arcos justos (custo zero) são: \((r,a)\), \((r,b)\), \((b,e)\), \((c,d)\) e \((d,c)\). Todos os vértices não-raiz possuem arcos justos entrando, exceto que o conjunto \(\{c,d\}\) forma um ciclo justo. Como não existem arcos justos entrando em \(\{c,d\}\) vindos de fora do conjunto, ele é um \textbf{subconjunto minimal}.

O algoritmo utiliza \emph{componentes fortemente conexas} (SCCs) formadas pelos arcos justos para identificar eficientemente os conjuntos minimais. No exemplo, após a primeira iteração, formam-se as componentes \(\{r,a\}\), \(\{b\}\), \(\{e\}\) e \(\{c,d\}\), ilustradas na Figura~\ref{fig:frank-scc-minimal}. A componente \(\{c,d\}\) é não-trivial, pois contém um ciclo justo entre \(c\) e \(d\).
...
Após a contração, os arcos justos formam uma \(r\)-arborescência no dígrafo contraído. Assim termina a fase 1 do algoritmo. A fase 2 consiste em expandir o supervértice \(x_C\) de volta para o ciclo \(\{c,d\}\) e reconstruir a \(r\)-arborescência ótima no dígrafo original, selecionando o arco que entra em \(x_C\) e substituindo-o pelo arco correspondente no dígrafo original, além dos arcos justos internos ao ciclo, exceto o arco que entra no vértice escolhido.

\subsection{Identificação de conjuntos minimais através de componentes fortemente conexas}
\label{sec:identificacao-minimais}



Quando construímos o \emph{grafo de condensação} dessas SCCs (onde cada nó representa uma SCC e existe um arco entre duas SCCs se há um arco justo no grafo original indo de um vértice da primeira para um vértice da segunda), uma \textbf{fonte} é uma SCC sem arcos justos entrando. No exemplo, \(\{c,d\}\) é uma fonte no grafo de condensação e corresponde exatamente ao conjunto minimal que buscamos.

Por que essa identificação funciona? Porque (i) por definição de fonte, não há arcos justos entrando na SCC, e (ii) como trabalhamos com SCCs, qualquer vértice dentro da componente é alcançável a partir de qualquer outro usando arcos justos, garantindo que não há subconjunto próprio sem arcos justos entrando.

\paragraph{Múltiplas escolhas.} Em uma iteração podem existir múltiplas fontes (excluindo a raiz). Qualquer uma pode ser escolhida. Para ilustrar, considere um dígrafo simples com raiz \(r_0\) conectada a três vértices \(a\), \(b\), \(c\) com custos 2, 3, 2. Inicialmente, todos os três vértices são fontes. Podemos escolher qualquer um --- digamos \(\{a\}\) --- elevando seu potencial por \(\Delta(\{a\}) = 2\) e tornando \((r_0,a)\) justo, como na Figura~\ref{fig:frank-multiplas-fontes}. Após isso, \(\{a\}\) deixa de ser fonte e podemos processar as fontes restantes. A ordem não afeta o custo final, pois todas as elevações são determinadas univocamente pelos custos dos arcos.
...
\end{scope}
\end{tikzpicture}
\caption{Exemplo de múltiplas fontes. À esquerda: três componentes \(\{a\}\), \(\{b\}\), \(\{c\}\) (em laranja) são fontes. Qualquer uma pode ser escolhida. À direita: após processar \(\{a\}\) (elevando seu potencial por \(\Delta(\{a\}) = 2\)), o arco \((r\_0, a)\) torna-se justo (azul). Agora \(\{a\}\) deixa de ser fonte e restam \(\{b\}\) e \(\{c\}\). A ordem de processamento não afeta a arborescência ótima final.}
\label{fig:frank-multiplas-fontes}
\end{figure}

\subsection{Evolução das componentes e término do algoritmo}

Retornando ao exemplo principal, observamos como as SCCs evoluem. Inicialmente, com \(A_0 = \emptyset\), cada vértice forma sua própria SCC. Após a primeira iteração (Figura~\ref{fig:frank-iter1-result}), temos as componentes \(\{r,a\}\), \(\{b\}\), \(\{e\}\) e \(\{c,d\}\). O grafo de condensação possui arcos de \(\{r,a\}\) para \(\{b\}\) e de \(\{b\}\) para \(\{e\}\), mas nenhum arco entrando em \(\{c,d\}\), que portanto é uma fonte, como mostra a Figura~\ref{fig:frank-condensation-evolution} (esquerda).

Após elevar os potenciais de \(\{c,d\}\) por \(\Delta(\{c,d\}) = 3\) (Figura~\ref{fig:frank-reducao-cd}), o arco \((a,d)\) torna-se justo. Esse novo arco conecta \(\{c,d\}\) à componente \(\{r,a\}\), e as SCCs se fundem em \(\{r,a,c,d\}\), como ilustra a Figura~\ref{fig:frank-condensation-evolution} (direita).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.85]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Evolução do grafo de condensação. À esquerda: após a 1ª iteração, temos 4 SCCs. A SCC \(\{r,a\}\) (verde) contém a raiz. A SCC \(\{c,d\}\) (laranja) é fonte e portanto minimal. À direita: após elevar os potenciais de \(\{c,d\}\), o arco \((a,d)\) torna-se justo, conectando \(\{c,d\}\) a \(\{r,a\}\). As SCCs se fundem em \(\{r,a,c,d\}\). Agora existe apenas uma fonte (contendo \(r\)), satisfazendo a condição de término da Fase~1.}
	\label{fig:frank-condensation-evolution}
\end{figure}

O algoritmo termina quando existe exatamente \textbf{uma fonte} contendo a raiz. Nesse momento, todo vértice não-raiz possui ao menos um arco justo entrando, e o grafo \((V, A_0)\) contém uma \(r\)-arborescência. A cada iteração, o número de SCCs diminui (ou permanece igual) --- nunca aumenta, pois apenas adicionamos arcos. Como processamos ao menos uma fonte por iteração e o número de SCCs é limitado por \(n\), o algoritmo termina em no máximo \(O(n)\) iterações.

\subsection{Por que é essencial escolher conjuntos minimais}
\label{sec:escolha-minimal}

A escolha de conjuntos minimais não é apenas uma conveniência algorítmica --- é \emph{necessária} para a corretude. Para entender, considere a interpretação dual: cada elevação de potencial de \(X\) por \(\Delta(X)\) aumenta uma variável dual. As condições de folga complementar da programação linear dual exigem que, se um conjunto \(X\) teve seu potencial elevado (\(\Delta(X) > 0\)), então exatamente um arco da arborescência ótima cruza a fronteira de \(X\).

Se escolhêssemos um conjunto não-minimal --- isto é, contendo um subconjunto próprio \(Y \subset X\) também sem arcos justos entrando --- poderíamos elevar potenciais de forma redundante, criando folga nas restrições duais. Isso pode levar a uma solução que satisfaz as restrições mas não atinge o valor ótimo. Ao garantir minimalidade, construímos progressivamente uma solução dual ótima que guia a arborescência primal ótima através dos arcos justos.

\subsection{O conjunto \texorpdfstring{\(A_0\)}{A0} pode conter ``sujeira''}

Um aspecto importante é que o conjunto \(A_0\) de arcos justos pode conter \textbf{mais arcos do que necessário}. Uma \(r\)-arborescência tem exatamente \(n-1\) arcos, mas \(A_0\) pode conter ciclos (como o ciclo \((c,d)\leftrightarrow(d,c)\) no exemplo) e múltiplos arcos entrando em um mesmo vértice.

Essa ``sujeira'' não é um problema --- é uma característica do algoritmo. Durante a Fase~1, todos os arcos em \(A_0\) são garantidamente parte de \emph{alguma} arborescência ótima (não necessariamente a mesma), mas apenas um subconjunto de \(A_0\) formará a arborescência específica construída.

Quando \(A_0\) contém um ciclo, a contração (Figura~\ref{fig:frank-contracao-ciclo}) elimina essa ambiguidade. Ao contrair o ciclo \(\{c,d\}\) em um supervértice \(x_C\) e resolver recursivamente, o algoritmo escolhe quais arcos do ciclo incluir. Durante a expansão, apenas \(|C|-1\) arcos do ciclo são incluídos na arborescência, descartando o arco que entra no vértice por onde a arborescência alcança o ciclo.

Mesmo quando \(A_0\) não contém ciclos, a Fase~2 constrói a arborescência \emph{incrementalmente}, adicionando arcos um por vez, garantindo que cada vértice não-raiz receba exatamente um arco de entrada. Esse processo naturalmente seleciona um subconjunto de \(A_0\), descartando arcos extras. Assim, a Fase~1 não constrói diretamente a arborescência, mas sim um \emph{certificado de otimalidade} através dos arcos justos e potenciais duais, do qual a Fase~2 extrai uma arborescência ótima específica.\section{Descrição do algoritmo}
