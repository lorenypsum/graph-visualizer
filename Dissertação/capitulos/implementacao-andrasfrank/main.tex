...
\subsection*{Ideia Central do Algoritmo}

O algoritmo de András Frank, assim como o de Chu--Liu--Edmonds, baseia-se em uma escolha gulosa:
para cada vértice (ou conjunto de vértices), escolher o arco de menor custo que entra nele
e ajustar os custos subtraindo esse valor mínimo. A diferença fundamental está na organização:

\begin{itemize}
	\item \textbf{Chu--Liu--Edmonds:} processa vértice a vértice, subtraindo o menor custo
	      de arco entrando em cada vértice individualmente.
	\item \textbf{András Frank:} identifica \emph{subconjuntos minimais} através de componentes
	      fortemente conexas e processa todos simultaneamente, subtraindo o menor custo
	      de arco entrando em cada subconjunto.
\end{itemize}

\textbf{Funcionamento iterativo:}

No início do algoritmo, nenhum arco tem custo reduzido zero. Cada vértice não-raiz forma
seu próprio subconjunto minimal. Para cada vértice \(v \neq r\), encontramos o menor custo
\(\delta(\{v\})\) entre todos os arcos que entram em \(v\), e subtraímos esse valor de
todos esses arcos. Isso cria ao menos um arco de custo zero entrando em cada vértice.
...
\subsection*{Exemplo Introdutório}
...
\subsection*{Custos Reduzidos e Arcos Justos}

Durante a execução do algoritmo, os custos dos arcos são progressivamente reduzidos.
Inicialmente, o \textbf{custo reduzido} de cada arco coincide com seu custo original.
A cada iteração, subtraímos valores dos custos dos arcos que entram em determinados conjuntos.

Quando o custo reduzido de um arco atinge zero, dizemos que o arco é \textbf{justo}.
Formalmente, um arco \((u,v)\) é justo quando seu custo reduzido atual é zero: \(c(u,v) = 0\).
O algoritmo garante que a arborescência final será formada exclusivamente por arcos justos.

\textbf{Interpretação dual:} A operação de subtrair \(\delta(X)\) dos arcos entrando em
\(X\) pode ser interpretada através de \emph{potenciais duais} \(y(v)\) associados aos vértices.
Quando subtraímos \(\delta(X)\) do custo de cada arco \((u,v)\) com \(v \in X\),
isso equivale a aumentar \(y(v)\) em \(\delta(X)\) e trabalhar com custos reduzidos
\(c_y(u,v) = c_{\text{original}}(u,v) - y(v)\). Entretanto, a implementação opera
diretamente sobre os custos dos arcos, sem armazenar explicitamente os potenciais \(y(v)\).
A equivalência entre as duas visões é útil para provar a corretude do algoritmo.
...
\begin{tcolorbox}[
		...
		fonttitle=\bfseries,
		title={Arcos justos e subconjuntos minimais},
		sharp corners, boxrule=0.8pt
	]
	\textbf{Arco justo:} Um arco é \textbf{justo} quando seu custo reduzido atual é zero.

	\medskip
	\textbf{Subconjunto minimal:} Um conjunto \(X \subseteq V \setminus \{r\}\) é \textbf{minimal}
	se não possui arcos justos entrando nele e é uma componente fortemente conexa no grafo
	formado apenas pelos arcos justos.

	\medskip
	No início, como não há arcos justos, cada vértice não-raiz forma seu próprio subconjunto minimal.
	À medida que arcos justos são criados, se formarem ciclos, as componentes fortemente conexas
	definem os novos subconjuntos minimais.

	\medskip
	\textbf{Operação de redução:} Para um subconjunto minimal \(X\), calculamos
	\[
		\delta(X) := \min\{c(u,v) : u \notin X, v \in X\}
	\]
	e subtraímos \(\delta(X)\) de todos os arcos entrando em \(X\). Isso cria ao menos um novo arco justo.
\end{tcolorbox}

% Figura 27: Exemplo visual ilustrando subconjuntos e arcos justos
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[>=Stealth]
		% estilos
		\tikzset{
			...
			vtxX/.style={circle, draw=orange!80!black, fill=orange!15, thick, inner sep=1.2pt, minimum size=8mm},
			arcJust/.style={->, very thick, draw=blue!70!black},
			arcNormal/.style={->, semithick, draw=black!40},
			arcMin/.style={->, ultra thick, draw=green!60!black},
		...
		% vértices
		\node[vtx] (r) at (0,0) {$r$};
		\node[vtxX] (u) at (3,1.5) {$u$};
		\node[vtxX] (v) at (3,-1.5) {$v$};

		% arcos
		\draw[arcMin] (r) -- node[costMin, above left] {$\delta(X)=2$} (u);
		\draw[arcNormal] (r) -- node[cost, below left] {4} (v);
		\draw[arcNormal] (u) -- node[cost, right] {3} (v);

		% destaque do conjunto X
		\draw[orange!60!black, thick, dashed, rounded corners=8pt]
		(2.2, 2.3) rectangle (3.8, -2.3);
		\node[orange!80!black, font=\bfseries] at (4.8, 0) {$X$};

		% anotação
		\node[font=\small, text=black!80, align=center] at (1.5, -3) {
			Subconjunto minimal $X = \{u, v\}$\\
			sem arcos justos entrando.\\
			Menor custo: $\delta(X) = 2$.
		};
	\end{tikzpicture}
	\caption{Exemplo de subconjunto minimal \(X = \{u, v\}\) que não contém a raiz e não possui arcos justos entrando. O arco de menor custo entrando em \(X\) tem custo \(\delta(X) = 2\). Após subtrair esse valor de todos os arcos em \(\delta^-(X)\), o arco \((r,u)\) torna-se justo.}
	\label{fig:conjunto-minimal}
\end{figure}

\subsection*{Iteração por Subconjuntos Minimais}

O algoritmo opera iterativamente. A cada iteração:

\begin{enumerate}[label=\arabic*., leftmargin=*]
	\item Calculamos as componentes fortemente conexas no grafo formado pelos arcos justos atuais.
	\item Cada componente que não contém a raiz e não possui arcos justos entrando é um subconjunto minimal \(X\).
	\item Para cada subconjunto minimal \(X\), calculamos \(\delta(X) := \min\{c(u,v) : u \notin X, v \in X\}\).
	\item Subtraímos \(\delta(X)\) de todos os arcos entrando em \(X\).
	\item Os arcos que atingem custo zero são adicionados ao conjunto de arcos justos.
\end{enumerate}

\medskip
\textbf{Primeira iteração:} Inicialmente não há arcos justos, então cada vértice não-raiz
\(v\) é sua própria componente minimal. Calculamos
\[
	\delta(\{v\}) := \min\{\, c(u,v) : u \in V \,\}
\]
e subtraímos esse valor de todos os arcos entrando em \(v\). Isso cria ao menos um arco
justo para cada vértice.

\textbf{Iterações subsequentes:} Se os arcos justos formarem ciclos, as componentes fortemente
conexas agrupam vértices. Cada componente sem arcos justos entrando é um novo subconjunto
minimal, e repetimos o processo.

Vamos ilustrar com o dígrafo \(D\).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth]
		...
		% anotação: custos originais
		\node[font=\small, text=black!70] at (3, -3.5) {Custos originais};
	\end{tikzpicture}
	\caption{Dígrafo \(D\) com custos originais. Inicialmente, não há arcos justos (\(A_0 = \emptyset\)).}
\end{figure}

\textbf{Iteração 1:} Como \(A_0 = \emptyset\), cada vértice não-raiz forma seu próprio subconjunto minimal. Calculamos \(\delta(\{v\})\) para cada \(v \neq r\):
\begin{align*}
	\delta(\{a\}) & = \min\{c(r,a), c(b,a)\} = \min\{1, 4\} = 1, \\
	\delta(\{b\}) & = \min\{c(r,b), c(a,b)\} = \min\{2, 5\} = 2, \\
	\delta(\{c\}) & = \min\{c(r,c), c(a,c)\} = \min\{3, 1\} = 1, \\
	\delta(\{d\}) & = \min\{c(a,d), c(b,d)\} = \min\{3, 1\} = 1, \\
	\delta(\{e\}) & = \min\{c(b,e), c(c,e)\} = \min\{2, 5\} = 2.
\end{align*}

Subtraímos cada \(\delta(\{v\})\) de todos os arcos entrando no respectivo vértice \(v\).
Após essa redução, os arcos que atingem custo zero são:
\[
	\{(r,a), (r,b), (a,c), (b,d), (b,e)\}.
\]

Esses cinco arcos justos formam a arborescência de custo mínimo.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth]
		...
		% anotação
		\node[font=\small, text=black!70] at (3, -3.5) {Custos reduzidos após Iteração 1};
	\end{tikzpicture}
	\caption{Dígrafo após a primeira iteração. Arcos em \textcolor{blue}{azul} são justos (custo reduzido zero); arcos em cinza têm custo reduzido positivo. Neste exemplo, os cinco arcos justos formam uma \(r\)-arborescência.}
\end{figure}

Observe que cada vértice \(v \neq r\) possui agora ao menos um arco justo entrando nele.
Mais ainda: neste exemplo, os arcos justos \(\{(r,a), (r,b), (a,c), (b,d), (b,e)\}\) formam uma \(r\)-arborescência! Como não há ciclos, o algoritmo termina e retorna essa arborescência como solução ótima.
