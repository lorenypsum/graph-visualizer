...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=0.95]
		...
		\begin{scope}
			...
			\draw[arc] (a1) -- node[costG, above] {1} (c1);
		...
		\begin{scope}[xshift=9.5cm]
			...
			\draw[arc] (a2) -- node[costG, above] {1} (c2);
...
Essa operação vai sendo repetida para todos os conjuntos minimais, criando ao menos um arco de custo zero entrando em cada vértice. A Figura~\ref{fig:frank-todos-vertices-reduzidos} mostra o resultado dessa primeira iteração completa no exemplo da Figura~\ref{fig:frank-exemplo-inicial}.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.1]
		...
		% arcos justos (um entrando em cada vértice não-raiz)
		...
		\draw[tight] (d) to[bend left=15] node[costG, left] {0} (c);  % 1-1 = 0

		% arcos não justos
		\draw[loose] (r) .. controls (2,3.5) and (5,3.5) .. node[costG, above] {6} (c);  % 7-1 = 6
		\draw[loose] (a) -- node[costG, right] {3} (d);  % 4-1 = 3
		\draw[loose] (b) to[bend right=15] node[costG, left] {3} (a);  % 5-2 = 3
		\draw[loose] (d) -- node[costG, right] {1} (e);  % 3-2 = 1
		\draw[loose] (c) .. controls (7.5,2.5) and (7.5,-2.5) .. node[costG, right] {4} (e);  % 6-2 = 4
	\end{tikzpicture}
	\caption{Dígrafo após a primeira iteração completa, com reduções aplicadas a todos os vértices.
		Cada vértice não-raiz possui ao menos um arco justo (custo 0) entrando, destacados em azul.
		Note que os arcos \((r,a)\), \((r,b)\), \((a,c)\), \((c,d)\) e \((b,e)\) formam uma potencial
		arborescência, mas observe também que \((d,c)\) tem custo 0, formando um ciclo com \((c,d)\).}
	\label{fig:frank-todos-vertices-reduzidos}
\end{figure}
