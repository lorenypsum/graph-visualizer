...
Conduzimos 2000 experimentos em digrafos aleatórios construídos a partir de uma raiz \(r\) com número fixo de 2000 vértices, quantidade de arestas \(|A|\) variando no intervalo \([2001, 6000]\) com média de \(3989\) arcos, e pesos inteiros associados \(\in [1, 10]\). Para cada instância, executamos Chu--Liu/Edmonds e as duas fases de András Frank com duas versões v1 e v2 para a Fase~II, sendo a v2 uma versão otimizada utilizando fila de prioridade - \emph{heap}.
...
Quanto ao desempenho temporal, a Fase~I de Frank apresenta tempo mediano de \(4{,}01\)s (média \(4{,}28\)s), significativamente superior ao tempo mediano de Chu--Liu/Edmonds (\(0{,}13\)s, média \(0{,}25\)s). A Fase~I, responsável pela identificação de subconjuntos minimais via componentes fortemente conexas, domina o tempo total de execução do método de Frank. A Fase~II, em contrapartida, representa uma fração residual do processamento com mediana de \(0{,}42\)s para versão 1 do algoritmo e \(0{,}0083\)s para a versão 2.
...
\begin{figure}[H]
    ...
    \caption{Distribuição de tempos: Fase~I apresenta maior mediana (\(4{,}01\)s) e variabilidade que Chu--Liu/Edmonds (\(0{,}13\)s).}
...
A comparação entre as duas variantes da Fase~II revela ganho expressivo com o uso de \emph{heap}. A Figura~\ref{fig:speedup} apresenta essa comparação em dois painéis: à esquerda, o boxplot evidencia a diferença de magnitude entre os tempos de execução --- enquanto v1 (lista) apresenta mediana de \(0{,}42\)s, a versão v2 (heap) reduz para \(0{,}0083\)s. À direita, o histograma do fator de aceleração (\emph{speedup}) mostra distribuição com mediana de 50,94 vezes e média de 49,53 vezes. Esses resultados confirmam empiricamente a vantagem da estrutura de dados com complexidade \(O(\log n)\) versus \(O(n)\) por operação de seleção de mínimo.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_speedup_hist.png}
    \caption{Comparação de desempenho entre implementações da Fase~II. À esquerda, o gráfico de boxplot mostra a distribuição de tempo da v1 do algoritmo com mediana de \(0{,}42\)s enquanto v2 reduz para \(0{,}0083\)s. À direita, o histograma do fator de aceleração (\emph{speedup}) mostra mediana de 50,94 vezes (linha tracejada vermelha) e média de 49,53 vezes (linha pontilhada laranja).}
    \label{fig:speedup}
\end{figure}

As métricas estruturais do algoritmo de Chu--Liu/Edmonds são apresentadas na Figura~\ref{fig:contr-depth} em dois histogramas. O painel esquerdo mostra que o número de contrações é pequeno (mediana 2, média 6,82, máximo 406), com a grande maioria das instâncias requerendo menos de 20 contrações. O painel direito exibe a profundidade de recursão, que acompanha o número de contrações.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_contractions_depth.png}
    \caption{Métricas estruturais de Chu--Liu/Edmonds. À esquerda, o histograma vermelho mostra que o número de contrações (eixo horizontal) é concentrado em valores baixos --- a maioria das 2000 instâncias (eixo vertical) requer menos de 20 contrações, com mediana 2 (linha tracejada) e média \(6{,}82\) (linha pontilhada). À direita, o histograma azul da profundidade de recursão exibe padrão similar).}
