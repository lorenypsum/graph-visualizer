...
\begin{frame}{Camadas da Aplicação Web}
    \begin{columns}[c]
        \column{0.55\textwidth}
        \textbf{Estrutura em três camadas:}
        \begin{itemize}
            \item \textbf{Apresentação:} Interface construída em HTML5, estilizada com Tailwind CSS e dinamizada por JavaScript. Responsável pela interação com o usuário, entrada de dados e exibição dos resultados.
            \item \textbf{Processamento (PyScript):} Ponte entre interface web e bibliotecas científicas. Executa algoritmos em Python (NetworkX) diretamente no navegador, gerando visualizações estáticas com Matplotlib.
        ...
        \column{0.45\textwidth}
        \begin{center}
            \begin{tikzpicture}[node distance=1.3cm, box/.style={rectangle,draw,rounded corners,minimum width=2.8cm,minimum height=0.9cm,align=center}]
...
\begin{frame}{Camadas da Aplicação Web}
    \textbf{Estrutura em três camadas:}
    \begin{itemize}
        \item \textbf{Apresentação:} HTML5 + Tailwind + JS
        \item \textbf{Processamento:} PyScript + NetworkX
        \item \textbf{Dados/Persistência:} JSON (\texttt{node_link})
    \end{itemize}
