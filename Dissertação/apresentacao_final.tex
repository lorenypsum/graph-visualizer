  ...
  \begin{columns}[c]
    ...
    \begin{center}
      \begin{tikzpicture}[>=Stealth, scale=0.9,
        ...
        \node[font=\Large\bfseries, color=green!70] at (-2.4,1.8) {$R$};
        ...
        \node[font=\small\bfseries, color=magenta!70] at (-2.2,1.5) {$R'_1$};

        % Subconjunto R'_2 (elipse em volta de b, d, e)
        \draw[draw=magenta!60, line width=1pt, dashed]
        (1.05,0.0) ellipse (1.05cm and 1.55cm);
        \node[font=\small\bfseries, color=magenta!70] at (2.4,1.0) {$R'_2$};

        % Arcos de F entrando nos subconjuntos (de vértice externo x)
        \draw[arcF] (x) to[bend right=8]
        node[pos=0.5, above, font=\small\bfseries, text=magenta!90] {$f_1 \in F$} (a);
        \draw[arcF] (r) to[bend right=22]
        node[pos=0.4, right, font=\small\bfseries, text=magenta!90] {$f_2 \in F$} (b);
