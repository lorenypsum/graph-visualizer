...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{graphs,arrows.meta,positioning,fit,calc,backgrounds}
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, xshift=-0.cm, yshift=0.2cm] at (current page.north east) {
      \begin{tikzpicture}[scale=0.65, >=Stealth,
        ...
        \draw[edge] (a) to[bend left=12] node[midway, below, sloped, font=\scriptsize] {3} (b);
        \draw[edge] (b) to[bend left=12] node[midway, right, font=\scriptsize] {2} (c);
        \draw[edge] (c) to[bend left=14] node[midway, above, sloped, font=\scriptsize] {3} (a);
...
% Ciclo de referência para exemplos
\newcommand{\referencecycle}{
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, xshift=-0.cm, yshift=0.2cm] at (current page.north east) {
      \begin{tikzpicture}[scale=0.65, >=Stealth,
          v/.style={circle, draw, fill=orange!8, minimum size=6mm, font=\scriptsize},
          edge/.style={->, draw=gray!60},
          dashedge/.style={->, draw=gray!50, dashed}]
       
        % Background expandido para incluir todos os elementos (desenhado primeiro)
        \fill[white, rounded corners] (-1.7,-2.2) rectangle (5.0,2.2);

        % Vértices externos
        \node[v, fill=blue!10] (x1) at (-0.5,1.5) {$v_1$};
        %\node[v, fill=green!10] (x2) at (0.2,-1.8) {$y$};
        \node[v, fill=blue!10] (z1) at (4.2,0.8) {$v_2$};
        \node[v, fill=blue!10] (z2) at (4.2,-0.8) {$v_3$};

        % Vértices do ciclo
        \node[v] (a) at (1.5,0) {$a$};
        \node[v] (b) at (2.8,1.2) {$b$};
        \node[v] (c) at (2.8,-1.2) {$c$};
        
        % Arcos do ciclo
        \draw[edge] (a) to[bend left=12] node[midway, above, sloped, font=\scriptsize] {0} (b);
        \draw[edge] (b) to[bend left=12] node[midway, right, font=\scriptsize] {0} (c);
        \draw[edge] (c) to[bend left=14] node[midway, below, sloped, font=\scriptsize] {0} (a);
        
        % Arcos dashed entrando no ciclo
        \draw[dashedge] (x1) to[bend right=10] (a);
        \draw[dashedge] (x1) to[bend left=5] (b);
        %\draw[dashedge] (x1) to[bend right=20] (c);
        
        % Arcos dashed saindo do ciclo
        \draw[dashedge] (b) to[bend left=10] (z1);
        \draw[dashedge] (c) to[bend right=10] (z2);
        
        % Frame correspondente ao fundo
        \draw[primarypurple, very thick, rounded corners] (-1.7,-2.5) rectangle (5.0,2.5);
        % Título posicionado por cima de tudo
        \node[font=\scriptsize\bfseries, color=primarypurple] at (2.7,2.2) {Exemplo};
      \end{tikzpicture}
    };
  \end{tikzpicture}
}
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, xshift=-0.cm, yshift=0.2cm] at (current page.north east) {
      \begin{tikzpicture}[scale=0.65, >=Stealth,
        ...
        %\node[v] (r) at (0,0) {$r$};
        \node[v] (a) at (1.2,0) {$a$};
        \node[v] (b) at (2.5,1.2) {$b$};
        \node[v] (c) at (2.5,-1.2) {$c$};
        
        \draw[edge] (a) to[bend left=12] node[midway, above, sloped, font=\scriptsize] {0} (b);
        \draw[edge] (b) to[bend left=12] node[midway, right, font=\scriptsize] {0} (c);
        \draw[edge] (c) to[bend left=14] node[midway, below, sloped, font=\scriptsize] {0} (a);
        % \draw[edge] (r) to[bend left=8] node[midway, below, sloped, font=\scriptsize] {3} (a);
        % \draw[edge] (r) to[bend left=18] node[midway, above, sloped, font=\scriptsize] {4} (b);
        % \draw[edge] (r) to[bend right=10] node[midway, below, sloped, font=\scriptsize] {5} (c);
        ...
        \draw[primarypurple, very thick, rounded corners] (-1.0,-2.2) rectangle (4.2,2.8);
        % Título posicionado por cima de tudo
        \node[font=\scriptsize\bfseries, color=primarypurple] at (3.0,2.5) {Ciclo em $x_c$};
      \end{tikzpicture}
    };
  \end{tikzpicture}
}
...
\begin{frame}{Encontrando uma $r$-Arborescência Geradora Mínima}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, yshift=1cm,
      ...
      \draw[expensive] (a) to[bend left=12] node[midway, below, sloped] {\scriptsize 3} (b);
      \draw[expensive] (b) to[bend left=12] node[midway, above, sloped] {\scriptsize 2} (c);
      \draw[expensive] (c) to[bend left=14] node[midway, above, sloped] {\scriptsize 3} (a);
...
\begin{frame}{Motivação}
  ...
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.6cm, yshift=1.5cm,
      ...
      \draw[cheap] (a) to[bend left=12] node[midway, below, sloped] {\scriptsize 3} (b);
      \draw[cheap] (b) to[bend left=12] node[midway, below, sloped] {\scriptsize 2} (c);
      \draw[expensive] (c) to[bend left=14] node[midway, above, sloped] {\scriptsize 3} (a);
...
\begin{frame}{Aqui tem um problema...}
  ...
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.6cm, yshift=1.5cm,
      ...
      \draw[cheap] (a) to[bend left=12] node[midway, below, sloped] {\scriptsize 3} (b);
      \draw[cheap] (b) to[bend left=12] node[midway, below, sloped] {\scriptsize 2} (c);
      \draw[cheap] (c) to[bend left=14] node[midway, above, sloped] {\scriptsize 3} (a);
...
\begin{frame}{Passo 1: Redução de Pesos}
  ...
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.0cm, yshift=-1.5cm,
      ...
      \draw[cycle] (a) to[bend left=12] node[midway, below, sloped] {\scriptsize 0} (b);
      \draw[cycle] (b) to[bend left=12] node[midway, below, sloped] {\scriptsize 0} (c);
      \draw[cycle] (c) to[bend left=14] node[midway, above, sloped] {\scriptsize 0} (a);
...
\begin{frame}{Passo 6: Reexpansão da Solução}
  ...
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.6cm, yshift=1.5cm,
      ...
      \draw[selected] (a) to[bend left=12] node[midway, below, sloped] {\scriptsize 3} (b);
      \draw[selected] (b) to[bend left=12] node[midway, below, sloped] {\scriptsize 2} (c);
    \end{tikzpicture}
...
\begin{frame}{Fase I: $r$-conjunto minimal}
  ...
  \begin{columns}[c]
    \column{0.48\textwidth}
    \begin{block}{$r$-conjunto minimal não coberto por $F \subseteq A$}
      Um $r$-conjunto $R$ é \textbf{minimal não coberto por $F \subseteq A$} se:
      \begin{itemize}
    ...
    \begin{center}
      \begin{tikzpicture}[>=Stealth, scale=0.9,
          v/.style={circle, draw=black!70, fill=white, minimum size=7mm, font=\small\bfseries},
          arc/.style={->, thick, draw=green!70},
          arcF/.style={->, line width=1.2pt, draw=magenta!80},
          arcOut/.style={->, thick, draw=green!70, dashed}]

        % Vértice raiz r (fora de R, destacado)
        \node[v, fill=orange!15, draw=orange!70, text=orange!70] (r) at (0,3.0) {$r$};

        % Vértice externo x (fora de R)
        \node[v, fill=blue!10, draw=blue!70, text=blue!70,] (x) at (-3.2,0.5) {$x$};

        % R-conjunto principal (elipse tracejada verde)
        \draw[draw=green!70, fill=green!3, line width=1.0pt]
        (0,0) ellipse (2.5cm and 2.3cm);
        \node[font=\Large\bfseries, color=green!70] at (-1.4,2.3) {$R$};

        % Vértices internos
        \node[v, fill=magenta!10, draw=magenta!70, text=magenta!80] (a) at (-1.3,0.9) {$a$};
        \node[v, fill=magenta!10, draw=magenta!70, text=magenta!80] (b) at (1.3,0.9) {$b$};
        \node[v, fill=cyan!10, draw=cyan!70, text=cyan!70] (c) at (-1.3,-0.9) {$c$};
        \node[v, fill=cyan!10, draw=cyan!70, text=cyan!70] (d) at (0.6,-0.6) {$d$};
        \node[v, fill=cyan!10, draw=cyan!70, text=cyan!70] (e) at (1.8,-0.4) {$e$};
        % Subconjunto R'_1 (círculo em volta de 'a')
        \draw[draw=magenta!60, line width=1pt, dashed]
        (-1.3,0.9) circle (0.6cm);
        \node[font=\small\bfseries, color=magenta!70] at (-0.5,1.5) {$R'_1$};

        % Subconjunto R'_2 (elipse em volta de b, d, e)
        \draw[draw=magenta!60, line width=1pt, dashed]
        (1.2,-0.2) ellipse (1.05cm and 1.55cm);
        \node[font=\small\bfseries, color=magenta!70] at (2.4,0.5) {$R'_2$};

        % Arcos de F entrando nos subconjuntos (de vértice externo x)
        \draw[arcF] (x) to[bend right=8]
        node[pos=0.5, below, font=\small\bfseries, text=magenta!90] {$f_1 \notin F$} (a);
        \draw[arcF] (r) to[bend right=22]
        node[pos=0.4, right, font=\small\bfseries, text=magenta!90] {$f_2 \notin F$} (b);

        % % Arcos NÃO de F tentando entrar em R (mas não conseguem)
        % \draw[arcOut] (-3.5,2.0) to[bend right=20]
        % node[pos=0.6, above, font=\tiny, sloped, text=red!80] {$\notin F$} (-2.5,1.2);
        % \draw[arcOut] (3.0,2.2) to[bend left=25]
        % node[pos=0.5, above, font=\tiny, sloped, text=red!80] {$\notin F$} (1.8,0.8);

        % Indicar que esses arcos são bloqueados (não entram em R)
        % \draw[red!70, line width=1.5pt] (-2.7,1.4) -- (-2.4,1.1);
        % \draw[red!70, line width=1.5pt] (-2.4,1.4) -- (-2.7,1.1);
        % \draw[red!70, line width=1.5pt] (2.0,1.0) -- (2.3,0.7);
        % \draw[red!70, line width=1.5pt] (2.3,1.0) -- (2.0,0.7);

        % Arcos internos a R (verde)
        \draw[arc] (a) to[bend right=12] (c);
        \draw[arc] (b) to[bend left=18] (d);
        \draw[arc] (d) to[bend right=12] (e);
        \draw[arc] (c) to[bend right=18] (d);

        % Legenda/Anotações na parte inferior
        \node[font=\scriptsize, color=black!70, align=left,
          draw=gray!40, fill=white, rounded corners, inner sep=4pt]
        at (0,-3.0) {
          $R$ é r-conjunto: $r \notin R \neq \emptyset$\\
        };
      \end{tikzpicture}
...
% SÓ COMENTAR SE QUISER MOSTRAR AS CONDIÇÕES DE OTIMALIDADE
...
\begin{frame}{Fase II: Iteração 1}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
        ...
        arc/.style={->, thick, draw=gray!70},
        selected/.style={->, very thick, draw=blue!70},
        pselected/.style={->, very thick, draw=orange!90},
        zero/.style={->, very thick, draw=gray!70}]
      \node[vs] (r) {\small $r$};
      \node[v] (b) [right=1.8cm of r] {\small $b$};
      \node[v] (a) [above=1.2cm of b] {\small $a$};
      ...
      % \draw[arc] (r) to[bend left=13] node[midway, above, sloped] {\scriptsize 1} (a);
      \draw[pselected] (r) to node[midway, above, sloped] {\scriptsize \textcolor{orange!90}{$f_7$}} (b);
      % \draw[arc] (r) to[bend right=13] node[midway, below, sloped] {\scriptsize 2} (c);
      % \draw[arc] (a) to node[midway, above, sloped] {\scriptsize 3} (b);
...
\begin{frame}{Fase II: Iteração 2}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \draw[pselected] (b) to node[midway, above, sloped] {\scriptsize \textcolor{orange!90}{$f_1$}} (c);
      % \draw[arc] (b) to node[midway, above, sloped] {\scriptsize 4} (d);
...
\begin{frame}{Fase II: Iteração 3}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (b) [right=1.8cm of r] {\small $b$};
      \node[v] (a) [above=1.2cm of b] {\small $a$};
      \node[vs] (c) [below=1.2cm of b] {\small $c$};
      ...
      % \draw[arc] (c) to node[midway, above, sloped] {\scriptsize 4} (g);
      \draw[pselected] (c) to[bend right=70] node[midway, below, sloped] {\scriptsize \textcolor{orange!90}{$f_5$}} (f);
...
\begin{frame}{Fase II: Iteração 4}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (f) [right=1.2cm of d] {\small $f$};
      ...
      \draw[pselected] (f) to node[midway, below, sloped] {\scriptsize \textcolor{orange!90}{$f_4$}} (d);
    \end{tikzpicture}
...
\begin{frame}{Fase II: Iteração 5}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (d) [right=1.2cm of b] {\small $d$};
      ...
      \draw[pselected] (d) to[bend left=14] node[midway, above, sloped] {\scriptsize \textcolor{orange!90}{$f_6$}} (e);
      \draw[pselected] (d) to node[midway, below, sloped] {\scriptsize \textcolor{orange!90}{$f_3$}} (g);
...
\begin{frame}{Fase II: Iteração 6}
  \begin{columns}[c]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (g) [below right=1.4cm and 1.5cm of b] {\small $g$};
...
\begin{frame}{Fase II: Iteração 7}
  \begin{columns}[c]
    \column{0.48\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (e) [above right=1.2cm and 0.6cm of d] {\small $e$};
      ...
      \draw[pselected] (e) to node[midway, above, sloped] {\scriptsize \textcolor{orange!90}{$f_2$}} (a);
      % \draw[arc] (e) to[bend left=14] node[midway, above, sloped] {\scriptsize 3} (f);
      % \draw[arc] (e) to[bend left=14] node[midway, below, sloped] {\scriptsize 3} (d);
...
\begin{frame}{Fase II: Resultado Final}
  \begin{columns}[c]
    \column{0.48\textwidth}
    \textbf{Estado:} $U = \{r,b,c,f,d,g,e,a\}$

    \vspace{0.2cm}
    \textbf{Ação:}
    \begin{itemize}
      \item Devolver $J$
      \item $U$ contém todos os vértices
    \end{itemize}
    \vspace{0.2cm}
    \textbf{Resultado:}
    \begin{itemize}
    ...
    \begin{tikzpicture}[>=Stealth, node distance=2.0cm, scale=0.9, transform shape,
      ...
      \node[vs] (r) {\small $r$};
      \node[vs] (b) [right=1.8cm of r] {\small $b$};
      \node[vs] (a) [above=1.2cm of b] {\small $a$};
      \node[vs] (c) [below=1.2cm of b] {\small $c$};
    ...
    {\small A arborescência geradora mínima é destacada em azul.}
  \end{columns}
\end{frame}
...
\begin{frame}{Comparação de Desempenho}
  \textbf{Implementação:} Python 3.13

  \textbf{Bibliotecas:} NetworkX, Matplotlib
...
\begin{frame}{Escalonamento e Consumo de Memória}
  ...
  \begin{itemize}
    \item Tempo cresce linearmente com o número de arcos
...
\begin{frame}{Resumidamente...}
      \begin{itemize}
        \item \textbf{Corretude verificada:} pesos idênticos em todas as instâncias
        \item \textbf{Chu-Liu-Edmonds}: mais rápido, mas cabe discutir implementação
        \item \textbf{Fase I de Frank}: gargalo computacional
        \item \textbf{Otimização heap}: fundamental na Fase II, retira necessidade de verificação de arboresencência na Fase I
        \item \textbf{Comportamento prático}: curioso
              \begin{itemize}
                \item Contrações: mediana 2 (limite $O(n)$)
                \item Uso modesto de memória: 11,5 MB
              \end{itemize}
      \end{itemize}
\end{frame}
...
\begin{frame}{Arquitetura da Aplicação}
  \begin{columns}[c]
    ...
    \begin{itemize}
      ...
      \item \textbf{Dados e Persistência:} Utiliza JSON (\texttt{node\_link}) para seriar grafos, armazenar pesos e permitir exportação/importação entre módulos.
    \end{itemize}
    \column{0.45\textwidth}
    \begin{center}
...
\begin{frame}{Contribuições do Trabalho}
  \begin{enumerate}
    ...
    \item \textbf{Análise experimental}
          \begin{itemize}
  ...
  \begin{columns}[c]
    ...
    \begin{minipage}{0.95\columnwidth}
      \begin{block}{Extensões Possíveis}
        \begin{itemize}
          \item Melhoria da versão recursiva do algoritmo de Chu-Liu-Edmonds usando pilha
          \item Melhoria da fase I do algoritmo de András Frank utilizando contração de componentes fonte
          \item Implementação de algoritmos para resolver o problema inverso da r-arborescência geradora mínima
        \end{itemize}
      \end{block}
    \end{minipage}
    \column{0.52\textwidth}
    \begin{minipage}{0.95\columnwidth}
      \begin{block}{Melhorias na Aplicação}
        \begin{itemize}
          \item Modo de edição visual de grafos no passo a passo
...
\begin{frame}[plain]
  ...
  {\normalsize \textcolor{textlightgray}{\texttt{https://lorenypsum.github.io/graph-visualizer/pages/home.html}}}
