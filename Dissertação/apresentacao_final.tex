  ...
  \begin{columns}[c]
    ...
    \begin{center}
      \begin{tikzpicture}[>=Stealth, scale=0.8,
          v/.style={circle, draw, fill=blue!8, minimum size=6mm, font=\small},
          arc/.style={->, thick, draw=gray!70},
          coveredge/.style={->, very thick, draw=green!70},
          nocoveredge/.style={->, thick, draw=red!70, dashed}]

        % R-conjunto (fundo - desenhar primeiro)
        \draw[draw=orange!70, very thick, dashed, rounded corners, fill=orange!10]
        (-2,-2.2) rectangle (2,0.7);
        \node[font=\small\bfseries, color=orange!90] at (-1.6,0.4) {$R$};

        % Vértices
        \node[v,fill=red!20] (r) at (0,2) {$r$};
        \node[v] (a) at (-1,0) {$a$};
        \node[v] (b) at (1,0) {$b$};
        \node[v] (c) at (0,-1.5) {$c$};

        % Subconjuntos de R
        \draw[blue!60, thick, dashed] (a) circle (0.5cm);
        \node[below left=-2pt, font=\scriptsize, color=blue!70] at (a) {$R'_1$};

        \draw[blue!60, thick, dashed] (b) circle (0.5cm);
        \node[below right=-2pt, font=\scriptsize, color=blue!70] at (b) {$R'_2$};

        % Arcos de F entrando em subconjuntos (cobertos)
        \draw[coveredge] (r) to[bend left=15] node[midway, left, font=\scriptsize] {$f_1 \in F$} (a);
        \draw[coveredge] (r) to[bend right=15] node[midway, right, font=\scriptsize] {$f_2 \in F$} (b);

        % Arco NÃO entrando em R (não coberto)
        \draw[nocoveredge] (-2.2,1) to node[midway, above, font=\scriptsize, sloped] {$\notin F$} (a);
        \draw[nocoveredge] (2.2,1) to node[midway, above, font=\scriptsize, sloped] {$\notin F$} (b);

        % Arcos internos a R
        \draw[arc] (a) to[bend left=10] (c);
        \draw[arc] (b) to[bend right=10] (c);
      \end{tikzpicture}
    \end{center}
