  ...
  \begin{columns}[c]
    ...
    \begin{center}
      \begin{tikzpicture}[>=Stealth, scale=0.9,
        ...
        \node[v, fill=red!15, draw=red!70] (r) at (0,3.0) {$r$};

        % Vértice externo x (fora de R)
        \node[v, fill=blue!10, draw=blue!70] (x) at (-3.2,0.5) {$x$};

        % R-conjunto principal (elipse tracejada verde)
        \draw[draw=green!70, line width=1.5pt, dashed]
        (0,-0.2) ellipse (2.9cm and 2.4cm);
        \node[font=\Large\bfseries, color=green!70] at (-1.4,2.3) {$R$};

        % Vértices internos
        \node[v, text=magenta!80] (a) at (-1.3,0.9) {$a$};
        \node[v, text=magenta!80] (b) at (1.3,0.9) {$b$};
        \node[v, text=cyan!70] (c) at (-1.3,-0.9) {$c$};
        \node[v, text=cyan!70] (d) at (0.6,-0.6) {$d$};
        \node[v, text=cyan!70] (e) at (1.3,-1.3) {$e$};
        ...
        \draw[draw=magenta!60, line width=1pt, dashed]
        (1.05,0.0) ellipse (1.05cm and 1.55cm);
        ...
        node[pos=0.5, above, font=\small\bfseries, text=magenta!90] {$f_1 \notin F$} (a);
        ...
        at (0,-3.0) {
          \textbf{(i)} $R$ é r-conjunto: $r \notin R$\\
          \textbf{(ii)} Minimal: $F$ entra em todo $R' \subset R$\\
          \textbf{(iii)} Não coberto: $F \cap \delta^-(R) = \emptyset$
        };
      \end{tikzpicture}
    \end{center}    \vspace{0.1cm}
    {\small \textbf{Ilustração:} $R = \{a,b,c\}$ é minimal não coberto: $F$ não entra em $R$, mas entra em todos subconjuntos próprios $R'_1=\{a\}$, $R'_2=\{b\}$.}
  \end{columns}
\end{frame}
...
\begin{frame}{Fase II: Resultado Final}
  \begin{columns}[c]
    ...
    {\small A árvore geradora mínima é destacada em azul.}
  \end{columns}
\end{frame}
