...
\begin{enumerate}\setlength{\itemsep}{4pt}
    ...
    \item (Condições dual–primal) Existem pesos $y(X)\ge 0$ para cada $\emptyset\neq X\subseteq V\setminus\{r\}$ tais que:
    \begin{enumerate}\setlength{\itemsep}{2pt}
...
\[
 	ext{custo}(B) = \sum_{a\in B} c(a) = \sum_{a\in B} \Big( c'(a) + \sum_{X: a\text{ entra }X} y(X) \Big).
\]
Trocando a ordem da soma:
\[
 	ext{custo}(B) = \sum_{a\in B} c'(a) + \sum_X y(X)\,\big| \{a\in B: a \text{ entra } X\}\big|.
\]
...
\]
logo $\text{custo}(T) \le \text{custo}(B)$ para qualquer $B$; $T$ é ótimo.
}}
\endgroup
\end{center}
