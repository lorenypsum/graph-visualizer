...
\begingroup
\setlength{\fboxsep}{15pt}% padding interno da faixa azul (título/enunciado)
\colorbox{orange!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Lema: mínimo de arcos para conexidade forte}\\[1.0em]
...
\paragraph{}
As arborescências são a principal estrutura que exploraremos ao longo desta dissertação, especialmente a r-arborescência de custo mínimo e r-arborescência inversa de custo mínimo, abordaresmo o problema de encontrá-las eficientemente em dígrafos com custos associados aos arcos.

\paragraph{O problema de busca por arborescências de custo mínimo:}

\paragraph{Noções aprofundadas em arborescências}

\paragraph{Grau e contagem de arcos:}
Se $T$ é uma arborescência (out-arborescência) enraizada em $r$ sobre $n$ vértices, então $|A_T|=n-1$, todo $v\ne r$ tem grau de entrada $d_T^-(v)=1$ e $d_T^-(r)=0$. Analogamente, numa in-arborescência cada $v\ne r$ tem grau de saída $d_T^+(v)=1$ e $d_T^+(r)=0$. Essas condições são necessárias e suficientes para caracterizar a estrutura de árvore dirigida.

\paragraph{Condição de existência via cortes (Fulkerson):}
Seja $D=(V,A)$ um dígrafo e $r\in V$. Existe uma out-arborescência enraizada em $r$ se, e somente se, para todo subconjunto não vazio $X\subseteq V\setminus\{r\}$ vale $\delta^-(X)\ne\emptyset$ (isto é, há ao menos um arco entrando em $X$ a partir de $V\setminus X$). Intuição: qualquer arborescência precisa “entrar” em cada $X$ pela primeira vez; reciprocamente, se todo corte voltado para dentro é cruzado, podemos construir uma arborescência por expansão guiada por BFS/DFS. Ver, e.g., \cite{schrijver2003comb}.

\paragraph{Teorema das arborescências disjuntas de Edmonds:}
Seja $D=(V,A)$ e $r\in V$. Para um inteiro $k\ge 1$, existem $k$ out-arborescências enraizadas em $r$ par de arcos disjuntas se, e somente se, para todo $X\subseteq V\setminus\{r\}$ tem-se $|\delta^-(X)|\ge k$. Essa condição de cortes é necessária (cada arborescência deve “entrar” em $X$ por um arco distinto) e suficiente (prova por matroides ou via teoremas de enxerto). Referências clássicas: Edmonds (1973), e \cite{schrijver2003comb}.

\paragraph{Nota sobre custo mínimo e dualidade (visão de alto nível):}
No problema de r-arborescência de custo mínimo, algoritmos como Chu--Liu/Edmonds e o primal--dual de Frank operam com \emph{custos reduzidos}: subtraem de cada componente-fonte o menor custo das arestas que a “alimentam”, fazendo surgir arestas de custo reduzido zero e circuitos/condensações que guiam contrações. No ótimo, há uma escolha de potenciais (variáveis duais) tal que todas as arestas escolhidas têm custo reduzido zero e cada conjunto “apertado” recebe exatamente uma aresta, certificando otimalidade. Ver \cite{frank2014connections, schrijver2003comb}.
Esses dois conceitos nos levam à ideia de ramificações geradoras, que são arborescências que abrangem todos os vértices do dígrafo original.
