...
\begingroup
\setlength{\fboxsep}{15pt}% padding interno da faixa azul (título/enunciado)
\colorbox{blue!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Teorema: condição de existência de arborescência via cortes - Fulkerson}\\[1.0em]
{\color{black!70!black}
...
\begingroup
\setlength{\fboxsep}{15pt}% padding interno da faixa azul (título/enunciado)
\colorbox{blue!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Teorema de $k$ arborescências arcodisjuntas - Edmonds}\\[1.0em]
{\color{black!70!black}
...
\begin{center}
\begingroup
\setlength{\fboxsep}{15pt}% padding externo
\fcolorbox{blue!15}{blue!3}{%
\parbox{\dimexpr\textwidth-2\fboxsep\relax}{%
\noindent
\begingroup
\setlength{\fboxsep}{15pt}% padding interno título
\colorbox{blue!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Teorema de Optimalidade de Fulkerson (r-arborescência de custo mínimo)}\\[1.0em]
{\color{black!70!black}
...
\paragraph{}
A ideia de “passo a passo” tem uma motivação filosófica: desde uma receita de cozinha ao método de Euclides para o máximo divisor comum, buscamos procedimentos finitos e reprodutíveis que transformem intenções em resultados. Um algoritmo torna explícitas as decisões que tomaríamos “de cabeça”: torna o raciocínio executável, auditável e menos sujeito a ambiguidades.

\emph{Definição intuitiva.} Um algoritmo é uma sequência finita de instruções claras que, dada uma entrada, conduz mecanicamente a uma saída desejada. Em termos práticos, cada passo deve ser:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item não ambíguo (instruições interpretáveis de uma única maneira);
    \item efetivo (cada operação é realizável em tempo finito);
    \item geral (funciona para toda instância válida do problema).
\end{itemize}

\emph{Comentário formal.} A ciência da computação formaliza esse conceito por meio de modelos como máquinas de Turing, funções recursivas, RAM, entre outros. Essas formalizações são equivalentes quanto ao que é computável (Tese de Church–Turing) e permitem discutir com rigor correção e complexidade (tempo e memória). Para nós, isso se traduz em duas questões centrais: (i) o procedimento sempre devolve uma arborescência válida? (ii) qual o seu custo assintótico?

No nosso contexto, um algoritmo para arborescências recebe um dígrafo com custos e uma raiz $r$ e devolve uma r-arborescência de custo mínimo. Nas próximas subseções veremos duas abordagens clássicas: Chu–Liu/Edmonds (escolhas locais com contração de ciclos) e um método primal–dual na linha de András Frank.
