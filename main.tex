...
\paragraph{}
O \emph{teorema fluxo–corte} afirma que, em dígrafos com valores não negativos, o valor de um fluxo máximo de \(s\) para \(t\) é igual ao valor de um min-corte \(s\text{--}t\) (isto é, \(\min\, c(\delta^+(X))\)). Essa é uma instância canônica de resultado \emph{min–max}, conectando um problema de \emph{empacotar} caminhos com um problema de \emph{cobrir} arcos.
