...
\begin{center}
\begingroup
\setlength{\fboxsep}{10pt}% padding externo da moldura
\fcolorbox{blue!15}{blue!3}{%
\parbox{\dimexpr\textwidth-2\fboxsep\relax}{%
\noindent
\begingroup
\setlength{\fboxsep}{15pt}% padding interno da faixa azul (título/enunciado)
\colorbox{blue!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Teorema: Fluxo–Corte Máximo = Mínimo Corte.}\\[1.0em]
...
\paragraph{Circuitos e Teoremas Relacionados}

\paragraph{}
\begin{center}
\begingroup
\setlength{\fboxsep}{15pt}% padding externo da moldura
\fcolorbox{orange!15}{orange!3}{%
\parbox{\dimexpr\textwidth-2\fboxsep\relax}{%
\noindent
\begingroup
\setlength{\fboxsep}{15pt}% padding interno da faixa azul (título/enunciado)
\colorbox{orange!15}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Lema: Princípio da casa dos pombos para circuitos.}\\[1.0em]
{\color{black!70!black}Se \(D=(V,A)\) é um dígrafo finito em que todo vértice tem grau de saída ao menos 1, isto é, \(d^+(v)\ge 1\) para todo \(v\in V\), então \(D\) contém pelo menos um circuito dirigido. (De forma equivalente, a afirmação vale trocando ``saída'' por ``entrada''.)}}}
\endgroup
...
\paragraph{}
Esse princípio ajuda a entender o comportamento básico dos métodos que os algoritmos empregam para encontrar as arborescências de custo mínimo. Vamos elaborar melhor esse ponto na seção de arborescências.

% Se, para cada vértice \(v\neq r\), escolhemos a aresta de menor custo que entra em \(v\), o subgrafo obtido fica com grau de entrada igual a 1 em todos os \(v\neq r\). Pelo lema, enquanto esse subgrafo ainda não for uma arborescência, ele necessariamente contém um circuito. É exatamente isso que o algoritmo de Chu--Liu/Edmonds explora: detectar o circuito inevitável, contraí-lo a um único vértice e repetir a seleção sob custos reduzidos. Quando não houver mais circuitos, as arestas escolhidas formam uma arborescência ótima enraizada em \(r\).
...
\paragraph{Componentes-fortemente conexas e empacotamento de circuitos}

\paragraph{Empacotamento de vértices}
\paragraph{}Um \textbf{empacotamento de vértices} em um dígrafo é um conjunto de vértices \(S \subseteq V\) tal que não existem arcos entre quaisquer dois vértices em \(S\). Formalmente, um empacotamento de vértices \(S\) satisfaz a seguinte propriedade:
\begin{itemize}
    \item Para quaisquer \(u, v \in S\), não existe arco \((u, v) \in A\) ou \((v, u) \in A\).
\end{itemize}
O objetivo de encontrar um empacotamento de vértices é maximizar o tamanho do conjunto \(S\), ou seja, encontrar o maior número possível de vértices que podem ser incluídos em \(S\) sem violar a propriedade acima.

\paragraph{}
A noção de componente-forte conversa diretamente com o conceito de \emph{empacotamento de circuitos}. Pois, em uma componente fortemente conexa, qualquer vértice pode ser alcançado a partir de qualquer outro vértice, o que implica que é possível encontrar circuitos que percorrem todos os vértices da componente. Assim, o empacotamento de circuitos em um dígrafo pode ser visto como a seleção de circuitos dentro de suas componentes fortemente conexas, garantindo que cada vértice seja visitado por pelo menos um circuito.
...
\paragraph{}
A noção de componente-forte conversa diretamente com o conceito de \emph{arborescências}. Pois, em uma arborescência, todos os vértices são alcançáveis a partir da raiz, o que implica que a arborescência é fortemente conexa. Além disso, a raiz de uma arborescência pode ser vista como uma componente-fonte, já que não há arcos direcionados entrando nela de outras componentes.
...
\paragraph{Propriedades de arborescências}
...
\paragraph{}
As arborescências são a principal estrutura que exploraremos ao longo desta dissertação, especialmente a r-arborescência de custo mínimo e r-arborescência inversa de custo mínimo, que é o foco do problema do qual tratamos aqui.
Vamos aprofundar mais nessas estruturas apresentando propriedades importantes sobre elas, teoremas relevantes e algoritmos que nascem dessas discussões, mas antes precisamos abordar algumas propriedades matemáticas em dígrafos.
...
% \paragraph{}
% No algoritmo primal--dual de András Frank, a mesma ideia aparece no “grafo de zeros”. Ao subtrair, para cada componente-fonte \(X\), o menor custo das arestas que entram em \(X\), garantimos que pelo menos uma delas zere. Quando todos os vértices (exceto \(r\)) têm alguma aresta entrante de custo reduzido zero, surgem circuitos e componentes fortemente conexas nesse grafo — exatamente as estruturas que a condensação/contração explora. Essa dinâmica explica por que a solução pode ser construída apenas com arestas de custo reduzido zero e por que a condição dual “exatamente uma aresta entra em cada conjunto apertado \(X\) com \(z(X)>0\)” certifica a otimalidade.