...
\begin{itemize}
    ...
    \item \textbf{Fluxo máximo}: Em um grafo direcionado com capacidades nas arestas, encontrar o fluxo máximo que pode ser enviado de uma fonte a um sumidouro sem exceder as capacidades.
\end{itemize}
...
\paragraph{}
Assim como falamos na sessão anterior, os dígrafos também possuem estruturas interessantes, como caminhos direcionados, ciclos direcionados, arborescências e componentes fortemente conexas. Essas estruturas são fundamentais para entender a topologia e as propriedades dos dígrafos, e muitas vezes são o foco de problemas de otimização.

\paragraph{Caminhos direcionados ou trilhas}
\paragraph{}Um \textbf{caminho direcionado} em um dígrafo é uma sequência de vértices conectados por arcos que respeitam a direção. Formalmente, um caminho direcionado \(P\) de comprimento \(k \geq 1\) é uma sequência de vértices \(P = (v_1, v_2, \ldots, v_{k+1})\) tal que cada par consecutivo \((v_i, v_{i+1})\) é um arco em \(A\). O comprimento do caminho é o número de arcos que ele contém, que é \(k\). 
\begin{center}
\begin{tikzpicture}[>=Stealth]
    % estilos
    \tikzset{v/.style={circle, draw, fill=blue!10, minimum size=7mm, inner sep=0pt}, a/.style={->, semithick, draw=gray!60}, p/.style={very thick, draw=blue!
60}}    
    ...
    \node[align=left, anchor=west] at (6.2,0.6) {$P=(v_1,v_2,v_3,v_4)$\\ $|P|=3$};
\end{tikzpicture}
\captionof{figure}{Caminho direcionado em dígrafo: o caminho $P=(v_1,v_2,v_3,v_4)$ está destacado em azul. Seu comprimento é o número de arcos percorridos, $|P|=3$.}
\label{fig:caminho-direcionado}
\end{center}

\paragraph{Ciclos direcionados ou circuitos}
\paragraph{}Um \textbf{ciclo direcionado} é um caminho direcionado que começa e termina no mesmo vértice, ou seja, \(v_1 = v_{k+1}\). Formalmente, um ciclo direcionado \(C\) é uma sequência de vértices \(C = (v_1, v_2, \ldots, v_k, v_1)\) tal que cada par consecutivo \((v_i, v_{i+1})\) é um arco em \(A\) e \(k \geq 2\). O comprimento do ciclo é o número de arcos que ele contém, que é \(k\).

\begin{center}
\begin{tikzpicture}[>=Stealth]
    % estilos
    \tikzset{v/.style={circle, draw, fill=blue!10, minimum size=7mm, inner sep=0pt}, a/.style={->, semithick, draw=gray!60}, c/.style={very thick, draw=blue!60}};
    % vértices do ciclo (dispostos aproximadamente em círculo)
    \node[v, label=below:$v_1$] (v1) at (0,0) {};
    \node[v, label=right:$v_2$] (v2) at (2.0,0.8) {};
    \node[v, label=above:$v_3$] (v3) at (1.2,2.1) {};
...
\end{tikzpicture}
\captionof{figure}{Arborescência: dígrafo conexo e acíclico com raiz $r$. No exemplo, $|V_T|=7$ e $|A_T|=6$, satisfazendo $|A_T|=|V_T|-1$. Não há ciclos direcionados e existe um único caminho direcionado da raiz para quaisquer outros vértices.}
