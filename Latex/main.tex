...
\paragraph{}
As ideias abstratas, como grafos, arborescências e algoritmos de otimização, são essenciais para modelar e resolver problemas do mundo real. No entanto, a natureza intangível desses conceitos pode representar um desafio significativo para estudantes e profissionais que buscam compreendê-los profundamente.
...
\subsection{Sobre o ensino de matemática no ensino superior}
\paragraph{}
...
Diretrizes de aprendizagem multimídia \cite{mayer2009multimedia} e a teoria da codificação dupla \cite{paivio1990} sugerem práticas concretas para reduzir a carga extrínseca e favorecer a pertinente:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Coerência e sinalização}: remover detalhes irrelevantes e destacar visualmente o que será usado (por exemplo, colorir arcos \emph{apertados} e cortes ativos).
    \item \textbf{Contiguidade espacial e temporal}: manter fórmulas, legendas e diagramas próximos e apresentados em sincronia (e.g., exibir o custo reduzido $c'(u,v)$ ao lado da aresta correspondente).
    \item \textbf{Modalidade e redundância}: preferir explicações textuais curtas + gráficos a blocos longos de texto ou duplicações desnecessárias do mesmo conteúdo em múltiplos canais.
    \item \textbf{Segmentação e pré‑treinamento}: quebrar provas e algoritmos em passos curtos, introduzindo previamente a notação e convenções que serão usadas.
    \item \textbf{Exemplos resolvidos com ``fading'' e autoexplicações}: começar com exemplos totalmente resolvidos, retirar gradualmente etapas e pedir justificativas locais (``por que este arco é apertado?''), deslocando esforço para a carga pertinente.
\end{itemize}

No campo específico de matemática avançada, Tall enfatiza a coordenação entre registros — intuitivo, visual, simbólico e formal — como motor da passagem do pensamento predominantemente procedimental para o conceitual \cite{tall1991advanced}. Diagramas não são meros adornos: estruturam inferências espaciais e relacionais de modo mais eficiente que sentenças lineares \cite{larkin1987diagram}. Assim, tarefas que pedem aos estudantes alternar e integrar registros (por exemplo, derivar a expressão de $c'$ e marcá‑la no grafo; identificar no condensado os conjuntos onde a complementaridade está ativa) tendem a aumentar a retenção e a transferência, ao mesmo tempo em que reduzem a carga extrínseca.

\paragraph{}
Em grafos, a visualização é crucial para entender propriedades globais (cortes, ciclos, conectividade) a partir de elementos locais (vértices, arestas) \cite{bondy2008graph,diestel2017graph,west2001introduction}. Ferramentas que permitem manipular e explorar grafos dinamicamente ajudam a construir intuições espaciais e relacionais \cite{larkin1987diagram,ware2012information}. Em algoritmos, visualizações interativas que promovem engajamento ativo (prever, manipular, explicar) são mais eficazes que exposições passivas \cite{hundhausen2002meta,naps2003engagement}.

\subsection{Dificuldades no ensino e na aprendizagem}
\paragraph{}
Em grafos e dígrafos, o concreto (nós/arcos) convive com propriedades globais (cortes, SCCs, laminaridade) \cite{bondy2008graph,diestel2017graph,west2001introduction}. No tema de r‑arborescências de custo mínimo, três obstáculos didáticos são frequentes: (i) articular o local com o global — mínimos por vértice podem induzir ciclos; a condensação em SCCs torna esse fenômeno visível (Fig.~\ref{fig:didatica-condensado}); (ii) acompanhar efeitos de contração/expansão nos custos reduzidos e na família de cortes ativos — a laminaridade oferece uma “geometria de inclusões” útil (Fig.~\ref{fig:didatica-laminar}); (iii) mapear o “fazer do algoritmo” para a linguagem primal–dual (apertude, complementaridade) e conectar métricas empíricas a garantias teóricas — explicitado por custos reduzidos e reexpansão (Figs.~\ref{fig:didatica-reduced-cost} e \ref{fig:didatica-reexpansion}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.72\linewidth]{figures/fig_laminaridade.pdf}
    \caption{Família laminar de cortes ativos: uma geometria de inclusões que organiza a prova primal–dual e guia a Fase I.}
    \label{fig:didatica-laminar}
\end{figure}

\subsection{Uso de ferramentas e recursos visuais}
\paragraph{}
Diagramas e visualizações bem projetados aceleram inferências espaciais e relacionais \cite{larkin1987diagram,ware2012information}. Em educação em computação, visualizações de algoritmos são mais efetivas quando promovem engajamento ativo (prever, manipular, explicar), não apenas consumo passivo \cite{hundhausen2002meta,naps2003engagement}. O mantra “overview $\to$ zoom/filter $\to$ details‑on‑demand” alinha interação a metas cognitivas \cite{shneiderman1996eyes}. Esses princípios informam as escolhas de destaque em \(D_0\), cortes e ciclos mostradas nas Figuras~\ref{fig:didatica-reduced-cost} e \ref{fig:didatica-reexpansion}.

\paragraph{}
Diretrizes adotadas neste projeto:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item Semântica visual consistente: destacar o subgrafo de zeros $D_0$, arcos \emph{apertados} e cortes ativos.
    \item Sincronizar passos e justificativas: cada ação (normalizar, contrair, reexpandir) acompanhada do “por quê” (efeitos em $c'$, invariantes).
    \item Controle de granularidade: passo a passo, agregação de passos, previsões com feedback.
    \item Trilhas auditáveis: logs que referenciem estados do grafo reduzem carga extrínseca.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=.7\linewidth]{figures/fig_chu_liu_reduced_cost.pdf}
    \caption{Custos reduzidos e arcos \emph{apertados}: destacar $c'\ge 0$ e as entradas de custo zero torna visível o vínculo entre ação e justificativa.}
    \label{fig:didatica-reduced-cost}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=.7\linewidth]{figures/fig_chu_liu_reexpansion.pdf}
    \caption{Contração e reexpansão de ciclos: visualizar o “macro” (vértice contraído) e o “micro” (remover uma aresta interna) facilita a prova de correção.}
    \label{fig:didatica-reexpansion}
\end{figure}

\subsection{A importância de disseminar o ensino e como fazer}
\paragraph{}
Conectar teoria, evidências empíricas e interatividade favorece transferência e retenção \cite{mayer2009multimedia,sweller1988cognitive}. Para os temas desta dissertação (r‑arborescências, primal–dual, laminaridade), propomos um eixo de disseminação com materiais reprodutíveis (código, CSVs, figuras) e uma aplicação web que materializa os conceitos (potenciais, 0‑arestas, ciclos) em ações manipuláveis.

\paragraph{Plano de ação em sala e além}
\begin{itemize}\setlength{\itemsep}{2pt}
    \item Exploração guiada com previsões: antes de cada passo, pedir hipóteses sobre o próximo 0‑arco/contração; depois, comparar com o resultado e justificar à luz de $c'$ e dos cortes.
    \item Reconstrução de provas a partir de instâncias: checar complementaridade/laminaridade em casos pequenos e explicitar a ponte entre “o que o algoritmo faz” e “por que funciona”.
    \item Mini‑investigações de desempenho: variar $|A|$, densidade e famílias adversárias; coletar tempos/contrações e confrontar com limites teóricos.
    \item Checklist operacional para a ferramenta: (i) visão geral do grafo e de $D_0$; (ii) passo a passo com rastro de cortes; (iii) painel de métricas (tempo por fase, contrações, pico de memória); (iv) botão “explicar por quê” referindo complementaridade; (v) \emph{presets} (aleatória, densa, esparsa, em camadas).
    \item Disseminação aberta: publicar código, dados e guias rápidos; sugerir atividades prontas (10–15 min) para disciplinas de grafos/otimização e formação docente.
\end{itemize}

\paragraph{}Em conjunto, essas práticas alinham fundamentos de aprendizagem com recursos visuais e artefatos reprodutíveis, ampliando acesso e consolidando compreensão conceitual e procedimental dos algoritmos.
