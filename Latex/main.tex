...
\newtcblisting{pybox}[2][]{
    ...
    listing only, listing engine=listings, title={#2}, #1,
...
\paragraph{Visão Operacional de Alto Nível - Chu–Liu/Edmonds}
\paragraph{}
...
Com estruturas de dados adequadas, evita‑se revarrer todas as arestas a cada contração e obtém‑se \(O(m\log n)\). A ideia é manter, para cada componente (vértice ou supervértice), uma fila de prioridade (heap) com as arestas que \emph{entram} nele, chaveadas pelo custo \emph{reduzido}. Os potenciais \(y(\cdot)\) são aplicados de forma \emph{lazy} como um deslocamento (offset) agregado por componente, de modo que reponderar não exige atualizar todas as chaves. Assim, o arco mínimo que entra em um componente vira um \emph{extract‑min} no seu heap; quando um ciclo é contraído, fazemos \emph{meld} (união) dos heaps dos vértices de \(C\) e ajustamos um único offset no novo supervértice. Cada aresta é inserida em algum heap no máximo uma vez e é extraída/descartada no máximo uma vez; como inserções/extrações custam \(O(\log n)\) amortizado, o total fica em \(O(m\log n)\). A detecção de ciclos no grafo funcional \(F^*\) permanece linear no tamanho do componente e é dominada pelo custo de heap.

\paragraph{}
O uso de memória é \(O(m+n)\), incluindo mapeamentos de contração/expansão e as filas de prioridade dos arcos de entrada.

\paragraph{}
A implementação a seguir adota a versão \(O(mn)\) por simplicidade, mas com estruturas que facilitam a transição para \(O(m\log n)\) se desejado.

\section{Implementação em Python}

\paragraph{Visão geral}
Esta seção apresenta uma implementação em Python do algoritmo de Chu–Liu/Edmonds, baseada no código do trabalho. A arquitetura segue os passos teóricos: (i) remoção de arestas que entram na raiz, (ii) normalização dos custos de entrada por vértice, (iii) construção do grafo funcional \(F^*\), (iv) detecção/contração de ciclos e (v) recursão seguida de reexpansão.

\paragraph{Contrato (entrada/saída e hipóteses)}
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Entrada:} dígrafo ponderado \(D=(V,A)\), custos \(c:A\to\mathbb{R}\), raiz \(r\in V\).
    \item \textbf{Hipóteses:} (i) todo \(v\neq r\) é alcançável a partir de \(r\) (caso contrário, não há r‑arborescência); (ii) para todo subconjunto não vazio \(X\subseteq V\setminus\{r\}\), existe ao menos um arco que entra em \(X\) (\(\delta^-(X)\neq\emptyset\); condições clássicas de existência \`a la Edmonds \cite{schrijver2003comb}).
    ...
    \item \textbf{Convenções:} paralelos são permitidos após contrações; laços (self‑loops) são descartados.
\end{itemize}

\paragraph{Observações de implementação}
...
def normalize_incoming_edge_weights(D: nx.DiGraph, node: str, lang: str = "pt") -> None:
    """Subtrai de cada aresta que entra em `node` o menor custo que entra em `node`.
    Modifica o grafo D in-place.
    """
    assert node in D, (
        "normalize_incoming_edge_weights: O vértice '%s' não existe no grafo." % node
        if lang == "pt" else
        "normalize_incoming_edge_weights: The vertex '%s' does not exist in the graph." % node
    )
    preds = list(D.in_edges(node, data="w"))
    if not preds:
        return
    yv = min(w for _, _, w in preds)
    for u, _, _ in preds:
        D[u][node]["w"] -= yv
\end{pybox}
...
def get_Fstar(D: nx.DiGraph, r0: str, lang: str = "pt") -> nx.DiGraph:
    ...
    assert r0 in D, (
        "get_Fstar: O vértice raiz '%s' não existe no grafo." % r0
        if lang == "pt" else
        "get_Fstar: The root vertex '%s' does not exist in the graph." % r0
    )
    F_star = nx.DiGraph()
    for v in D.nodes():
        if v == r0:
...
def contract_cycle(D: nx.DiGraph, C: nx.DiGraph, label: str, lang: str = "pt"):
    ...
    assert label not in D, (
        "contract_cycle: O rótulo '%s' já existe em G." % label
        if lang == "pt" else
        "contract_cycle: The label '%s' already exists in G." % label
    )
...
def remove_edges_to_r0(D: nx.DiGraph, r0: str, lang: str = "pt") -> nx.DiGraph:
    assert r0 in D, (
        "remove_edges_to_r0: O vértice raiz '%s' não existe." % r0
        if lang == "pt" else
        "remove_edges_to_r0: The root vertex '%s' does not exist." % r0
    )
