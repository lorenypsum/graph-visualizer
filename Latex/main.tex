...
\subsubsection{Exemplo prático: Chu–Liu/Edmonds passo a passo}
...
%   title={Exemplo prático: execução do algoritmo},
%   boxed title style={sharp corners, boxrule=0.6pt},
%   sharp corners, boxrule=0.6pt
% ]
% \paragraph{Grafo original:}
% \begin{center}
% \begin{tikzpicture}[>=Stealth, node distance=1.5cm, scale=0.9, every node/.style={scale=0.9}]
%   % Nodes
%   \foreach \i/\x/\y in {0/0/0, 1/2/1, 2/4/0, 3/2/-1, 4/6/1, 5/8/0, 6/6/-1, 7/8/2, 8/10/1}
%     \node[circle,draw,minimum size=7mm] (n\i) at (\x,\y) {\i};
%   % Edges (sample, not all from your JSON)
%   \draw[->] (n0) -- node[above] {3} (n1);
%   \draw[->] (n0) -- node[above] {6} (n2);
%   \draw[->] (n1) -- node[left] {1} (n2);
%   \draw[->] (n1) -- node[left] {2} (n3);
%   \draw[->] (n1) -- node[above] {10} (n4);
%   \draw[->] (n2) -- node[right] {1} (n1);
%   \draw[->] (n3) -- node[below] {1} (n4);
%   \draw[->] (n4) -- node[above] {10} (n2);
%   \draw[->] (n4) -- node[above] {1} (n5);
%   \draw[->] (n5) -- node[above] {1} (n6);
%   \draw[->] (n6) -- node[above] {1} (n4);
%   \draw[->] (n6) -- node[right] {8} (n7);
%   \draw[->] (n6) -- node[below] {2} (n8);
%   \draw[->] (n7) -- node[above] {4} (n8);
%   \draw[->] (n8) -- node[right] {5} (n6);
% \end{tikzpicture}
% \end{center}

\paragraph{Passos principais do algoritmo:}
\begin{itemize}
  \item Remover arestas que entram na raiz (vértice $0$).
  \item Normalizar pesos das arestas de entrada para cada vértice.
  \item Construir $F^*$: selecionar para cada vértice a aresta de menor custo de entrada.
  \item Detectar ciclos e contrair, se necessário.
  \item Repetir o processo recursivamente até obter uma arborescência.
\end{itemize}

% \paragraph{Arborescência final encontrada:}
% \begin{center}
% \begin{tikzpicture}[>=Stealth, node distance=1.5cm, scale=0.9, every node/.style={scale=0.9}]
%   % Nodes
%   \foreach \i/\x/\y in {0/0/0, 1/2/1, 2/4/0, 3/2/-1, 4/6/1, 5/8/0, 6/6/-1, 7/8/2, 8/10/1}
%     \node[circle,draw,minimum size=7mm,fill=blue!10] (n\i) at (\x,\y) {\i};
%   % Arborescência (edges from your result)
...
%   \draw[->,thick,blue] (n6) -- (n7);
%   \draw[->,thick,blue] (n6) -- (n8);
% \end{tikzpicture}
% \end{center}
...
\emph{Função recursiva que encontra a arborescência ótima em um dígrafo D com raiz r0 usando o algoritmo de Chu–Liu/Edmonds.}
\tcblower
\begin{lstlisting}[mathescape=true, language=Python]
