...
\begin{lstlisting}[language=Python,caption={Estado global e função de desenho de grafos},label={lst:draw_graph}]
...
\begin{lstlisting}[language=Python,caption={Manipuladores de eventos para edição e preparação de exemplos},label={lst:event_handlers}]
@when("click", "#add-edge")
def add_edge():
        global G
...
\begin{lstlisting}[language=Python,caption={Execução do algoritmo e exportação},label={lst:algorithm_export}]
...
\begin{lstlisting}[language=HTML,caption={Estrutura principal da página HTML com PyScript},label={lst:index_html}]
