...
\paragraph{}
Os primeiros algoritmos para arborescência de custo mínimo foram propostos por Chu–Liu (1965) e Edmonds (1967). Eles introduziram a ideia de contrair ciclos e de caracterizar quando um conjunto de arcos forma uma arborescência. Mais tarde, essa mesma lógica passou a ser lida no arcabouço \emph{primal–dual}: mantemos uma solução "estrutural" (primal) no grafo contraindo ciclos e, em paralelo, uma solução "numérica" (dual) por meio de potenciais e custos reduzidos. András Frank sistematizou essa leitura em um procedimento em duas fases \cite{frank2014}:

\paragraph{}
(i) Potenciais e criação de zeros. Inicialize \(y\equiv 0\) e mantenha o subgrafo \(D_0\) formado pelos arcos com \emph{custo reduzido} zero (isto é, \(c'(u,v)=c(u,v)-y(v)=0\)). Enquanto existir \(v\neq r\) sem nenhuma entrada a zero em \(D_0\), considere o conjunto \(X=\mathrm{Anc}_{D_0}(v)\cup\{v\}\) (os vértices que alcançam \(v\) por arcos de \(D_0\), mais o próprio \(v\)). Eleve uniformemente os potenciais em \(X\) em \(\Delta\), onde \(\Delta\) é a menor folga entre os arcos que \emph{entram} em \(X\): \(\Delta=\min\{\,c'(u,x): u\notin X,\ x\in X\,\}=\min c'(\delta^-(X))\). Isso faz ao menos um arco que entra em \(X\) zerar. À medida que surgem novos zeros, pode aparecer um ciclo composto só de arcos de custo reduzido zero; quando isso ocorrer, contraia o ciclo e continue no grafo menor, preservando a equivalência de custos. A Fase I termina quando, após eventuais contrações, todo \(v\neq r\) dispõe de ao menos uma entrada com \(c'(u,v)=0\). (Resumo formal em Alg.\ \ref{th:frank-fase-I}.)

\paragraph{}
(ii) Extração sobre o subgrafo de zeros. No grafo (possivelmente contraído), escolha para cada \(v\neq r\) exatamente um arco \((u,v)\) com \(c'(u,v)=0\), evitando ciclos. Se um ciclo surgir, contraia-o e prossiga no grafo menor. Ao final, expanda as contrações em ordem inversa: em cada componente, mantenha o arco de custo reduzido zero que \emph{entra} no componente e remova uma única aresta interna para quebrar o ciclo e restaurar grau de entrada 1. Como todas as escolhas são \emph{apertadas} (custos reduzidos zero) em relação a \(y\), a arborescência resultante é ótima também para os custos originais \(c\) por complementaridade primal–dual. (Ver Alg.\ \ref{th:frank-fase}.)

\paragraph{}
Essa leitura também conecta o problema a condições de corte. Cortes “apertados” (com entrada disponível mínima a custo reduzido) podem ser escolhidos de modo a \emph{não se cruzarem} — formam uma família laminar — o que guia as contrações sem ambiguidades e simplifica prova e implementação \cite{frank2014,schrijver2003comb}. Há ainda vínculos com matroides, úteis para generalizações e empacotamentos.

\paragraph{Técnicas primal–duais:}
\paragraph{}
Em poucas palavras, “primal–dual” significa acompanhar duas coisas ao mesmo tempo, que se verificam mutuamente:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Solução primal:} a estrutura que queremos construir (a arborescência) — ou o grafo após contrações.
    \item \textbf{Solução dual:} um número \(y(v)\) para cada vértice (os “potenciais”). Com eles, definimos o \emph{custo reduzido} de um arco \((u,v)\) por \(c'(u,v)=c(u,v)-y(v)\). Arcos com \(c'=0\) são \emph{apertados} e elegíveis na solução.
\end{itemize}
Se \(c'(u,v)=0\), dizemos que o arco está \emph{apertado}. O algoritmo eleva potenciais exatamente quando um vértice ainda não tem entrada apertada: isso desce, em bloco, os custos das entradas daquele conjunto \(X\) até que alguma zere. Sempre que surge um ciclo só com arcos apertados, contraímos e seguimos. No fim, quando a árvore usa apenas arcos apertados e cada vértice (exceto a raiz) tem uma única entrada, as regras de \emph{complementaridade} garantem a otimalidade: não há como baratear sem violar alguma restrição.

\paragraph{}
Essa abordagem tem várias vantagens:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item Fornece um “comprovante” de otimalidade (os próprios potenciais servem de certificado dual).
    \item Leva a algoritmos simples e polinomiais, com passos puramente combinatórios.
    \item Lida bem com pesos gerais (até negativos), graças aos custos reduzidos.
    \item Explora cortes que não se cruzam (família laminar), o que simplifica prova e implementação.
    \item Serve de base para versões mais rápidas e para extensões/aproximações \cite{schrijver2003comb,frank2014}.
\end{itemize}

\paragraph{Conexão com condições de corte}

\paragraph{}
Pelas condições clássicas de existência, para todo subconjunto não vazio \(X\subseteq V\setminus\{r\}\) deve existir ao menos um arco que entra em \(X\) (corte \(\delta^-(X)\) não vazio). No método de Frank, a Fase I pode ser vista como “gastar” potencial sobre cadeias de conjuntos \(X\) até zerar alguma entrada desses cortes. Os cortes que efetivamente ficam no limite formam uma família laminar e guiam as contrações. Ao final, a r-arborescência mínima emerge escolhendo apenas arcos de custo reduzido zero (Fase II).

\paragraph{Método de duas fases}

\paragraph{}
Resumo do procedimento (ver Alg.\ \ref{th:frank-fase-I} e Alg.\ \ref{th:frank-fase}):

\paragraph{}
Entrada: dígrafo \(D=(V,A)\), custos \(c\), raiz \(r\).

\begin{enumerate}\setlength{\itemsep}{2pt}
    \item (Fase I) Mantenha potenciais \(y\) e o subgrafo \(D_0\). Enquanto existir \(v\neq r\) sem entrada a zero, eleve potenciais em \(X=\mathrm{Anc}_{D_0}(v)\cup\{v\}\) em \(\Delta=\min c'(\delta^-(X))\) para criar novos zeros; contraia ciclos de zeros se surgirem.
...
\subsection{Fundamentos teóricos e algoritmo detalhado}

\paragraph{Potenciais e custos reduzidos}
Há duas leituras equivalentes e úteis: (i) uma formulação \emph{primal--dual} por cortes e (ii) um ponto de vista de \emph{potenciais} mais local (por vértice), que conduz à mesma mecânica operativa.

- Formulação primal (relaxada). Escolhemos variáveis \(x_a\in[0,1]\) para cada arco \(a\in A\); o objetivo é
\[
    \min\; \sum_{a\in A} c(a)\,x_a \quad \text{sujeito a}\quad \sum_{a\in \delta^-(X)} x_a \ge 1\;\; \forall\, \emptyset\ne X\subseteq V\setminus\{r\}.
\]
Essas restrições garantem que todo conjunto \(X\) não contendo \(r\) receba ao menos um arco de entrada, condição necessária e suficiente para a existência de uma r-arborescência.

- Dual por cortes. Associamos uma variável dual \(y(X)\ge 0\) a cada \(\emptyset\ne X\subseteq V\setminus\{r\}\) e maximizamos
\[
    \max\; \sum_{\emptyset\ne X\subseteq V\setminus\{r\}} y(X)\quad \text{sujeito a}\quad \sum_{X:\, v\in X,\ u\notin X} y(X)\;\le\; c(u,v)\;\; \forall\,(u,v)\in A.
\]
Definindo os \emph{custos reduzidos} por
\[
    c'(u,v)\;:=\; c(u,v)\;-
    \sum_{X:\, v\in X,\ u\notin X} y(X),
\]
as restrições duais dizem exatamente que \(c'(u,v)\ge 0\) para todo arco. Arcos com \(c'(u,v)=0\) são chamados \emph{apertados}. Por um argumento de \emph{uncrossing}, pode-se escolher uma solução dual ótima \(y\) cuja família \(\mathcal{L}=\{X: y(X)>0\}\) seja \emph{laminar} (os conjuntos não se cruzam), o que simplifica a implementação \cite{frank2014,schrijver2003comb}.

- Potenciais por vértice. Uma forma prática de manter \(c'\) não-negativo é usar potenciais \(\tilde y:V\to\mathbb{R}\) que atuam apenas nas entradas de cada vértice: \(c'(u,v)=c(u,v)-\tilde y(v)\). Quando \(\tilde y\) deriva de um dual laminar, ele coincide com a soma dos \(y(X)\) nos conjuntos \(X\) de uma cadeia laminar que contém \(v\). Operacionalmente, aumentar \(\tilde y(v)\) diminui uniformemente os custos das arestas que \emph{entram} em \(v\), preservando ordens relativas entre entradas e, portanto, a equivalência de otimalidade entre \(c\) e \(c'\). A meta da Fase I é ajustar \(\tilde y\) até garantir, para todo \(v\neq r\), ao menos uma entrada com \(c'(u,v)=0\).

\paragraph{Cortes apertados e laminaridade}
Escreva \(\delta^-(X)=\{(u,v)\in A: u\notin X,\ v\in X\}\). Dizemos que um arco \((u,v)\) é \emph{apertado} se \(c'(u,v)=0\), e que um corte \(X\) é \emph{apertado} se a restrição correspondente está no limite: \(\sum_{a\in \delta^-(X)} x_a=1\) no primal e \(y(X)>0\) no dual. O \emph{uncrossing} mostra que sempre existe solução dual ótima cuja família de conjuntos ativos \(\mathcal{L}=\{X: y(X)>0\}\) é laminar; geometricamente, \(\mathcal{L}\) se organiza em cadeias aninhadas que guiam as contrações. Nos passos do algoritmo, elevar potenciais até zerar alguma aresta que entra em um \(X\) “mínimo” cria novos arcos apertados; se um ciclo de arcos apertados aparece, contraímo-lo e prosseguimos no grafo menor, mantendo \(c'\ge 0\) e a correspondência primal--dual \cite{frank2014,schrijver2003comb}.

\paragraph{Corretude (esboço)}
Sejam \(x\) (primal) e \(y\) (dual) soluções viáveis. As condições de \emph{complementaridade} dizem: (i) se \(x_{(u,v)}>0\) então a restrição dual do arco está \emph{justa}, logo \(c'(u,v)=0\); (ii) se \(y(X)>0\) então a restrição primal do corte está \emph{justa}, isto é, \(\sum_{a\in \delta^-(X)} x_a=1\). O procedimento em duas fases constrói exatamente uma \(r\)-arborescência \(T\) formada apenas por arcos apertados e uma família laminar \(\mathcal{L}\) de cortes com \(y(X)>0\). Cada contração de ciclo preserva (e simplifica) as duas soluções; na reexpansão, escolhemos o único arco apertado que entra no componente e removemos uma aresta interna para restaurar grau de entrada 1 e aciclicidade. Por indução sobre o número de contrações, obtemos \(x=\mathbf{1}_T\) viável no primal, \(y\) viável no dual e \(\sum c\,x = \sum y\), logo \(T\) é ótimo para \(c'\) e, por equivalência, para \(c\) \cite{frank2014,schrijver2003comb}.

\paragraph{Complexidade}
Na forma didática, manter o subgrafo de zeros, encontrar o próximo \(\Delta=\min c'(\delta^-(X))\) e detectar/contrair ciclos custa \(O(m)\) por nível; com no máximo \(O(n)\) níveis (devido às contrações), resulta \(O(mn)\) e memória \(O(m+n)\). Melhorias clássicas reduzem para \(O(m\log n)\): (i) manter, para cada \(v\), o menor custo reduzido de entrada em uma \emph{heap} (atualizações amortizadas quando \(y\) varia), (ii) detectar ciclos em \(D_0\) incrementalmente com DFS/union-find, e (iii) realizar contrações com redirecionamentos agregados. Em implementações otimizadas, o fator logarítmico vem das estruturas de seleção de mínimos, enquanto o número de contrações permanece \(O(n)\) \cite{frank2014,schrijver2003comb}.

\subsection{Implementação em Python}

\paragraph{Visão geral}
A seguir apresentamos uma implementação didática em duas fases. Mantemos um subgrafo \(D_0\) com as arestas de custo reduzido zero e iterativamente elevamos potenciais para criar novas entradas a zero; em seguida, extraímos uma r-arborescência usando apenas arestas de custo zero. O foco é a clareza; a complexidade segue \(O(mn)\) na prática.

\paragraph{Construção do subgrafo de zeros}
Cria \(D_0\) contendo apenas arestas com peso zero e uma lista \(A_0\) com essas arestas.

\begin{pybox}{Subgrafo de zeros e lista de zeros}
def build_D_zero(D: nx.DiGraph):
    """
    Constrói o dígrafo D_zero com as arestas de peso zero de D
    e retorna também a lista A_zero com essas arestas.
    Complexidade: O(m).
    """
    D_zero = nx.DiGraph()
    A_zero = []
...
\paragraph{Arestas que entram em X e corte mínimo}
Seleciona \(\delta^-(X)\) e calcula o menor peso dentre essas arestas.

\begin{pybox}{Arestas que entram em X e mínimo}
def get_arcs_entering_X(D: nx.DiGraph, X: set[str]):
    """
    Retorna a lista de arestas (u,v,data) com u fora de X e v em X.
    """
    return [(u, v, data) for u, v, data in D.edges(data=True) if u not in X and v in X]

def get_minimum_weight_cut(arcs: list[tuple[str, str, dict]]):
    """
    Retorna o menor peso entre as arestas fornecidas; supõe lista não vazia.
    """
    return min(data.get("w", 0) for _, _, data in arcs)
\end{pybox}

\paragraph{Atualização de pesos que entram em X}
Eleva o potencial de cada vértice de X em \(\Delta\) subtraindo \(\Delta\) aos pesos das arestas que \emph{entram} em X; ao zerar, a aresta é adicionada a \(D_0\) e a \(A_0\).

\begin{pybox}{Atualização de pesos e manutenção de zeros}
def update_weights_in_X(D: nx.DiGraph, X: set[str], delta: float, A_zero: list[tuple[str, str]], D_zero: nx.DiGraph):
    """
    Subtrai delta de toda aresta que entra em X; ao zerar, atualiza D_zero e A_zero.
    Efeito colateral: modifica D e D_zero in-place. Complexidade: O(m).
    """
    for u, v, data in D.edges(data=True):
        if v in X:
            data["w"] = data.get("w", 0) - delta
            if data["w"] == 0:
                if (u, v) not in A_zero:
                    A_zero.append((u, v))
                D_zero.add_edge(u, v, **data)
\end{pybox}

\paragraph{Fase I: normalização por potenciais}
Itera sobre vértices não raiz, computa \(X=\mathrm{Anc}_{D_0}(v)\cup\{v\}\) e, se necessário, eleva o potencial de \(v\) para criar ao menos uma entrada a zero; detecta e contrai ciclos de zeros por meio do subgrafo \(D_0\) (omissões de implementação para foco didático).

\begin{pybox}{Fase I de Frank (didática)}
def phase1_find_minimum_arborescence(D_original: nx.DiGraph, r0: str):
    """
    Ajusta potenciais elevando entradas a zero e mantém D_zero/A_zero.
    Retorna (D_zero, A_zero). Complexidade: O(mn) na forma direta.
    """
    D = D_original.copy()
    ...
    while progress:
        ...
        for v in D.nodes:
            if v == r0:
                continue
            X = set(nx.ancestors(D_zero, v)) | {v}
            if r0 in X:
                continue  # já alcançável por zeros
            arcs = get_arcs_entering_X(D, X)
            if not arcs:
                continue  # instância inviável para este X
            delta = get_minimum_weight_cut(arcs)
            if delta > 0:
                update_weights_in_X(D, X, delta, A_zero, D_zero)
                progress = True
    return D_zero, A_zero
\end{pybox}

\paragraph{Fase II: extração sobre zeros}
Constrói uma r-arborescência escolhendo apenas arestas em \(A_0\) (peso zero reduzido). Estratégia gulosa: expande a fronteira a partir de \(r_0\).

\begin{pybox}{Fase II de Frank (didática)}
def phase2_build_arborescence(D_original: nx.DiGraph, r0: str, A_zero: list[tuple[str, str]]):
    """
    Constrói r-arborescência usando apenas arestas de custo reduzido zero de A_zero.
    Complexidade: O(n + |A_zero|) em tempo; usa dados de D_original.
    """
    T = nx.DiGraph()
    T.add_node(r0)
    ...
    for _ in range(n):
        ...
        for (u, v) in A_zero:
            if u in T.nodes and v not in T.nodes:
                T.add_node(v)
                edge_data = D_original.get_edge_data(u, v, default={"w": 0})
...
\paragraph{Função orquestradora}
Integra as fases e devolve \(T\).

\begin{pybox}{Frank: função principal}
def find_minimum_arborescence_frank(D: nx.DiGraph, r0: str = "r0"):
    """
    Encontra uma r-arborescência mínima pelo método de Frank em duas fases.
    """
    D_zero, A_zero = phase1_find_minimum_arborescence(D, r0)
    T = phase2_build_arborescence(D, r0, A_zero)
    return T
\end{pybox}

\paragraph{Observações e casos‑limite}
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Inviabilidade.} Se algum \(v\neq r_0\) permanece sem entrada possível, a instância não admite r-arborescência.
    \item \textbf{Ciclos de zeros.} Uma implementação robusta deve contrair ciclos detectados em \(D_0\) e manter mapeamentos para a expansão, como em Chu–Liu/Edmonds.
    \item \textbf{Pesos negativos.} São suportados: a normalização por potenciais desloca custos de forma relativa e não afeta comparações entre entradas do mesmo vértice.
    \item \textbf{Complexidade.} O esqueleto acima é didático (\(O(mn)\)). Com manutenção incremental de mínimos e detecção eficiente de ciclos, obtém-se \(O(m\log n)\).
\end{itemize}

\paragraph{Corretude e complexidade}
O arcabouço primal–dual com potenciais e cortes apertados fornece as condições de otimalidade: arestas escolhidas são \emph{apertadas} em relação a \(y\) e nenhum ajuste adicional reduz o custo sem violar restrições. A complexidade segue a discussão anterior: \(O(mn)\) na forma direta e \(O(m\log n)\) com estruturas eficientes \cite{frank2014,schrijver2003comb}.
