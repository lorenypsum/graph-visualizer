...
\begin{enumerate}\setlength{\itemsep}{2pt}
    \item Para cada \(v\neq r\), escolha \(a_v\in\arg\min\{c(u,v):(u,v)\in A\}\) e forme \(A'=\{a_v: v\neq r\}\).
    \item Se \((V,A')\) é acíclico, devolva \(A'\) (é uma r‑arborescência ótima \cite[Obs.~4.36]{kleinberg2006}).
    \item Caso contrário, seja \(C\) um ciclo em \(A'\). Contraia \(C\) em \(x_C\). Para cada arco \((u,w)\) que \emph{entra} em \(C\) (isto é, \(w\in V(C), u\notin V(C)\)), defina
    \[c'(u,x_C) \leftarrow c(u,w) - c(a_w).\]
    Resolva recursivamente com custos \(c'\). Na volta, expanda \(C\) e, dentro de \(C\), remova exatamente um arco de \(C\) e inclua o arco que entra em \(C\) escolhido na recursão (de custo reduzido zero), preservando aciclicidade e grau de entrada 1.
\end{enumerate}
\end{algobox}
