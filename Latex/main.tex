...
\paragraph{}
Os primeiros algoritmos para arborescência de custo mínimo foram propostos por Chu–Liu (1965) e Edmonds (1967). Eles introduziram a ideia de contrair ciclos e de caracterizar quando um conjunto de arcos forma uma arborescência. Mais tarde, esses métodos passaram a ser interpretados no arcabouço \emph{primal–dual}. András Frank organizou essa visão em um procedimento em duas fases:

\paragraph{}
(i) Potenciais e criação de zeros. Inicialize \(y\equiv 0\) e mantenha o subgrafo \(D_0\) formado pelas arestas com \emph{custo reduzido} zero (isto é, \(c'(u,v)=c(u,v)-y(v)=0\)). Enquanto existir \(v\neq r\) sem nenhuma entrada a zero em \(D_0\), aumente \(y(v)\) em \(\Delta:=\min\{c'(u,v):(u,v)\in A\}\) de modo que ao menos uma aresta que entra em \(v\) zere. À medida que novos zeros surgem, pode aparecer um ciclo composto apenas de arestas de custo reduzido zero; quando isso ocorrer, contraia o ciclo e continue no grafo menor, preservando a equivalência de custos. A Fase I termina quando, após eventuais contrações, todo \(v\neq r\) dispõe de ao menos uma entrada com \(c'(u,v)=0\).

\paragraph{}
(ii) Extração sobre o subgrafo de zeros. Trabalhando no grafo (possivelmente contraído), selecione exatamente uma entrada de custo reduzido zero para cada \(v\neq r\), evitando ciclos. Se a seleção induzir um ciclo, contraia-o e prossiga recursivamente. Ao término, expanda as contrações em ordem inversa: para cada componente contraído, escolha o arco de custo reduzido zero que entra no componente e remova uma única aresta interna para quebrar o ciclo. Como todas as arestas escolhidas são \emph{apertadas} (têm custo reduzido zero) em relação a \(y\), a arborescência construída é ótima também para os custos originais \(c\) por complementaridade.

\paragraph{}
Essa leitura também conecta o problema a cortes “apertados” (que não se cruzam, isto é, formam uma família laminar) e a ferramentas de matroides, unificando a teoria e a implementação \cite{frank2014,schrijver2003comb}.

\paragraph{Técnicas primal–duais:}
\paragraph{}
Em poucas palavras, “primal–dual” significa trabalhar com duas coisas ao mesmo tempo:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Solução primal:} a estrutura que queremos construir (a arborescência) — ou o grafo após contrações.
    \item \textbf{Solução dual:} um número \(y(v)\) para cada vértice (os “potenciais”). Com eles, definimos o \emph{custo reduzido} de uma aresta \((u,v)\) por \(c'(u,v)=c(u,v)-y(v)\).
\end{itemize}
Se \(c'(u,v)=0\), dizemos que a aresta está \emph{apertada}. O algoritmo aumenta \(y(v)\) quando \(v\) ainda não tem nenhuma entrada apertada: isso empurra alguns custos para baixo até zerar e cria ao menos uma opção de entrada “barata” para \(v\). Sempre que surge um ciclo formado só por arestas apertadas, contraímos o ciclo e continuamos. No fim, quando a árvore usa apenas arestas apertadas e cada vértice (exceto a raiz) tem exatamente uma entrada, as regras de \emph{complementaridade} dizem que a solução é ótima: não há como baratear sem violar as regras.
...
\paragraph{}
O procedimento em duas fases pode ser resumido como segue:
...
\begin{algobox}{Frank: fases II}{frank-fase}
Entrada: dígrafo \(D=(V,A)\), custos \(c:A\to\mathbb{R}\), raiz \(r\).
\begin{enumerate}\setlength{\itemsep}{2pt}
    \item \textbf{Fase I — normalização primal–dual (potenciais e cortes).}
    ...
    \end{enumerate}
    \item \textbf{Fase II — extração sobre o subgrafo de zeros.}
    \begin{enumerate}\setlength{\itemsep}{1pt}
...
Definimos potenciais nos vértices \(y:V\to\mathbb{R}\) e custos reduzidos \(c'(u,v):=c(u,v)-y(v)\). Aumentar \(y(v)\) desloca uniformemente os custos das arestas que \emph{entram} em \(v\), preservando a ordem relativa entre entradas e, portanto, a otimalidade global (equivalência entre \(c\) e \(c'\)). A meta da Fase I é ajustar \(y\) até garantir, para todo \(v\neq r\), a existência de pelo menos uma entrada com \(c'(u,v)=0\).

\paragraph{Cortes apertados e laminaridade}
Para \(X\subseteq V\setminus\{r\}\), denote \(\delta^-(X)=\{(u,v)\in A: u\notin X, v\in X\}\). Um corte é \emph{apertado} se a soma do custo reduzido disponível para entrar em \(X\) é mínima; sob as atualizações de \(y\), os cortes que se tornam apertados formam uma \emph{família laminar} (não se cruzam), o que permite progressos via contrações sem ambiguidade \cite{frank2014,schrijver2003comb}. Quando surge um ciclo apenas com arestas de custo reduzido zero, contraímo-lo e continuamos no grafo menor.

\paragraph{Corretude (esboço)}
As condições de complementaridade entre os potenciais \(y\) e a escolha final de arestas de custo reduzido zero asseguram que nenhuma troca local reduz o custo. A cada contração, mantemos uma bijeção entre soluções no grafo contraído e no original, como em Chu–Liu/Edmonds: a expansão escolhe o arco que entra no componente com custo reduzido zero e remove uma única aresta interna para quebrar o ciclo. Repetindo por indução, a solução obtida é ótima para \(c'\) e, portanto, para \(c\).

\begin{algobox}{Frank: algoritmo detalhado}{frank-detalhado}
Entrada: dígrafo \(D=(V,A)\), custos \(c\), raiz \(r\).
\begin{enumerate}\setlength{\itemsep}{2pt}
    \item Inicialize potenciais \(y(v)\gets 0\) para todo \(v\in V\). Defina custos reduzidos \(c'(u,v)\gets c(u,v)-y(v)\).
    \item \textbf{Fase I (normalização).}
    \begin{enumerate}
        \item Enquanto existir \(v\neq r\) sem nenhuma aresta \((u,v)\) com \(c'(u,v)=0\):
        \begin{enumerate}
            \item Seja \(\Delta:=\min\{c'(u,v):(u,v)\in A\}\) sobre arestas que entram em \(v\). Faça \(y(v)\gets y(v)+\Delta\) e atualize \(c'(u,v)\gets c'(u,v)-\Delta\) para todas as arestas que entram em \(v\).
            \item Se o subgrafo de arestas com \(c'=0\) contiver um ciclo dirigido \(C\), contraia \(C\) em um supervértice, descartando laços, e continue.
        \end{enumerate}
    \end{enumerate}
    \item \textbf{Fase II (extração).}
    \begin{enumerate}
        \item No grafo (possivelmente contraído), selecione uma r-arborescência apenas entre as arestas com \(c'=0\) (se surgir ciclo, contraia e prossiga).
        \item Expanda as contrações em ordem inversa: em cada expansão, substitua o arco \((u,x_C)\) por um \((u,w)\) com \(w\in C\) e \(c'(u,w)=0\), e remova uma aresta interna de \(C\) para quebrar o ciclo.
    \end{enumerate}
    \item Devolva a r-arborescência obtida.
\end{enumerate}
\end{algobox}

\paragraph{Complexidade}
Na forma direta, cada ajuste de potencial e checagem/contração custa tempo linear no grau de entrada do vértice ou no tamanho do corte considerado, levando a \(O(m)\) por iteração e até \(O(n)\) iterações no pior caso: \(O(mn)\). Com estruturas adequadas (p.ex., filas de prioridade/\emph{heaps} para mínimos incrementais e manutenção eficiente do subgrafo de zeros), é possível atingir \(O(m\log n)\) em implementações mais sofisticadas.
