...
\paragraph{Classes de Problemas}
Além de analisar a complexidade de algoritmos específicos, é importante entender as \emph{classes de problemas} com base em sua dificuldade intrínseca. Algumas das classes mais comuns incluem:    
\begin{itemize}\setlength{\itemsep}{2pt}
...
\paragraph{}
Cada classe de problemas pode ter diferentes níveis de dificuldade, que avaliamos com base na complexidade dos algoritmos conhecidos para resolvê-los.
Alguns problemas são considerados "fáceis" (resolvíveis em tempo polinomial), enquanto outros são "difíceis" (não se conhece nenhum algoritmo eficiente para resolvê-los).
Apresentamos esse conceito de forma breve a seguir.

\paragraph{Classes de complexidade de problemas}

\paragraph{}
Não avaliamos só o desempenho de \emph{um algoritmo}; também queremos saber \emph{quão difícil é o próprio problema}, independentemente do método usado. Como regra prática, consideramos \emph{tratáveis} os problemas que admitem soluções em tempo (ou espaço) \emph{polinomial} no tamanho da entrada\footnote{Por “tamanho da entrada” entendemos a quantidade de símbolos necessária para codificar a instância em uma codificação razoável (tipicamente, o número de bits). Exemplos: (i) para um grafo, mede-se usualmente por \(n=|V|\) e \(m=|E|\); se há pesos, no modelo de bits também se conta o número de bits para representar cada peso; (ii) para inteiros, é o número de dígitos (em base 2/10) do valor; (iii) para uma string, o seu comprimento. Em análises de alto nível, é comum expressar o custo como função de \(n\) e \(m\) (modelo RAM com palavras de \(\Theta(\log n)\) bits), mas quando os pesos são muito grandes a complexidade de bits pode ser determinante.}. Abaixo apresentamos, de forma breve, as classes mais usadas:

\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{P} ("polynomial time"): problemas de \emph{decisão} resolvíveis em tempo polinomial. Ex.: conectividade em grafos, árvore geradora mínima (MST), caminho mínimo com pesos não negativos, fluxo máximo.
    \item \textbf{NP} ("nondeterministic polynomial time"): problemas de decisão cujas soluções/certificados podem ser \emph{verificados} em tempo polinomial. Ex.: \emph{SAT} (satisfatibilidade booleana), \emph{Clique}, \emph{Vertex Cover}.
    \item \textbf{co-NP}: complementos dos problemas em NP. Ex.: \emph{TAUT} (verificar se uma fórmula é tautologia) está em co-NP.
    \item \textbf{NP-difícil}: pelo menos tão difíceis quanto os problemas de NP; todo problema em NP reduz-se (no sentido polinomial) a eles. Podem ser de decisão, otimização ou contagem e \emph{não precisam} estar em NP. Ex.: versão de otimização do \emph{TSP} (caixeiro-viajante), programação inteira, coloração mínima de grafos.
    \item \textbf{NP-completo}: problemas \emph{de decisão} que são ao mesmo tempo \emph{NP-difíceis} e pertencem a \emph{NP}. Se algum NP-completo tiver algoritmo polinomial, então \(\mathrm{P}=\mathrm{NP}\). Ex.: \emph{SAT}, \emph{3-SAT}, \emph{Hamiltoniano} (existe ciclo hamiltoniano?).
    \item \textbf{PSPACE}: problemas resolvíveis com \emph{espaço} polinomial (tempo pode ser maior). Ex.: \emph{QBF} (satisfatibilidade com quantificadores) é PSPACE-completo.
\end{itemize}

\paragraph{Reduções polinomiais}
Para comparar dificuldades, usamos reduções: dizemos que \(A\) reduz-se a \(B\) em tempo polinomial (\(A\le_p B\)) se uma solução para \(B\) permite resolver \(A\) com sobrecusto polinomial. Mostrar que um problema é NP-difícil (ou NP-completo) normalmente requer construir tais reduções a partir de problemas conhecidos.

\paragraph{Relações conhecidas}
Sabemos que \(\mathrm{P}\subseteq \mathrm{NP}\) e \(\mathrm{P}\subseteq \mathrm{co\text{-}NP}\), e também que \(\mathrm{P}\subseteq \mathrm{NP}\subseteq \mathrm{PSPACE}\subseteq \mathrm{EXP}\). As igualdades entre essas classes (em especial \(\mathrm{P}\) vs \(\mathrm{NP}\)) são problemas em aberto famosos.
...
\paragraph{}
É comum ouvirmos que “o melhor é inimigo do bom”. Essa frase, atribuída a Voltaire, expressa a ideia de que buscar a perfeição pode impedir que se alcance um resultado satisfatório. Essa tensão entre buscar o ideal do “ótimo”  ou aceitar o “suficientemente bom” quando recursos e tempo são limitados\footnote{Na teoria da decisão, essa postura pragmática é conhecida como \emph{satisficing}, termo introduzido por Herbert A. Simon.}. se materializa em mais uma forma de classificação de algoritmos:  \emph{algoritmos exatos} e \emph{algoritmos heurísticos}. Os primeiros vêm com garantias formais de optimalidade, enquanto os segundos privilegiam rapidez e simplicidade, buscando boas soluções na prática, por vezes com garantias de aproximação.
...
\paragraph{}
Além dessas distinções precisamos citar mais alguns conceitos úteis na análise de algoritmos:
\begin{itemize}\setlength{\itemsep}{2pt}
...
\paragraph{}
Os problemas de otimização, certamente se enquadram nessa turma. O objetivo final (os fins) é encontrar a r-arborescência de custo mínimo, que é uma estrutura que conecta todos os vértices de um dígrafo de forma eficiente e econômica. Esse objetivo é claro e bem definido. Mas, os meios para alcançá-lo são igualmente importantes.

\paragraph{}
Por meios entendemos os algoritmos e técnicas que utilizamos para encontrar essa arborescência: eles precisam ser eficientes, corretos e capazes de lidar com a complexidade do problema e também revelam diversas propriedades interessantes dos dígrafos e das arborescências.

\paragraph{}
Nesta seção, abordaremos os meios na busca por r-arborescências de custo mínimo, falaremos sobre o algoritmo de Chu–Liu/Edmonds e o procedimento em duas fases de András Frank, mas antes,começaremos com uma contextualização 
histórica de como esses algoritmos foram desenvolvidos.


\paragraph{História}
\paragraph{}
Antes de tratarmos do caso \emph{dirigido}, a intuição dominante sobre \emph{como construir estruturas de conectividade de menor custo} vinha do caso de \emph{grafos não dirigidos}: as \textbf{árvores geradoras mínimas}\footnote{Definição. Em um grafo não dirigido, conexo e ponderado \(G=(V,E)\) com pesos \(w:E\to\mathbb{R}\), uma árvore geradora mínima é um subconjunto de arestas \(T\subseteq E\) que forma uma árvore (conecta todos os vértices, é acíclica e tem \(|T|=|V|-1\)) e que minimiza \(\sum_{e\in T} w(e)\).} (\emph{Minimum Spanning Trees}, MST). A regra geral era: “escolha sempre a aresta mais barata disponível e encontraremos uma estrutura ótima”. Em grafos \emph{não dirigidos}, essa ideia realmente funciona porque há duas regras estruturais muito fortes que “protegem” as escolhas locais:

\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Princípio do corte (regra do corte seguro).} Um \emph{corte} é uma separação do conjunto de vértices em duas partes \(S\) e \(V\setminus S\). Dizemos que uma aresta “cruza” o corte se tem uma ponta em cada lado. O princípio afirma: \emph{a aresta de menor peso que cruza qualquer corte é segura}, ou seja, pode ser incluída em alguma MST sem perder optimalidade.
    \paragraph{}
    Intuitivamente, dizemos que se alguma solução ótima usa uma aresta \(e^*\) que cruza um certo corte e existe outra aresta \(e\) \emph{mais barata} cruzando o mesmo corte, podemos trocar \(e^*\) por \(e\). A troca mantém o grafo conectado (o corte continua sendo cruzado) e não aumenta o custo. Portanto, a mais barata do corte é sempre segura.
    \item \textbf{Princípio do ciclo (regra da aresta pesada).} Em qualquer \emph{ciclo} (um passeio fechado sem repetir arestas), a aresta de maior peso \emph{não} pode pertencer a uma MST, pois existe uma troca que reduz (ou não aumenta) o custo total removendo essa aresta pesada.
    \paragraph{}
    Ou seja, em um ciclo \(C\), remover a aresta \emph{mais pesada} não desconecta o grafo (há caminho alternativo dentro do próprio ciclo). Como é a mais cara, retirá-la só pode reduzir (ou manter) o custo. Logo, nenhuma MST precisa conter a aresta mais pesada de um ciclo.
\end{itemize}

\paragraph{}
Esses dois princípios justificam os gulosos clássicos:
\begin{itemize}\setlength{\itemsep}{2pt}
...
\end{itemize}
Por isso, em grafos não dirigidos, “pegar as arestas mais baratas” funciona: trocas locais seguras garantem a otimalidade global. Assim o problema de encontrar uma árvore geradora mínima (MST) consiste em dado um grafo (não dirigido), conexo e ponderado \(G=(V,E)\) com pesos \(w:E\to\mathbb{R}\), queremos um subconjunto de arestas \(T\subseteq E\) que conecta todos os vértices sem ciclos (forma uma árvore) e minimiza \(\sum_{e\in T} w(e)\). Duas soluções gulosas consagradas:
...
\begin{enumerate}\setlength{\itemsep}{2pt}
    ...
    \item Inicialize um \emph{Union-Find} com cada vértice em seu próprio conjunto; comece com \(T=\emptyset\).
...
\begin{enumerate}\setlength{\itemsep}{2pt}
    \item Inicialize \(T=\{s\}\) e uma fila de prioridades com as arestas que saem de \(T\), chaveando pelo menor peso.
...
\paragraph{}Prim também é correto pelo princípio do corte; com fila de prioridades binária, executa em \(O(m\log n)\) (ou em \(O(m+n\log n)\)); em grafos densos com \emph{heaps} de Fibonacci, pode-se obter \(O(m+n\log n)\) \cite{cormen2009,kleinberg2006,west2001introduction,diestel2017graph}.

\paragraph{Revisão dos Conceitos Fundamentais}

\paragraph{}
A revisão resumida dos conceitos fundamentais (que já apresentamos em sessões anteriores) e das técnicas que serão utilizadas.
\paragraph{}
...
\paragraph{}Contudo, ao migrarmos para \emph{dígrafos}, essa inocência desaparece. Na variante dirigida, queremos uma \textbf{r-arborescência de custo mínimo}: para cada vértice \(v\neq r\), exatamente \emph{um arco de entrada} deve ser escolhido, e o conjunto resultante precisa ser acíclico e alcançável a partir de \(r\). Se tentarmos copiar a estratégia gulosa das MSTs — por exemplo, escolher para cada vértice o arco de entrada mais barato — surgem \emph{ciclos dirigidos}. Além disso, os princípios estruturais usados na prova de otimalidade de MST (corte/ciclo em grafos não dirigidos) não se traduzem diretamente para o caso dirigido. É nesse cenário que entram as ideias de \textbf{Chu–Liu} e \textbf{Edmonds}: manter \emph{custos reduzidos} (criando arcos de custo zero) e lidar com conflitos via \emph{contração de ciclos}. A síntese dessas ideias, o algoritmo de \emph{Chu–Liu/Edmonds}, alia escolhas locais baratas a um mecanismo global de consistência e garante otimalidade no caso dirigido \cite{schrijver2003comb}.
