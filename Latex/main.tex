...
\paragraph{}
Para orientar a leitura, apresentamos um roteiro das seções. Em \textbf{Definições Preliminares}, fixamos notação e conceitos fundamentais (dígrafos, arborescências, custos e dualidade). Em \textbf{Em busca da Arborescência Perdida}, motivamos o problema e delineamos intuições que guiam os algoritmos. As seções \textbf{Algoritmo de Chu--Liu/Edmonds} e \textbf{Procedimento em Duas Fases de András Frank} detalham, respectivamente, a abordagem primal por normalização/contração de ciclos e a abordagem dual por cortes c‑viáveis, com ênfase em invariantes e correção. Em \textbf{Discussão e Resultados: Chu-liu/Edmonds vs. Frank}, comparamos desempenho e estrutura das soluções e reportamos testes em larga escala. A \textbf{Didática do Abstrato} discute fundamentos cognitivos e princípios de IHC aplicados ao ensino do tema. Em \textbf{A interação humano--computacional em ação: uma aplicação web interativa}, apresentamos a aplicação que utiliza PyScript, NetworkX, Matplotlib, suas páginas e decisões de design para montar instâncias, escolher a raiz, executar os algoritmos e acompanhar logs e visualizações. Por fim, em \textbf{Conclusão} sintetizamos contribuições, limitações e trabalhos futuros.
...
\paragraph{\texttt{Chuliu.html}:} página dedicada ao visualizador do algoritmo de Chu-Liu/Edmonds. Inclui um passo a passo guiado para criar um grafo, selecionar o nó raiz e executar o algoritmo, com feedback visual e textual. A seguir, apresentamos o código completo dessa página.
...
\paragraph{\texttt{Andrasfrank.html}:} página dedicada ao visualizador do algoritmo de Andras Frank. Inclui um passo a passo guiado para criar um grafo, selecionar o vértice raiz e executar o algoritmo, com feedback visual e textual. A seguir, apresentamos o código completo dessa página.
...
\begin{tikzpicture}[node distance=2.2cm,>=Stealth]
    ...
    \draw[->] (draw) -- (ch);
    \draw[->] (ch) -- (afp);
    \foreach \x in {home,draw,ch,afp} {\draw[->] (sb) -- (\x);}
\end{tikzpicture}
