...
\paragraph{}
Descrevendo informalmente, “primal–dual” significa acompanhar duas coisas ao mesmo tempo, que se verificam mutuamente:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Solução primal:} a estrutura que queremos construir (a arborescência) — ou o grafo após contrações.
    \item \textbf{Solução dual:} um número \(y(v)\) para cada vértice (os “potenciais”: pequenos ajustes numéricos por vértice que funcionam como um \emph{desconto} aplicado a todas as arestas que \emph{entram} em \(v\)). Com eles, definimos o \emph{custo reduzido} de um arco \((u,v)\) por \(c'(u,v)=c(u,v)-y(v)\). Arcos com \(c'=0\) são \emph{apertados}\,— isto é, \emph{tight} no jargão de PL: têm \textit{folga zero} (o custo original é exatamente compensado pelo potencial do vértice de chegada) — e são elegíveis na solução.
\end{itemize}

Em resumo, elevamos potenciais onde falta uma entrada de custo zero, criando novos arcos \emph{apertados}; sempre que surgir um ciclo só com apertados, contraímos e seguimos no grafo menor; ao final, extraímos uma r‑arborescência usando apenas arcos apertados. Como as escolhas finais são apertadas em relação a \(y\), a otimalidade decorre por complementaridade. As Fases I e II estão formalizadas nos Alg.\ \ref{th:frank-fase-I} e \ref{th:frank-fase} \cite{frank2014}.
...
\paragraph{}
Há duas leituras equivalentes e úteis: 

\begin{itemize}
    \item (i) uma formulação \emph{primal--dual} por cortes e; 
    \item (ii) um ponto de vista de \emph{potenciais} mais local (por vértice), que conduz à mesma mecânica operativa;
\end{itemize}

\paragraph{Formulação primal}
\paragraph{} 
Escolhemos variáveis \(x_a\in[0,1]\) para cada arco \(a\in A\); o objetivo é
\[
...
\paragraph{Formulação dual por cortes}
\paragraph{}
Associamos uma variável dual \(y(X)\ge 0\) a cada \(\emptyset\ne X\subseteq V\setminus\{r\}\) e maximizamos
\[
...
\[
    ...
    \sum_{X:\, v\in X,\ u\notin X} y(X),
\]
as restrições duais dizem exatamente que \(c'(u,v)\ge 0\) para todo arco. Arcos com \(c'(u,v)=0\) são chamados \emph{apertados}. 

Por um argumento de \emph{uncrossing}\footnote{“Uncrossing” (descruzamento) é a técnica de substituir dois conjuntos que se cruzam, \(X\) e \(Y\), pelos conjuntos \(X\cap Y\) e \(X\cup Y\), observando que (i) a viabilidade das desigualdades por cortes é preservada e (ii) o valor do objetivo dual não diminui. Iterando esse passo, obtém-se uma solução cuja família de conjuntos ativos não se cruza (\emph{laminar}). Essa normalização é clássica em LPs de cortes; ver, por exemplo, \cite{schrijver2003comb,frank2014}.}, pode-se escolher uma solução dual ótima \(y\) cuja família \(\mathcal{L}=\{X: y(X)>0\}\) seja \emph{laminar} (os conjuntos não se cruzam), o que simplifica a implementação \cite{frank2014,schrijver2003comb}.

\paragraph{Potenciais por vértice}
\paragraph{}
Uma forma prática é manter um único número por vértice, o \emph{potencial} \(\tilde y(v)\). Ele atua como um desconto aplicado a todas as arestas que entram em \(v\):
\[
    c'(u,v) = c(u,v) - \tilde y(v).
\]
Se \(\tilde y\) vier do dual por cortes, ele é a soma dos \(y(X)\) dos conjuntos laminares \(X\) que contêm \(v\). Ao aumentar \(\tilde y(v)\), todos os arcos que entram em \(v\) ficam igualmente mais baratos; a ordem entre essas entradas não muda. Por isso, resolver com \(c\) ou com \(c'\) é equivalente. Na Fase I, ajustamos \(\tilde y\) até que todo \(v\neq r\) tenha ao menos uma entrada com \(c'(u,v)=0\).
...
\end{itemize}
Pelo descruzamento (\emph{uncrossing}; ver nota acima), existe solução dual ótima cuja família ativa \(\mathcal{L}=\{X: y(X)>0\}\) é \emph{laminar} (os conjuntos não se cruzam), o que guia as contrações \cite{frank2014,schrijver2003comb}. 
Na mecânica do algoritmo: elevamos potenciais até zerar a entrada de algum \(X\) “mínimo”, criando novos arcos apertados; se surgir um ciclo formado só por apertados, contraímo‑lo e seguimos no grafo reduzido, mantendo \(c'\ge 0\) e a correspondência primal–dual.
\paragraph{}
...
\noindent\textbf{Lema 1 (elevação de potenciais).} Seja \(X\subseteq V\setminus\{r\}\) e \(\Delta=\min c'(\delta^-(X))\). Atualizar \(y(X)\gets y(X)+\Delta\) (ou, na versão por vértices, \(\tilde y(x)\gets \tilde y(x)+\Delta\) para todo \(x\in X\)) mantém \(c'\ge 0\) e torna \emph{apertado} ao menos um arco que entra em \(X\).\newline
Prova. Para \((u,x)\in \delta^-(X)\), \(c'\) é reduzido em \(\Delta\); pela definição de \(\Delta\), nenhum fica negativo e os de menor folga zeram. Os demais arcos não mudam. \qed

\noindent\textbf{Lema 2 (contração de ciclo a zero).} Se \(D_0\) contém um ciclo dirigido \(C\) de arcos com \(c'=0\), então contrair \(C\) em um supervértice preserva viabilidade primal e dual e há uma bijeção de soluções ótimas entre a instância contraída e a original; ao reexpandir, basta manter o arco (de custo reduzido zero) que entra no componente e remover uma única aresta interna de \(C\).\newline
Prova (esboço formal). Todo arco interno de \(C\) tem \(c'=0\). Ao contrair, redirecionamos apenas arcos que entram/saem de \(C\), preservando \(c'\ge 0\). Dado um \(r\)-arborescência ótima \(T'\) no grafo contraído, reexpandir \(C\) e remover um arco interno restaura grau de entrada 1 e aciclicidade. Reciprocamente, contrair qualquer \(T\) ótimo produz \(T'\) viável no grafo menor. A soma dos custos reduzidos é preservada porque os internos de \(C\) valem 0 e entradas em \(C\) descontam exatamente o potencial aplicado, mantendo equivalência de custos \cite[Sec.~4.9]{schrijver2003comb}. \qed

\noindent\textbf{Lema 3 (cortes ativos entram exatamente uma vez).} Seja \(T\) uma r‑arborescência e \(X\subseteq V\setminus\{r\}\). Se contraímos \(X\) a um vértice, o grau de entrada desse vértice na imagem de \(T\) é 1; logo \(\sum_{a\in \delta^-(X)} x_a=1\). Em particular, para todo \(X\in\mathcal{L}\) com \(y(X)>0\), a restrição primal do corte é \emph{justa}.\newline
...
\noindent\textbf{Teorema.} O algoritmo devolve uma r‑arborescência ótima para \(c\).\newline
