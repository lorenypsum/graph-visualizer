...
\paragraph{Bibliotecas e dependências}
\paragraph{}
A aplicação utiliza as seguintes bibliotecas e dependências principais:
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{PyScript:} Permite a execução de código Python diretamente no navegador.
    \item \textbf{NetworkX:} Biblioteca para criação, manipulação e estudo de grafos complexos.
    \item \textbf{Matplotlib:} Utilizada para gerar visualizações estáticas dos grafos.
    \item \textbf{Tailwind CSS:} Framework CSS para estilos utilitários e design responsivo.
    \item \textbf{Cytoscape.js:} (importada para futuras implementações) Biblioteca JavaScript para visualização interativa de grafos.
\end{itemize}

\paragraph{Main.py: Lógica de manipulação de grafos}
\paragraph{}
O arquivo \texttt{main.py} contém funções para manipulação de grafos, execução dos algoritmos e geração de visualizações. A seguir, destacamos o código:
