...
Na prática, \texttt{update\_weights\_in\_X} é o passo que “cria” novas 0‑arestas: ele reduz os pesos das entradas de \(X\) e insere em \texttt{D\_zero}/\texttt{A\_zero} aquelas que atingirem 0. O processo se repete até que toda componente diferente da raiz tenha ao menos uma entrada de peso 0, preparando o terreno para a Fase II. Parâmetros opcionais \texttt{draw\_fn} e \texttt{log} controlam visualização e mensagens.
...
\paragraph{Fase 2 do algoritmo de Frank:}
...
\begin{itemize}\setlength{\itemsep}{1pt}
    ...
    \end{itemize}
    \item \emph{Desenho opcional:} se \texttt{draw\_fn} for fornecida, a arborescência parcial é desenhada a cada inclusão (título em EN/PT conforme \texttt{lang}).
