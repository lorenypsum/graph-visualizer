...
\begin{tikzpicture}[>=Stealth, node distance=1.4cm]
    ...
    \draw[->] (u2) -- node[above,align=center] {$c'(u,x_C)=c(u,w)-c(a_w)$} (xC);
\end{tikzpicture}
...
\begin{tikzpicture}[>=Stealth, node distance=1.2cm]
    ...
    % cycle C
    \draw[dashed,rounded corners] (2.6,-0.9) rectangle (4.6,1.7) node[above right] {$C$};
    \node[circle,draw,minimum size=6mm] (aC) at (3.0,1.2) {$a$};
    \node[circle,draw,minimum size=6mm] (bC) at (4.2,0.4) {$w$};
    \node[circle,draw,minimum size=6mm] (cC) at (3.2,-0.2) {$c$};
    \draw[->] (aC) -- (bC);
    \draw[->] (bC) -- (cC);
    \draw[->] (cC) -- (aC);
    % external edges
    ...
    \draw[->] (r2) -- (p2);
    \draw[->] (q2) -- (aC);
    \draw[->,very thick] (u2b) -- node[above] {entra em $w$} (bC);
\end{tikzpicture}
...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[dashed,rounded corners] (-1.2,-0.8) rectangle (1.2,1.6) node[above right] {$C$};
    ...
    \draw[dashed,rounded corners] (3.6,-0.8) rectangle (6.0,1.6) node[above right] {$C$};
...
\paragraph{}
Para tornar essa experiência concreta, desenvolvemos uma aplicação \textit{web} interativa que permite acompanhar, passo a passo, o funcionamento de ambos os algoritmos, evidenciando suas semelhanças e diferenças.
