...
\paragraph{Objetivo pedagógico}
Fornecer um ambiente de exploração incremental dos algoritmos de arborescência (Chu--Liu/Edmonds e variante dual de Frank em estágio inicial) enfatizando: (i) visibilidade de estados estruturais (arestas, pesos, escolhas), (ii) redução de carga extrínseca por layout estável, (iii) engajamento ativo por manipulação direta e (iv) registro persistente da execução (\emph{log}).

\paragraph{Visão geral das páginas}
A aplicação é composta por páginas HTML autônomas carregadas no navegador, cada uma focada em um aspecto: (i) \texttt{home.html} (apresentação / resumo), (ii) \texttt{chuliu.html} (execução passo a passo do algoritmo de Chu--Liu/Edmonds), (iii) \texttt{andrasfrank.html} (estrutura para futura visualização da abordagem primal--dual), (iv) \texttt{draw\_graph.html} (editor livre de grafos), (v) \texttt{tese.html} (incorpora o PDF gerado em \LaTeX), além do componente compartilhado de navegação lateral \texttt{sidebar.html}. Cada página injeta o \texttt{sidebar} dinamicamente e carrega scripts específicos.

\paragraph{Arquitetura tecnológica}
\begin{itemize}\setlength{\itemsep}{2pt}
...
\paragraph{Fluxo de interação}\label{par:fluxo-interacao}
\begin{enumerate}\setlength{\itemsep}{2pt}
...
O estado principal é mantido em memória (objeto \texttt{networkx.DiGraph}). O log textual funciona como uma \emph{trilha de auditoria didática}. Não há backend; portanto, a persistência entre sessões depende de exportação manual (JSON) — escolha deliberada para simplicidade e transparência pedagógica.

\paragraph{Mapeamento aos princípios de IHC (Tabela~\ref{tab:principios-ihc})}
Usabilidade: nomenclatura direta e ausência de menus profundos; Eficiência cognitiva: layout estável, ausência de animações distrativas; Feedback imediato: cada ação gera log + figura; Overview→Details: sequência de figuras + inspeção textual; Consistência semântica: estilos visuais fixos; Múltiplos registros: grafo, texto, parâmetros; Prevenção de erros: validações de campos e reinicialização controlada.
...
\paragraph{Extensões planejadas}
\begin{itemize}\setlength{\itemsep}{2pt}
...
\paragraph{Síntese}
A seção integra conceitos teóricos a affordances interativas concretas: cada decisão de design (layout estável, log paralelo, exportação explícita) funciona como um \emph{andame didático} alinhado aos princípios cognitivos e de IHC previamente discutidos, reduzindo a distância entre formalismo e experiência exploratória.
